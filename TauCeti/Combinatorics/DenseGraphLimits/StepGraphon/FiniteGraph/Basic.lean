...
* `TauCeti.DenseGraphLimits.finiteGraphGraphon_apply_of_pos`,
...
definition is total in `m`; see the module docstring. Use `finiteGraphGraphon_apply_of_pos` and the
...
@[simp]
theorem finiteGraphGraphon_apply_of_pos (G : SimpleGraph (Fin m)) [DecidableRel G.Adj]
...
theorem homDensity_finiteGraphGraphon (F : SimpleGraph V) [DecidableRel F.Adj] (hm : 0 < m)
  ...
  have hstep : ∀ x : V → I, ∏ e ∈ F.edgeFinset, edgeFactor (finiteGraphGraphon G) x e
    ...
    · rw [ite_eq_left h]
      ...
      | _ a b =>
        intro he
        rw [edgeFactor_mk, finiteGraphGraphon_apply, ite_eq_left (h a b (by simpa using he))]
    · rw [ite_eq_right h]
      push Not at h
      obtain ⟨a, b, hab, hnot⟩ := h
      refine Finset.prod_eq_zero (i := s(a, b)) (by simpa using hab) ?_
      rw [edgeFactor_mk, finiteGraphGraphon_apply, ite_eq_right hnot]

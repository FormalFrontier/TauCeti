...
noncomputable def toAlgEquivOfFinrankEq {K L M : Type*} [Field K] [Field L] [Field M]
    [Algebra K L] [Algebra K M] [FiniteDimensional K L] [FiniteDimensional K M]
    (f : L →ₐ[K] M) (hfin : Module.finrank K L = Module.finrank K M) : L ≃ₐ[K] M :=
...
@[simp]
theorem toAlgEquivOfFinrankEq_apply {K L M : Type*} [Field K] [Field L] [Field M]
    [Algebra K L] [Algebra K M] [FiniteDimensional K L] [FiniteDimensional K M]
    (f : L →ₐ[K] M) (hfin : Module.finrank K L = Module.finrank K M) (x : L) :
    TauCeti.AlgHom.toAlgEquivOfFinrankEq f hfin x = f x :=

...
* `TauCeti.Place.eq_infty_or_exists_eq_adic`,
  `TauCeti.Place.eq_infty_or_exists_eq_adicOfIrreducible` and `TauCeti.Place.ratFuncEquiv`: these
...
theorem adicOfIrreducible_eq_adicOfIrreducible_iff {q r : k[X]} (hq : Irreducible q)
  ...
  rw [adicOfIrreducible_def, adicOfIrreducible_def, (adic_injective k (RatFunc k)).eq_iff,
    ← Ideal.span_singleton_eq_span_singleton, ← HeightOneSpectrum.ofIrreducible_asIdeal hq,
    ← HeightOneSpectrum.ofIrreducible_asIdeal hr]
  exact ⟨congrArg HeightOneSpectrum.asIdeal, HeightOneSpectrum.ext⟩
...
theorem eq_infty_or_exists_eq_adicOfIrreducible (P : Place k (RatFunc k)) :

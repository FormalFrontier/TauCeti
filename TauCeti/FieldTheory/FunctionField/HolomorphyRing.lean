...
model `𝒪_S`, whose height one primes are — by `TauCeti.coe_holomorphyRing_subset_integers_iff`
and the dictionary of `TauCeti/FieldTheory/FunctionField/AffineModel/Prime.lean` — exactly the
places of `S`.
...
* `TauCeti.holomorphyRing`: the holomorphy ring `𝒪_S` of a set of places, as a `k`-subalgebra
  of `F`.
* `TauCeti.holomorphyRingHeightOneSpectrumEquiv`: the identification of `S` with the height one
  primes of `𝒪_S`, once `𝒪_S` is an affine model.
...
so it has a least element, realized by some function of the ideal. -/
theorem exists_mem_ideal_ord_le {P : Place k F} (hP : P ∈ S) {I : Ideal ↥(holomorphyRing S)}
    (hI : I ≠ ⊥) : ∃ z ∈ I, (z : F) ≠ 0 ∧
...
Proposition 3.2.10.  Weak approximation (`TauCeti.Place.exists_ne_zero_forall_mem_ord_eq`)
combines the functions of least order at the separate places of `S`
(`TauCeti.exists_mem_ideal_ord_le`) into a single one, by damping all but one summand at each
place. -/
theorem exists_mem_ideal_forall_ord_le (hS : S.Finite) {I : Ideal ↥(holomorphyRing S)}
    (hI : I ≠ ⊥) : ∃ t ∈ I, (t : F) ≠ 0 ∧
  ...
  have hord : ∀ Q ∈ hS.toFinset,
    ...
    intro Q hQ
    rw [Q.ord_sum_eq_of_forall_lt (f := fun P ↦ u P * (z P : F)) hQ (fun P _ ↦ hf0 P) (hlt Q hQ),
  ...
  · rw [hsum]
    exact Q₀.sum_ne_zero_of_forall_ord_lt (f := fun P ↦ u P * (z P : F)) hQ₀
      (fun P _ ↦ hf0 P) (hlt Q₀ hQ₀)
  · intro P hP w hwI hw
    rw [hsum, hord P (hS.mem_toFinset.mpr hP)]
    exact hzmin P hP w hwI hw

/-- **Stichtenoth, Proposition 3.2.10**: the holomorphy ring of a *finite* set of places of an
algebraic function field is a principal ideal domain.  A nonzero ideal is generated by any of its
functions of least order at every place of `S`, which
`TauCeti.exists_mem_ideal_forall_ord_le` produces, since `𝒪_S` is then divided by such a function
...
an affine model, whose height one primes are the places of `S`
(`TauCeti.holomorphyRingHeightOneSpectrumEquiv`). -/
theorem isPrincipalIdealRing_holomorphyRing (hS : S.Finite) :
  ...
  principal I := by
    ...
    · rcases eq_or_ne (w : F) 0 with hw0 | hw0
      · rw [show w = 0 from Subtype.ext hw0]
        exact zero_mem _
...
variable [IsDedekindDomain ↥(holomorphyRing S)] [IsFractionRing ↥(holomorphyRing S) F]
...
finite chart of the model `𝒪_S` is exactly `S`, so `S` is the place set of an affine model. -/
noncomputable def holomorphyRingHeightOneSpectrumEquiv (hF : IsFunctionField k F) :
    S ≃ IsDedekindDomain.HeightOneSpectrum ↥(holomorphyRing S) where
  toFun P := (P : Place k F).center
    ((forall_algebraMap_mem_integers_holomorphyRing_iff hF).mpr P.2)
  invFun 𝔭 := ⟨Place.ofPrime k F 𝔭, (forall_algebraMap_mem_integers_holomorphyRing_iff hF).mp
    (Place.algebraMap_mem_integers_ofPrime k F 𝔭)⟩
  left_inv _ := Subtype.ext (Place.ofPrime_center k F _ _)
  right_inv 𝔭 := Place.center_ofPrime k F 𝔭
...
theorem coe_holomorphyRingHeightOneSpectrumEquiv_symm_apply (hF : IsFunctionField k F)
    (𝔭 : IsDedekindDomain.HeightOneSpectrum ↥(holomorphyRing S)) :

...
needed — only that `⟨σ⟩` be finite, which is what Mathlib's `FixedPoints.toAlgAutMulEquiv` asks
for; it identifies a finite group of automorphisms with the Galois group of its fixed points, and
that fixed-point subfield is the one underlying `IntermediateField.fixedField`.
...
theorem isCyclic_fixedField_zpowers (σ : M ≃ₐ[K] M) [Finite (Subgroup.zpowers σ)] :
  ...
  isCyclic_of_surjective _ (FixedPoints.toAlgAutMulEquiv (Subgroup.zpowers σ) M).surjective
...
theorem zpowers_toAlgAutMulEquiv_self_eq_top (σ : M ≃ₐ[K] M)
  ...
  rw [show (FixedPoints.toAlgAutMulEquiv (Subgroup.zpowers σ) M) ⟨σ, Subgroup.mem_zpowers σ⟩
      = (FixedPoints.toAlgAutMulEquiv (Subgroup.zpowers σ) M).toMonoidHom
        ⟨σ, Subgroup.mem_zpowers σ⟩ from rfl,
    ← MonoidHom.map_zpowers, Subgroup.zpowers_mk_self_eq_top,
    Subgroup.map_top_of_surjective
      (FixedPoints.toAlgAutMulEquiv (Subgroup.zpowers σ) M).toMonoidHom
      (FixedPoints.toAlgAutMulEquiv (Subgroup.zpowers σ) M).surjective]

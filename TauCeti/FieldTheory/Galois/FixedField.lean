...
The last results specialise the correspondence to a *cyclic* subgroup: the field fixed by `⟨σ⟩`
has `M` cyclic over it, generated by `σ` itself. Neither `M / K` Galois nor `M / K` finite is
needed — only that `⟨σ⟩` be finite.
...
/-- **A finite group of automorphisms is isomorphic to the Galois group of its fixed field.**

Mathlib's `IntermediateField.subgroupEquivAlgEquiv` is the same isomorphism under
`[FiniteDimensional K M]`; this one asks only that `H` itself be finite, which an infinite
extension can still supply. -/
noncomputable def equivAlgEquivFixedField (H : Subgroup (M ≃ₐ[K] M)) [Finite H] :
...
theorem isCyclic_fixedField_zpowers (σ : M ≃ₐ[K] M) [Finite (Subgroup.zpowers σ)] :
  ...
  isCyclic_of_surjective _ (Subgroup.equivAlgEquivFixedField (Subgroup.zpowers σ)).surjective

/-- **And `σ` is a generator.** Its image under the identification of `⟨σ⟩` with
`Gal(M / M ^ ⟨σ⟩)` generates that Galois group.

A fibre count needs the relative Frobenius exhibited as a specific power of a chosen `σ`, which
`IsCyclic` alone does not give. -/
theorem zpowers_equivAlgEquivFixedField_self_eq_top (σ : M ≃ₐ[K] M)
    [Finite (Subgroup.zpowers σ)] :
    Subgroup.zpowers (Subgroup.equivAlgEquivFixedField (Subgroup.zpowers σ)
        ⟨σ, Subgroup.mem_zpowers σ⟩) = ⊤ := by
  rw [show (Subgroup.equivAlgEquivFixedField (Subgroup.zpowers σ)) ⟨σ, Subgroup.mem_zpowers σ⟩
      = (Subgroup.equivAlgEquivFixedField (Subgroup.zpowers σ)).toMonoidHom
        ⟨σ, Subgroup.mem_zpowers σ⟩ from rfl,
    ← MonoidHom.map_zpowers, Subgroup.zpowers_mk_self_eq_top,
    Subgroup.map_top_of_surjective _
      (Subgroup.equivAlgEquivFixedField (Subgroup.zpowers σ)).surjective]

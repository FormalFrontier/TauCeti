...
theorem exists_sq_mem_bot_adjoin_eq_of_finrank_eq_two [NeZero (2 : K)]
  ...
  -- Its minimal polynomial is quadratic, because `K⟮y⟯` is a nontrivial subfield of `E`.
  ...
  have hdeg : (minpoly K y).natDegree = 2 := by
    have hne1 : Module.finrank K (IntermediateField.adjoin K {y}) ≠ 1 := fun h =>
      hyb (IntermediateField.adjoin_simple_eq_bot_iff.mp
        (IntermediateField.finrank_eq_one_iff.mp h))
    ...
    omega
  have hEeq : IntermediateField.adjoin K {y} = E :=
    IntermediateField.eq_of_le_of_finrank_eq hle (by rw [hadj, hdeg, hE])
  -- Read off the monic quadratic `y² + b y + c = 0` satisfied by `y`.
  obtain ⟨b, c, hrel⟩ : ∃ b c : K, y ^ 2 + algebraMap K L b * y + algebraMap K L c = 0 := by

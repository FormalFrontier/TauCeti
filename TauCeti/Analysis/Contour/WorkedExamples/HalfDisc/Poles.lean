...
each enclosed pole contributes its full residue, while the pole *on* the contour contributes only
half of its own. The classical residue theorem cannot reach the second term at all, because that
pole lies on the path of integration and the integral exists only as a principal value.

The `S = ∅` case, where the on-contour pole is the whole story, is
`WorkedExamples/HalfDisc/HalfResidue.lean`; the case where `f` is regular at the origin is the
classical residue theorem for this contour.

The pole on the real axis is pinned at the origin, as throughout the half-disc development: the
contour `halfDiscBoundary` is centred there, and a real pole elsewhere is reached by centring the
contour on it instead. Every pole is required to be at worst simple, which is what makes the
Hungerbühler–Wasem conditions (A′) and (B) automatic.

## Main results

* `TauCeti.Contour.hasCauchyPV_halfDiscBoundary_of_simple_poles` — the principal value along the
  half-disc boundary, with enclosed poles and a pole on the contour at the origin.
...
/-- The origin is not one of the enclosed poles: those lie strictly above the real axis. -/
private theorem zero_notMem_of_im_pos (hS : ∀ s ∈ S, 0 < s.im ∧ ‖s‖ < R) : (0 : ℂ) ∉ S :=
  fun h => absurd (hS 0 h).1 (by simp)

/-- **The half-disc residue theorem with enclosed poles.** Let `f` be holomorphic off the finite
set `insert 0 S`, with at worst a simple pole at each of its points, and let every point of `S` lie
in the open upper half-disc of radius `R`. Then the Cauchy principal value of `∮ f` along the
half-disc boundary is

`2π i · Σ_{s ∈ S} Res_s f + π i · Res₀ f`:

each enclosed pole contributes its full residue, because the generalized winding number there is
`1`, while the pole *on* the contour contributes only half of its own, the winding number at a
smooth crossing being `½`. -/
theorem hasCauchyPV_halfDiscBoundary_of_simple_poles {f : ℂ → ℂ} (hR : 0 < R)
  ...
  rw [Finset.sum_insert (zero_notMem_of_im_pos hS), windingNumber_halfDiscBoundary hR,
    Finset.sum_congr rfl (fun s hs => by
      rw [windingNumber_halfDiscBoundary_eq_one hR (hS s hs).1 (hS s hs).2, one_mul])] at key
  rw [show 2 * (Real.pi : ℂ) * Complex.I * (∑ s ∈ S, residue f s) +
      (Real.pi : ℂ) * Complex.I * residue f 0
      = 2 * (Real.pi : ℂ) * Complex.I * (1 / 2 * residue f 0 + ∑ s ∈ S, residue f s) by ring]
  exact key
...
residue theorem cannot reach, because one of the poles sits on the line of integration. -/

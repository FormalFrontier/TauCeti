...
theorem comp_spatial {Φ : Type*} [FunLike Φ W V] [AddHomClass Φ W V]
  ...
  have hK := hF.posSemidef.submatrix (fun p : ℝ≥0 × W => (p.1, φ p.2))
  have heq : Matrix.submatrix
      (fun p q : ℝ≥0 × V => F (p.1 + q.1, p.2 - q.2))
      (fun p : ℝ≥0 × W => (p.1, φ p.2)) (fun p => (p.1, φ p.2)) =
      fun p q : ℝ≥0 × W =>
        (fun x : ℝ≥0 × W => F (x.1, φ x.2)) (p.1 + q.1, p.2 - q.2) := by
    ext p q
    simp [Matrix.submatrix, map_sub_of_addHomClass φ]
  exact heq ▸ hK
...
theorem of_comp_spatial_surjective {Φ : Type*} [FunLike Φ W V] [AddHomClass Φ W V]
  ...
  have hK := hcomp.posSemidef.submatrix (fun p : ℝ≥0 × V => (p.1, w p.2))
  have heq : Matrix.submatrix
      (fun p q : ℝ≥0 × W =>

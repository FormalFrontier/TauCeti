...
The mechanism is moment determinacy. If the weighted measure `w·μ` has all exponential moments
finite then `TauCeti.ae_eq_zero_of_forall_moment_eq_zero_of_finite_expMoments` says a
...
* `TauCeti.memLp_two_algebraMap_eval` — polynomials lie in `L²` of a measure with all exponential
  moments finite.
...
/-- A measure with all exponential moments finite integrates every monomial squared: `xⁿ` lies in
`L²`, because `|x|ⁿ ≤ n!·e^{|x|}` and `e^{|x|}` is square-integrable. -/
theorem memLp_two_algebraMap_pow {ν : Measure ℝ}
  ...
  have hexp2 : MemLp (fun x : ℝ => Real.exp (1 * |x|)) 2 ν := by
    refine (memLp_two_iff_integrable_sq (hexpmeas 1)).2 ?_
    have h := hexp 2 (by norm_num)
    have hfun : (fun x : ℝ => Real.exp (2 * |x|))
        = fun x : ℝ => Real.exp (1 * |x|) ^ 2 := by
      funext x
      rw [sq, ← Real.exp_add]
      ring_nf
    exact hfun ▸ h
  refine MemLp.of_le (hexp2.const_mul (Nat.factorial n : ℝ)) ?_ ?_
...
/-- Every polynomial lies in `L²` of a measure with all exponential moments finite. -/
...
If the weighted measure `w·μ` has every exponential moment finite and `p n` has degree exactly `n`,
then the normalized polynomials span a dense subspace of `L²(w·μ)` — the orthogonal complement of
their span is trivial. This is the `hcomplete` input that

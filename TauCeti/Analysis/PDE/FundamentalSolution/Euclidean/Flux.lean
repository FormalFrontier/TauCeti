...
For dimension `n ≥ 3`, this file computes the outward normal derivative of the normalized
Newtonian kernel on a sphere. Integrating against the canonical measure on the unit sphere,
with the radial surface Jacobian, gives total flux `-1` through every sphere centered at the
pole. This is the boundary normalization needed for the distributional identity
`-Δ Gₙ = δ₀`.
...
* `TauCeti.fderiv_newtonianKernel_sub_sphere_normal`: the outward normal derivative on a sphere.
* `TauCeti.integral_fderiv_newtonianKernel_sub_sphere_normal`: the total sphere flux is `-1`.
-/
...
@[simp]
theorem fderiv_newtonianKernel_sub_sphere_normal (n : ℕ) (hn2 : n ≠ 2)
    ...
    fderiv ℝ (fun y ↦ newtonianKernel n (y - a)) (a + r • (u : EuclideanSpace ℝ (Fin n))) u =
      -(((n : ℝ) * volume.real (ball (0 : EuclideanSpace ℝ (Fin n)) 1))⁻¹ *
        r ^ (1 - (n : ℝ))) := by
  have hu_norm : ‖(u : EuclideanSpace ℝ (Fin n))‖ = 1 := by
    simpa only [mem_sphere, dist_zero_right] using u.property
  have hru : r • (u : EuclideanSpace ℝ (Fin n)) ≠ 0 := by
    rw [ne_eq, ← norm_eq_zero, norm_smul, Real.norm_eq_abs, abs_of_pos hr, hu_norm]
    exact mul_ne_zero hr.ne' one_ne_zero
  have hnorm_ru : ‖r • (u : EuclideanSpace ℝ (Fin n))‖ = r := by
    rw [norm_smul, Real.norm_eq_abs, abs_of_pos hr, hu_norm, mul_one]
  ...
  have hpow : r ^ (-(n : ℝ)) * r = r ^ (1 - (n : ℝ)) := by
    calc
      r ^ (-(n : ℝ)) * r = r ^ (-(n : ℝ)) * r ^ (1 : ℝ) := by
        rw [Real.rpow_one]
      _ = r ^ (-(n : ℝ) + 1) := (Real.rpow_add hr _ _).symm
      _ = r ^ (1 - (n : ℝ)) := by ring_nf
  calc
    -(((n : ℝ) * volume.real (ball (0 : EuclideanSpace ℝ (Fin n)) 1))⁻¹ *
          r ^ (-(n : ℝ)) * r) =
        -(((n : ℝ) * volume.real (ball (0 : EuclideanSpace ℝ (Fin n)) 1))⁻¹ *
...
theorem integral_fderiv_newtonianKernel_sub_sphere_normal (n : ℕ) (hn0 : n ≠ 0) (hn2 : n ≠ 2)
  ...
  simp_rw [fderiv_newtonianKernel_sub_sphere_normal n hn2 hr]
  rw [integral_const]
  ...
  have hvol : 0 < volume.real (ball (0 : EuclideanSpace ℝ (Fin n)) 1) := by

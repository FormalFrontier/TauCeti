...
* `TauCeti.isJordanDomain_of_convex` — the same for an arbitrary bounded convex domain, on the
...
many, since an infinite intersection of open half-planes is convex but need not be open. Each of
the three defining conditions comes straight from convexity: a nonempty convex set is connected,
and its frontier is a Jordan curve by `TauCeti.isJordanCurve_frontier_of_convex`. -/

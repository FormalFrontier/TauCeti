...
public import TauCeti.Analysis.Complex.PlaneSeparation
...
strictly generalising the circle case proved in `TauCeti.Analysis.Normed.Module.FilledHull` and
supplying a concrete test of the `filledHull` interface that `Conformal/Caratheodory.lean` consumes.

...
Here an arc is presented as the range of an injective path `γ : Path x y`; the main theorem is
`TauCeti.hasContinuousLogOn_range_of_injective_path`.
...
`Complex.exists_continuousOn_eqOn_exp_comp` supplies the lift. An injective path from the compact
unit interval into a Hausdorff space is an embedding, so its parametrization is a homeomorphism
onto its range and the lifted logarithm descends along that homeomorphism to the arc itself. The
Borsuk-map specialization `TauCeti.hasContinuousLogOn_sub_div_sub_range_of_injective_path`
records the form used by plane separation. No formalization is vendored.
...
This is deliberately stated for paths in an arbitrary Hausdorff space, since neither the lifting
...
* `TauCeti.hasContinuousLogOn_range_of_injective_path` — every zero-free continuous function on
  the range of an injective path has a continuous logarithm.
...
variable {X : Type*} [TopologicalSpace X] [T2Space X] {x y : X} {g : X → ℂ}

/-- **A zero-free continuous function on a simple arc has a continuous logarithm.** Let `γ` be an
injective path in a Hausdorff space. If `g` is continuous and nonzero on `range γ`, then there is a
function continuous on `range γ` whose exponential is `g` there.

The path is first extended constantly to the simply connected line, where its pullback of `g` is
lifted through `Complex.exp`. Compactness of the unit interval and the Hausdorff hypothesis make
`γ` an embedding, so its inverse on the range is continuous and the lift descends to the arc. -/
theorem hasContinuousLogOn_range_of_injective_path (γ : Path x y)
    (hγ : Function.Injective γ) (hg : ContinuousOn g (range γ))
    (hzero : ∀ z ∈ range γ, g z ≠ 0) : HasContinuousLogOn g (range γ) := by
  classical
  let G : ℝ → ℂ := fun t => g (γ.extend t)
  have hγe : IsEmbedding γ :=
    (IsClosedEmbedding.of_continuous_injective_isClosedMap γ.continuous hγ
      γ.continuous.isClosedMap).isEmbedding
...
arc.** This is `TauCeti.hasContinuousLogOn_range_of_injective_path` applied to
`z ↦ (z - a) / (z - b)`. The two nonmembership hypotheses say exactly that its numerator and
denominator do not vanish along the arc.
...
theorem hasContinuousLogOn_sub_div_sub_range_of_injective_path {p q a b : ℂ} (γ : Path p q)
  ...
  refine hasContinuousLogOn_range_of_injective_path γ hγ

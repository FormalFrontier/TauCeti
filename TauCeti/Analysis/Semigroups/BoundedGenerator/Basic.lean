...
These two facts are about the exponential on the Banach algebra `X →L[ℝ] X` itself; neither
mentions a semigroup, so they live here rather than inside `StronglyContinuousSemigroup`, which
consumes them to build `ofBounded` below. Also consumed by the Yosida approximation
(`TauCeti/Analysis/Semigroups/YosidaApproximation.lean`), which is why they are not private. -/

omit [CompleteSpace X] in
/-- `‖exp x‖ ≤ Real.exp ‖x‖`, for `x : X →L[ℝ] X`. -/
...
/-- `exp (x + y) = exp x * exp y`, for commuting `x y : X →L[ℝ] X`.

Not a restatement of `NormedSpace.exp_add_of_commute`: that lemma needs
`NormedAlgebra ℚ (X →L[ℝ] X)`, an instance that is not available here. This is the ℝ-scalar
form, via `NormedSpace.exp_add_of_commute_of_mem_ball`. -/

...
`yosidaLimit A t x`, defined for every real `t` as the limit of `exp (t A_lambda) x` along
`lambda -> +∞`. The limit is linear and contractive in `x`, satisfies `S(0) = I` and
`S(s + t) = S(s) S(t)` at nonnegative times, and — because the convergence is uniform on compact
time intervals — depends continuously on `t`. Packaging these facts gives a genuine contraction
semigroup `yosidaLimitSemigroup`.
...
* `TauCeti.Semigroups.yosidaLimit`: the vector `lim_{lambda -> ∞} exp (t A_lambda) x`.
...
/-- The **Yosida limit** of an unbounded operator `A` at time `t`, applied to `x`: the limit of
`exp (t A_lambda) x` as `lambda -> +∞`.

The definition is meaningful when `A` is m-dissipative and `t ≥ 0`, where the Cauchy estimate
`IsMDissipative.exp_yosidaApproximation_uniformCauchySeqOn_compact` makes the limit exist; the
API below carries those hypotheses explicitly. -/
...
This is the existence half of the Lumer--Phillips generation theorem; identifying the generator
of the resulting semigroup with `A` is what remains. -/

...
* `TauCeti.Semigroups.IsMDissipative.exp_yosidaApproximation_cauchyOn_compact`: the Yosida
  semigroups are Cauchy uniformly on compact time intervals on `D(A)`.
...
theorem IsMDissipative.tendsto_smul_resolvent_apply_atTop {A : X →ₗ.[ℝ] X}
  ...
  have hTnorm : ‖T‖ ≤ 1 := by
    rw [show T = lambda • LinearPMap.resolvent A lambda by rfl, norm_smul,
      Real.norm_eq_abs, abs_of_pos hlambda_pos]
  ...
  have hdecomp : T x - x = T (x - y) + (T y - y) + (y - x) := by
    rw [map_sub]
    abel
  rw [show lambda • LinearPMap.resolvent A lambda x = T x by rfl, dist_eq_norm, hdecomp]
  calc
    ‖T (x - y) + (T y - y) + (y - x)‖
...
theorem IsMDissipative.exp_yosidaApproximation_cauchyOn_compact_of_mem {A : X →ₗ.[ℝ] X}
    (hA : IsMDissipative A) (hdense : Dense (A.domain : Set X)) (x : A.domain)
    {T : ℝ} (hT : 0 ≤ T) {epsilon : ℝ} (hepsilon : 0 < epsilon) :
    ∃ L : ℝ, ∀ lambda mu : ℝ, L ≤ lambda → L ≤ mu →
      ∀ t : ℝ, t ∈ Set.Icc 0 T →
        ‖exp (t • yosidaApproximation A lambda) (x : X) -
          exp (t • yosidaApproximation A mu) (x : X)‖ < epsilon := by
  let C : ℝ := T + 1
  ...
  refine ⟨max N 1, fun lambda mu hlambda hmu t ht => ?_⟩
...
`A`. The domain case is `exp_yosidaApproximation_cauchyOn_compact_of_mem`; contractivity extends
it to the whole space by density of `D(A)`. -/
theorem IsMDissipative.exp_yosidaApproximation_cauchyOn_compact {A : X →ₗ.[ℝ] X}
    (hA : IsMDissipative A) (hdense : Dense (A.domain : Set X)) (x : X)
    {T : ℝ} (hT : 0 ≤ T) {epsilon : ℝ} (hepsilon : 0 < epsilon) :
    ∃ L : ℝ, ∀ lambda mu : ℝ, L ≤ lambda → L ≤ mu →
      ∀ t : ℝ, t ∈ Set.Icc 0 T →
        ‖exp (t • yosidaApproximation A lambda) x -
          exp (t • yosidaApproximation A mu) x‖ < epsilon := by
  obtain ⟨y, hy, hxy⟩ := hdense.exists_dist_lt x (by positivity : 0 < epsilon / 3)
  have hxy_norm : ‖x - y‖ < epsilon / 3 := by simpa only [dist_eq_norm] using hxy
  let y' : A.domain := ⟨y, hy⟩
  obtain ⟨L, hL⟩ := hA.exp_yosidaApproximation_cauchyOn_compact_of_mem hdense y' hT
    (by positivity : 0 < epsilon / 3)
  refine ⟨max L 1, fun lambda mu hlambda hmu t ht => ?_⟩
  have hlambda_pos : 0 < lambda :=
    lt_of_lt_of_le zero_lt_one (le_trans (le_max_right _ _) hlambda)
  have hmu_pos : 0 < mu :=
    lt_of_lt_of_le zero_lt_one (le_trans (le_max_right _ _) hmu)
  ...
  have hmiddle : ‖Slambda y - Smu y‖ < epsilon / 3 := by
    simpa only [Slambda, Smu] using
      hL lambda mu (le_trans (le_max_left _ _) hlambda)
        (le_trans (le_max_left _ _) hmu) t ht
  have hdecomp : Slambda x - Smu x =
      Slambda (x - y) + (Slambda y - Smu y) + Smu (y - x) := by
    rw [map_sub, map_sub]
    abel
  rw [show exp (t • yosidaApproximation A lambda) x = Slambda x by rfl,
    show exp (t • yosidaApproximation A mu) x = Smu x by rfl, hdecomp]
  calc

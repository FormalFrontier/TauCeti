...
The completeness of the space then turns this Cauchy property into a semigroup, exactly as in the
m-dissipative case treated in
`TauCeti/Analysis/Semigroups/Generation/LimitSemigroup.lean`; that limit, its growth bound `(0, M)`
and the identification of its generator are the remaining steps of the generation theorem, and the
reduction of the general growth exponent to zero is
`TauCeti.LinearPMap.hilleYosida_zero_of`.

## Main results

* `TauCeti.Semigroups.exp_yosidaApproximation_uniformCauchySeqOn_compact_of_mem_of_pow_le`: the
...
theorem exp_yosidaApproximation_uniformCauchySeqOn_compact_of_mem_of_pow_le (hM : 1 ≤ M)
...
This is the estimate from which the semigroup generated by `A` is defined; nothing here asserts
that such a semigroup exists. -/

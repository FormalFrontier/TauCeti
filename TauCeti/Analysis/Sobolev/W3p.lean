...
The theorem `TauCeti.w3pSubmodule_def` restates this body as a rewrite rule, so that goals about
membership in `TauCeti.w3pSubmodule` can be turned into goals about the generic graph step
...
def w3pSubmodule (mu : Measure E) [mu.IsAddHaarMeasure] (Omega : Opens E) (p : ENNReal)
  ...
  weakDerivStepSubmodule mu Omega p (W2p.hessianL (mu := mu) (Omega := Omega) (p := p))

/-- `w3pSubmodule` is the weak-derivative graph step taken over the weak Hessian.  Rewriting with
this identity replaces the `w3pSubmodule` term by the generic one, which is how the membership
...
theorem mem_w3pSubmodule_iff (J : Sobolev3JetLp mu Omega p) :
  ...
  simp only [w3pSubmodule_def, mem_weakDerivStepSubmodule_iff, W2p.hessianL_apply]
...
theorem mem_w3pSubmodule_iff_hasWeakFDerivOn (J : Sobolev3JetLp mu Omega p) :
  ...
  simp only [w3pSubmodule_def, mem_weakDerivStepSubmodule_iff_hasWeakFDerivOn, W2p.hessianL_apply]
...
theorem W3p.secondOrder_mk (u : W2p mu Omega p)
  ...
  WeakDerivStep.prev_mk (W2p.hessianL (mu := mu) (Omega := Omega) (p := p)) u T (by simpa using h)
...
instance : CompleteSpace (W3p mu Omega p) :=
  inferInstanceAs
    (CompleteSpace (WeakDerivStep mu Omega p (W2p.hessianL (mu := mu) (Omega := Omega) (p := p))))

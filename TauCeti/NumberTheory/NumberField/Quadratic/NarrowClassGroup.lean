...
/-!
# The narrow class group of an imaginary quadratic field

For a quadratic number field `K = ℚ(√d)` with `d < 0` (given by an algebraic integer `θ` with
`minpoly ℤ θ = X² - d`), there are no real infinite places, so the totally positive condition on
generators is vacuous and the narrow class group coincides with the ordinary one. This file records
the two consequences that genus theory uses: the narrow class number is the class number, and the
narrow `2`-rank is the ordinary `2`-rank.
...
/-- **The narrow class number of an imaginary quadratic field is its class number.** For `d < 0` the
field `ℚ(√d)` is totally complex, so no positivity condition survives. -/
...
/-- **The narrow and ordinary `2`-ranks of an imaginary quadratic field agree.** For `d < 0` the
narrow class group `Cl⁺(ℚ(√d))` is the ordinary class group, so the genus-theory `2`-rank may be

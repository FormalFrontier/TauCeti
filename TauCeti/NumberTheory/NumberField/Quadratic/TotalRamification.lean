...
* `TauCeti.NumberField.isPrincipal_of_absNorm_eq_sq_of_mem_ramifiedPrimes`: every ideal of
  absolute norm `p ^ 2` is principal.
...
/-- **An ideal whose absolute norm is the square of a ramified rational prime is principal.** In a
quadratic number field, such an ideal is the square of the unique prime `𝔭` above `p`, and
`𝔭 ^ 2 = p 𝓞 K` is generated by `p`. -/
theorem isPrincipal_of_absNorm_eq_sq_of_mem_ramifiedPrimes (hK : finrank ℚ K = 2)
  ...
  rw [hIJ, hJP, ← pow_two, ← _root_.NumberField.map_span_eq_sq_of_mem_ramifiedPrimes hK hmem P,

...
* `NumberField.NarrowClassGroup.mem_closure_of_sq_eq_one`: every `2`-torsion narrow class is such a
  product, for a quadratic field of either signature.
-/
...
theorem mk0_mem_closure_of_map_eq_self (hmin : minpoly ℤ θ = X ^ 2 - C d)
    (hgen : Algebra.adjoin ℚ {(θ : K)} = ⊤)
    (hprime : ∀ p ∈ ramifiedPrimes K, (Q p : Ideal (𝓞 K)).IsPrime)
    (hover : ∀ p ∈ ramifiedPrimes K,
      (Q p : Ideal (𝓞 K)).LiesOver (Ideal.span {(p : ℤ)})) {I : (Ideal (𝓞 K))⁰}
    (hI : Ideal.map (ringOfIntegersQuadraticConj hmin hgen) (I : Ideal (𝓞 K)) =
      (I : Ideal (𝓞 K))) :
    mk0 I ∈ Subgroup.closure ((fun p ↦ mk0 (Q p)) '' ramifiedPrimes K) := by
  classical
  obtain ⟨α, hα0, hαpos, s, hs, hIeq⟩ :=
    exists_isTotallyPositive_and_eq_span_singleton_mul_prod_of_map_eq_self hmin hgen hprime hover
      (mem_nonZeroDivisors_iff_ne_zero.mp I.2) hI
  -- The totally positive factor, as an element of the monoid of nonzero ideals.
  set A : (Ideal (𝓞 K))⁰ := ⟨Ideal.span {α}, mem_nonZeroDivisors_iff_ne_zero.mpr (by
    simpa [Ideal.zero_eq_bot, Ideal.span_singleton_eq_bot] using hα0)⟩ with hA
  have hIfact : I = A * ∏ p ∈ s, Q p := by
    refine Subtype.ext ?_
    rw [Submonoid.coe_mul, hA, Submonoid.coe_finsetProd]
    exact hIeq
  have hAone : mk0 A = 1 := mk0_eq_one_of_isTotallyPositive hα0 hαpos rfl
...
theorem mem_closure_of_sq_eq_one (hmin : minpoly ℤ θ = X ^ 2 - C d)
    (hgen : Algebra.adjoin ℚ {(θ : K)} = ⊤)
    (hprime : ∀ p ∈ ramifiedPrimes K, (Q p : Ideal (𝓞 K)).IsPrime)
    (hover : ∀ p ∈ ramifiedPrimes K,
      (Q p : Ideal (𝓞 K)).LiesOver (Ideal.span {(p : ℤ)}))
    {C : NarrowClassGroup K} (hC : C ^ 2 = 1) :
    C ∈ Subgroup.closure ((fun p ↦ mk0 (Q p)) '' ramifiedPrimes K) := by
  obtain ⟨I, hIfix, rfl⟩ :=
    exists_map_ringOfIntegersQuadraticConj_eq_self_of_sq_eq_one hmin hgen hC
  exact mk0_mem_closure_of_map_eq_self hmin hgen hprime hover hIfix

...
The integer-unit signature is the archimedean input to the narrow class group `Cl⁺(K)` (Layer 3
of the multiquadratic roadmap): its **cokernel** — the full sign group modulo `unitSignatures`, the
signatures realized by units — is what contributes the kernel of the surjection
`Cl⁺(K) → Cl(K)` between the narrow and ordinary class groups, and the `2`-rank of `Cl⁺(K)` is
what the `t - 1` genus-theory formula computes for a real quadratic field.

...
The lift of fractional-ideal factorization to units follows the proof organization in
`TauCeti.RingTheory.ClassGroup.HeightOneSpectrum`. That proof is adapted from Michael Stoll's
...
theorem idealsAway_mono {S S' : Finset (HeightOneSpectrum (𝓞 K))} (h : S ⊆ S') :
...
def idealsAwayInclusion {S S' : Finset (HeightOneSpectrum (𝓞 K))} (h : S ⊆ S') :
  ...
  Subgroup.inclusion (idealsAway_mono h)
...
theorem coe_idealsAwayInclusion {S S' : Finset (HeightOneSpectrum (𝓞 K))} (h : S ⊆ S')
  ...
  simpa only [idealsAwayInclusion] using
    Subgroup.coe_inclusion (idealsAway_mono h) I

/-- A height-one prime, regarded as an invertible fractional ideal. -/
private noncomputable def primeUnit (v : HeightOneSpectrum (𝓞 K)) :
...
private lemma finite_mulSupport_primeUnit_zpow (I : (FractionalIdeal (𝓞 K)⁰ K)ˣ) :
    (Function.mulSupport fun v : HeightOneSpectrum (𝓞 K) ↦
      primeUnit v ^ FractionalIdeal.count K v (I : FractionalIdeal (𝓞 K)⁰ K)).Finite := by
  refine (Filter.eventually_cofinite.mp
    (FractionalIdeal.finite_factors (I : FractionalIdeal (𝓞 K)⁰ K))).subset fun v hv hc ↦ ?_
  exact hv (by simp only [hc, zpow_zero])

/-- Unique factorization of an invertible fractional ideal, lifted to the unit group. -/
private lemma eq_finprod_primeUnit_zpow_count (I : (FractionalIdeal (𝓞 K)⁰ K)ˣ) :
    I = ∏ᶠ v : HeightOneSpectrum (𝓞 K),
      primeUnit v ^ FractionalIdeal.count K v (I : FractionalIdeal (𝓞 K)⁰ K) := by
  have hmap : ((∏ᶠ v : HeightOneSpectrum (𝓞 K),
        primeUnit v ^ FractionalIdeal.count K v (I : FractionalIdeal (𝓞 K)⁰ K)) :
        (FractionalIdeal (𝓞 K)⁰ K)ˣ) =
      ∏ᶠ v : HeightOneSpectrum (𝓞 K),
        ((primeUnit v ^ FractionalIdeal.count K v (I : FractionalIdeal (𝓞 K)⁰ K) :
          (FractionalIdeal (𝓞 K)⁰ K)ˣ) : FractionalIdeal (𝓞 K)⁰ K) :=
    MonoidHom.map_finprod (Units.coeHom (FractionalIdeal (𝓞 K)⁰ K))
      (finite_mulSupport_primeUnit_zpow I)
  refine Units.ext ?_
  rw [hmap]
  simp only [primeUnit, Units.val_zpow_eq_zpow_val, Units.val_mk0]
  exact (FractionalIdeal.finprod_heightOneSpectrum_factorization' (K := K)
    (I := (I : FractionalIdeal (𝓞 K)⁰ K)) (Units.ne_zero I)).symm
...
theorem idealsAway_eq_closure_primes (S : Finset (HeightOneSpectrum (𝓞 K))) :
  ...
  · intro I hI
    classical
    have hfin := finite_mulSupport_primeUnit_zpow (K := K) I
    rw [eq_finprod_primeUnit_zpow_count I,

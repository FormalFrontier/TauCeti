...
* `NumberField.classNumber_le_bound`: `h_F ≤ |d_F| · 4^[F:ℚ]`.
* `NumberField.classNumber_le_of_abs_discr_le_of_finrank_le`: the monotone
...
`NumberField.units_sq_index_le`. The `classNumber_mul_units_sq_index_le` family records
...
discriminant bound `NumberField.abs_discr_le_of_sq_intCast` to turn a square-root model
...
This is the monotone form of `NumberField.classNumber_le_bound`, useful when the
...
theorem classNumber_le_of_abs_discr_le_of_finrank_le (F : Type*) [Field F] [NumberField F]
    {D : ℝ} {n : ℕ} (hD : |(NumberField.discr F : ℝ)| ≤ D) (hn : Module.finrank ℚ F ≤ n) :
...
/-- Monotone form of `NumberField.classNumber_mul_units_sq_index_le`: if `|d_F| ≤ D`
and `[F : ℚ] ≤ n`, then
`h_F * [O_F^× : (O_F^×)^2] ≤ D * 8^n`. -/
...
theorem classNumber_mul_units_sq_index_le_of_abs_discr_le
    (F : Type*) [Field F] [NumberField F] {D : ℕ} (hD : |NumberField.discr F| ≤ D) :
    NumberField.classNumber F * (Subgroup.square (𝓞 F)ˣ).index ≤
      D * 8 ^ finrank ℚ F :=
...
theorem classNumber_mul_card_units_elementaryTwoQuotient_le (F : Type*) [Field F] [NumberField F] :
...
`NumberField.abs_discr_le_of_sq_intCast` gives `|d_K| ≤ 4·|a|`, and composing it with the
bounds above yields the closed forms `h_K ≤ 64·|a|` and
`h_K · [O_K^× : (O_K^×)²] ≤ 256·|a|`. -/

private lemma intCast_four_mul_natAbs (a : ℤ) : ((4 * a.natAbs : ℕ) : ℤ) = 4 * |a| := by
...
/-- Natural-number form of `NumberField.classNumber_le_of_sq_intCast`:
...
/-- A version of `NumberField.classNumber_le_of_sq_intCast` with a separate
...
theorem classNumber_le_of_sq_intCast_of_natAbs_le {K : Type*} [Field K] [NumberField K]
    ...
    (hx2 : x ^ 2 = algebraMap ℤ K a) (hx : x ∉ (algebraMap ℚ K).range) (hA : a.natAbs ≤ A) :
...
theorem classNumber_mul_units_sq_index_le_natAbs_of_sq_intCast
    ...
    (hfin : finrank ℚ K = 2) (hx2 : x ^ 2 = algebraMap ℤ K a) (hx : x ∉ (algebraMap ℚ K).range) :
...
/-- A version of
`NumberField.classNumber_mul_units_sq_index_le_natAbs_of_sq_intCast` with a separate
natural-number bound for `|a|`. -/
...
`NumberField.classNumber_mul_units_sq_index_le_natAbs_of_sq_intCast`. -/
...
/-- A version of
`NumberField.classNumber_mul_card_units_elementaryTwoQuotient_le_natAbs_of_sq_intCast`

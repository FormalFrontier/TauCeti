...
`TauCeti.NumberTheory.NumberField.Frobenius` transports this computation to the Galois
group of a number field, and `TauCeti.NumberTheory.Multiquadratic.Frobenius` applies it to the

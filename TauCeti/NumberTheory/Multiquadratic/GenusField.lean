...
* `TauCeti.Multiquadratic.exists_finset_primeDiscriminant_and_finrank_of_squarefree`: the
  relative-degree companion — for squarefree `d` that is not a perfect square, the same radicand
...
theorem exists_finset_primeDiscriminant_and_finrank_of_squarefree

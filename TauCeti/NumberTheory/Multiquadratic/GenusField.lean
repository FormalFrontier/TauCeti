...
public import TauCeti.NumberTheory.Multiquadratic.Prime.Discriminant.RelativeDegree
...
* `TauCeti.Multiquadratic.exists_finset_primeDiscriminant_and_finrank_adjoin_range_over_prodRootMem_univ_of_squarefree`:
  the relative-degree companion — for squarefree `d` that is not a perfect square, the same radicand
  compositum has degree `2 ^ (s.card - 1)` over the copy of `ℚ(√d)` generated by the product root,
  i.e. `[K_gen : ℚ(√d)] = 2 ^ (t - 1)`.
-/
...
/-- **Relative degree of the candidate genus field over `ℚ(√d)`, for squarefree `d`.** For a
squarefree `d` that is not a perfect square (so `ℚ(√d)` is a genuine quadratic field), the
fundamental discriminant `fundamentalDiscriminant d` factors into a finite set `s` of prime
discriminants (at most one even), and for any chosen roots of their radicands the candidate genus
field `adjoin ℚ (Set.range root)` has degree `2 ^ (s.card - 1)` over the quadratic subfield
generated by the product of all the roots — the copy of `ℚ(√d)` inside it. This is the relative
degree `[K_gen : ℚ(√d)] = 2 ^ (t - 1)`, `t = s.card` being the number of ramified primes. It
packages `finrank_adjoin_range_over_prodRootMem_univ_of_primeDiscriminants` with the factorization
`IsFundamentalDiscriminant.exists_finset_primeDiscriminant`, the relative-degree companion of
`exists_finset_primeDiscriminant_and_exists_mem_adjoin_sq_eq_of_squarefree`. -/
theorem exists_finset_primeDiscriminant_and_finrank_adjoin_range_over_prodRootMem_univ_of_squarefree
    {d : ℤ} (hd : Squarefree d) (hd' : ¬ IsSquare d) :
    ∃ s : Finset ℤ, (∀ P ∈ s, IsPrimeDiscriminant P) ∧
      (∀ P ∈ s, ∀ Q ∈ s, IsEvenPrimeDiscriminant P → IsEvenPrimeDiscriminant Q → P = Q) ∧
      ∏ P ∈ s, P = fundamentalDiscriminant d ∧
      ∀ {L : Type*} [Field L] [Algebra ℚ L] (root : {P // P ∈ s} → L),
        (∀ P, root P ^ 2 = algebraMap ℚ L ((primeDiscriminantRadicand P.val : ℤ) : ℚ)) →
          Module.finrank
              (IntermediateField.adjoin ℚ
                {prodRootMem (K := ℚ) root (Finset.univ : Finset {P // P ∈ s})})
              (IntermediateField.adjoin ℚ (Set.range root)) = 2 ^ (s.card - 1) := by
  obtain ⟨s, hs, hev, hprod⟩ :=
    (isFundamentalDiscriminant_fundamentalDiscriminant hd).exists_finset_primeDiscriminant
  have hfd1 : fundamentalDiscriminant d ≠ 1 := by
    have hd1 : d ≠ 1 := by rintro rfl; exact hd' ⟨1, by norm_num⟩
    rw [fundamentalDiscriminant_def]
    split <;> omega
  have hsne : s.Nonempty := by
    rw [Finset.nonempty_iff_ne_empty]
    rintro rfl
    rw [Finset.prod_empty] at hprod
    exact hfd1 hprod.symm
  refine ⟨s, hs, hev, hprod, ?_⟩
  intro L _ _ root hroot
  haveI : Nonempty {P // P ∈ s} := nonempty_subtype.mpr hsne

...
in `FundamentalDiscriminant/Examples.lean`. Their genus-field claims follow from
`isGenusField_candidateGenusField`; their class-group identifications remain future work.

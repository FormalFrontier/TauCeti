...
* `TauCeti.Multiquadratic.adjoin_candidateGenusFieldBaseSqrt`: the chosen root generates the base
  as an intermediate field.
* `TauCeti.Multiquadratic.adjoin_candidateGenusFieldBaseGen`: it generates the base over `ℚ`.
-/
...
@[simp] theorem candidateGenusFieldBaseSqrt_sq (hd : Squarefree d) :
  ...
  apply Subtype.ext
  push_cast
  change candidateGenusFieldBaseRoot hd ^ 2 = algebraMap ℤ (candidateGenusField hd) d
  rw [show algebraMap ℤ (candidateGenusField hd) d =
      algebraMap ℚ (candidateGenusField hd) ((d : ℤ) : ℚ) by
    rw [IsScalarTower.algebraMap_apply ℤ ℚ]; norm_num]
...
Together with `adjoin_candidateGenusFieldBaseGen` this presents `candidateGenusFieldBase hd` as the
quadratic field `ℚ(√d)` in the form the quadratic-field API expects. -/
...
theorem adjoin_candidateGenusFieldBaseSqrt (hd : Squarefree d) :
...
theorem adjoin_candidateGenusFieldBaseGen (hd : Squarefree d) :
  ...
  rw [coe_candidateGenusFieldBaseGen,
    ...
    adjoin_candidateGenusFieldBaseSqrt hd]
  rfl

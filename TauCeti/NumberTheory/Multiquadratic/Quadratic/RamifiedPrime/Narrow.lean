...
* `isTotallyPositive_or_neg_of_mul_ringOfIntegersQuadraticConj_eq_one`: a unit of norm one is `±` a
  totally positive unit.
* `exists_isTotallyPositive_notMem_square`: a real quadratic field with no unit of norm `-1` has a
  totally positive unit that is not a square.
...
theorem exists_nonempty_prod_narrowMk0_eq_one_of_unit
    ...
    (hnorm : (ε : 𝓞 K) * ringOfIntegersQuadraticConj hmin hgen (ε : 𝓞 K) = 1)
    (hne : ∀ v : (𝓞 K)ˣ,
...
theorem exists_nonempty_prod_narrowMk0_eq_one (hmin : minpoly ℤ θ = X ^ 2 - C d)
  ...
  · -- No such unit: `K` is real and every unit has norm one.
    ...
    refine exists_nonempty_prod_narrowMk0_eq_one_of_unit hmin hgen hprime hover hεpos
      (hnormone ε) fun v hv => hεsq ?_
    have hvv := hnormone v
    rw [hv] at hvv

...
classes generate has at most `2 ^ (t - 1)` elements
(`natCard_closure_image_narrowClassGroupMk0_le`, where the `- 1` comes from the narrow relation
`exists_nonempty_prod_narrowClassGroupMk0_eq_one`). Since the maximal
...
* `TauCeti.Multiquadratic.narrowClassGroupTwoRank_le_ncard_ramifiedPrimes_sub_one`: the upper bound
  on the narrow `2`-rank of a quadratic field.
...
`exists_nonempty_prod_narrowClassGroupMk0_eq_one`. -/
theorem narrowClassGroupTwoRank_le_ncard_ramifiedPrimes_sub_one
  ...
  have hbound := hinj.trans
    (natCard_closure_image_narrowClassGroupMk0_le hmin hgen hsf hd hscoe hprime hover)
...
the narrow one, which `narrowClassGroupTwoRank_le_ncard_ramifiedPrimes_sub_one` bounds by `t - 1`.
For a real
...
theorem twoRank_le_ncard_ramifiedPrimes_sub_one
  ...
  (NumberField.NarrowClassGroup.classGroupTwoRank_le_twoRank K).trans
    (narrowClassGroupTwoRank_le_ncard_ramifiedPrimes_sub_one hmin hgen hsf hd)

...
theorem absNorm_span_natCast (m : ℕ) :
  ...
  -- Bridge the direct natural-number coercion with the integer algebra map expected by the norm.
  have hcast : ((m : 𝓞 K)) = algebraMap ℤ (𝓞 K) (m : ℤ) := by
    push_cast
    ring
  rw [Ideal.absNorm_span_singleton, hcast, Algebra.norm_algebraMap]

...
* `TauCeti.primePowerSummatory_isLittleO_of_le_higherPrimePowerWeight`: the same conclusion for
  any nonnegative prime-power weight dominated by a constant multiple of the standard one.  This
  isolates exactly the hypotheses another arithmetic weight has to supply.
...
/-- A prime ideal of the ring of integers has absolute norm at least `2`. -/
theorem two_le_absNorm_of_prime {P : Ideal (𝓞 K)} (hP : Prime P) : 2 ≤ Ideal.absNorm P := by
  have h0 : Ideal.absNorm P ≠ 0 := Ideal.absNorm_eq_zero_iff.not.mpr hP.ne_zero
  have h1 : Ideal.absNorm P ≠ 1 := fun h ↦
    hP.not_isUnit (Ideal.isUnit_iff.mpr (Ideal.absNorm_eq_one_iff.mp h))
  omega
...
theorem two_pow_card_le_absNorm {I : Ideal (𝓞 K)} (hI : I ≠ 0)
  ...
  calc 2 ^ s.card = ∏ _P ∈ s, 2 := by rw [Finset.prod_const]
    _ ≤ ∏ P ∈ s, Ideal.absNorm P :=
        Finset.prod_le_prod' fun P hP ↦ two_le_absNorm_of_prime (hprime P hP)
...
/-- The absolute norm of the ideal generated by a natural number `m` is `m ^ [K:ℚ]`. -/
theorem absNorm_span_natCast (m : ℕ) :
    Ideal.absNorm (Ideal.span {(m : 𝓞 K)}) = m ^ Module.finrank ℚ K := by
  rw [Ideal.absNorm_span_singleton,
    show ((m : 𝓞 K)) = algebraMap ℤ (𝓞 K) (m : ℤ) by push_cast; ring,
    Algebra.norm_algebraMap]
  simp [RingOfIntegers.rank]
...
theorem card_primesLE_mul_log_two_le (K : Type*) [Field K] [NumberField K] {x : ℝ} (hx : 1 ≤ x) :
  ...
  have hcard : 2 ^ (primesLE K x).card ≤ (Nat.factorial M) ^ Module.finrank ℚ K := by
    ...
    rw [← himg, ← absNorm_span_natCast (K := K) (Nat.factorial M)]
...
/-- The higher-prime-power weight vanishes on the primes. -/
theorem higherPrimePowerWeight_eq_zero_of_prime {A : IdealPrimePower K}
    (hA : Prime (A : Ideal (𝓞 K))) : higherPrimePowerWeight A = 0 := by
  have h : primePowerExponent A = 1 := (primePowerExponent_eq_one_iff A).mpr hA
  rw [higherPrimePowerWeight, ite_eq_right (by omega)]
...
/-- The hypotheses another arithmetic weight has to supply: nonnegativity, and domination by a
constant multiple of the standard logarithmic weight on the higher prime powers.  Both are
inequalities on the weight alone, with no reference to any Euler product. -/
theorem primePowerSummatory_isBigO_of_le_higherPrimePowerWeight
    (K : Type*) [Field K] [NumberField K] {w : IdealPrimePower K → ℝ} {C : ℝ} (hC : 0 ≤ C)
    (hw₀ : ∀ A, 0 ≤ w A) (hw : ∀ A, w A ≤ C * higherPrimePowerWeight A) :
  ...
  have hle : ∀ x : ℝ, primePowerSummatory K w x ≤ C * higherPrimePowerTheta K x := by
    intro x
    rw [primePowerSummatory_apply, higherPrimePowerTheta, primePowerSummatory_apply,
      Finset.mul_sum]
    exact Finset.sum_le_sum fun A _ ↦ hw A
  refine Asymptotics.IsBigO.of_bound
    (C * (Module.finrank ℚ K / (2 * Real.log 2))) ?_
  filter_upwards [eventually_ge_atTop (1 : ℝ)] with x hx
  rw [Real.norm_of_nonneg (primePowerSummatory_nonneg K w hw₀ x),
    Real.norm_of_nonneg (by positivity : (0:ℝ) ≤ Real.sqrt x * Real.log x ^ 2), mul_assoc]
  exact (hle x).trans (mul_le_mul_of_nonneg_left (higherPrimePowerTheta_le K hx) hC)

/-- Any nonnegative prime-power weight dominated by a constant multiple of the standard
logarithmic weight on the higher prime powers has `o(x)` summatory function. -/
theorem primePowerSummatory_isLittleO_of_le_higherPrimePowerWeight
    (K : Type*) [Field K] [NumberField K] {w : IdealPrimePower K → ℝ} {C : ℝ} (hC : 0 ≤ C)
    (hw₀ : ∀ A, 0 ≤ w A) (hw : ∀ A, w A ≤ C * higherPrimePowerWeight A) :
    (fun x ↦ primePowerSummatory K w x) =o[atTop] fun x : ℝ ↦ x :=
  (primePowerSummatory_isBigO_of_le_higherPrimePowerWeight K hC hw₀ hw).trans_isLittleO
...
theorem primePowerSummatory_indicator_isLittleO (K : Type*) [Field K] [NumberField K]
  ...
  primePowerSummatory_isLittleO_of_le_higherPrimePowerWeight K zero_le_one
    (fun A ↦ Set.indicator_nonneg (fun A _ ↦ higherPrimePowerWeight_nonneg A) A)
    fun A ↦ by
      rw [one_mul]

...
lemma neZero_of_mul_dvd_left {d M N : ℕ} [NeZero N] (h : d * M ∣ N) : NeZero d :=
  ⟨fun hd ↦ NeZero.ne N (by simpa [hd] using h)⟩

/-- A factor of a divisor of a nonzero number is nonzero. -/
lemma neZero_of_mul_dvd_right {d M N : ℕ} [NeZero N] (h : d * M ∣ N) : NeZero M :=
...
is old. -/
theorem levelRaise_mem_cuspFormsOld [NeZero N] [NeZero d] (h : d * M ∣ N) (hM : M ≠ N)
...
theorem ofLe_mem_cuspFormsOld [NeZero N] (h : M ∣ N) (hM : M ≠ N) (k : ℤ)
  ...
  rwa [show CuspForm.levelRaise 1 (Gamma1_map_le_conjAct_scaleGL_of_dvd h1) f =
    _root_.CuspForm.ofLe (Subgroup.map_mono (Gamma1_le_Gamma1_of_dvd h)) f from
      _root_.CuspForm.ext fun τ ↦ (CuspForm.levelRaise_one_apply _ f τ).trans
        (congrFun (_root_.CuspForm.coe_ofLe _ f) τ).symm] at this

/-- **Level one has no oldforms**: `1` has no proper divisors. -/
@[simp]
theorem cuspFormsOld_one (k : ℤ) : cuspFormsOld 1 k = ⊥ :=
  le_bot_iff.mp <| cuspFormsOld_le fun _ d _ h hM _ ↦
...
theorem cuspFormsOld_le_of_prime [NeZero N]
  ...
  · -- `V₁` from a proper divisor level `M`: split off a prime factor `p` of `N / M`
    ...
    rwa [CuspForm.levelRaise_levelRaise_of_mul_eq (one_mul 1)
      (Gamma1_map_le_conjAct_scaleGL_of_dvd h1L) (Gamma1_map_le_conjAct_scaleGL_of_dvd h1M)
      (Gamma1_map_le_conjAct_scaleGL_of_dvd hdvd) f] at hmem
  · -- `V_d` with `d ≠ 1`: split off a prime factor `q` of `d`
    ...
    have : NeZero e := ⟨fun h ↦ NeZero.ne d (by rw [he, h, mul_zero])⟩
    ...
    rwa [CuspForm.levelRaise_levelRaise_of_mul_eq (show e * q = d by rw [he]; ring)
      (Gamma1_map_le_conjAct_scaleGL_of_dvd hqLN) (Gamma1_map_le_conjAct_scaleGL_of_dvd heM)
      (Gamma1_map_le_conjAct_scaleGL_of_dvd hdvd) f] at hmem
...
/-- Unfolding: the new subspace is the orthogonal complement of the old one. -/
theorem cuspFormsNew_def (N : ℕ) [NeZero N] (k : ℤ) :
...
theorem mem_cuspFormsNew_iff [NeZero N] {f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k} :
    f ∈ cuspFormsNew N k ↔ ∀ (M d : ℕ) [NeZero d] (h : d * M ∣ N), M ≠ N →
      ∀ g : CuspForm ((Gamma1 M).map (mapGL ℝ)) k,
        peterssonInnerCosets
          (CuspForm.levelRaise d (Gamma1_map_le_conjAct_scaleGL_of_dvd h) g) f = 0 := by
  rw [cuspFormsNew_def]
  refine ⟨fun hf M d _ h hM g ↦ CuspForm.mem_peterssonOrthogonal_iff.mp hf _
...
theorem disjoint_cuspFormsOld_cuspFormsNew (N : ℕ) [NeZero N] (k : ℤ) :
  ...
  CuspForm.disjoint_peterssonOrthogonal _
...
theorem cuspFormsNew_one (k : ℤ) : cuspFormsNew 1 k = ⊤ := by
  rw [cuspFormsNew_def, cuspFormsOld_one, CuspForm.peterssonOrthogonal_bot]
...
theorem CuspForm.diamondOpCusp_levelRaise [NeZero M] [NeZero d] [NeZero N] (hdvd : d * M ∣ N)
    (k : ℤ) (u : (ZMod N)ˣ) (f : CuspForm ((Gamma1 M).map (mapGL ℝ)) k) :

...
double coset `Γ₁(N) · diag(1, n) · Γ₁(N)`. The nonzero index records the classical convention
that Hecke operators are indexed by positive integers. -/
noncomputable def heckeT_n (n : ℕ) [NeZero n] :
...
cuspidality. -/
noncomputable def heckeT_n_cusp (n : ℕ) [NeZero n] :
    Module.End ℂ (CuspForm ((Gamma1 N).map (mapGL ℝ)) k) :=
  heckeSlashGamma1CuspFormEnd k (diagCosetGamma1 N n)

/-- The defining equation of `heckeT_n`. -/
lemma heckeT_n_def (n : ℕ) [NeZero n] :
    heckeT_n (N := N) k n = heckeSlashGamma1ModularFormEnd k (diagCosetGamma1 N n) :=
  (rfl)

/-- The defining equation of `heckeT_n_cusp`. -/
lemma heckeT_n_cusp_def (n : ℕ) [NeZero n] :
    heckeT_n_cusp (N := N) k n = heckeSlashGamma1CuspFormEnd k (diagCosetGamma1 N n) :=
  (rfl)
...
@[simp] lemma coe_heckeT_n (n : ℕ) [NeZero n]
  ...
  rw [heckeT_n_def, coe_heckeSlashGamma1ModularFormEnd]
...
@[simp] lemma coe_heckeT_n_cusp (n : ℕ) [NeZero n]
  ...
  rw [heckeT_n_cusp_def, coe_heckeSlashGamma1CuspFormEnd]

/-- **The classical `T_p` formula on modular forms, at every prime.** -/
theorem coe_heckeT_n_prime [NeZero p] (hp : p.Prime)
    (f : ModularForm ((Gamma1 N).map (mapGL ℝ)) k) :
    ⇑(heckeT_n (N := N) k p f) =
...
theorem coe_heckeT_n_cusp_prime [NeZero p] (hp : p.Prime)
    (f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k) :
    ⇑(heckeT_n_cusp (N := N) k p f) =
...
theorem heckeT_n_eq_upperTri [NeZero p] (hpN : p ∣ N) :
    heckeT_n (N := N) k p = heckeSlashUpperTriModularFormEnd k hpN := by
  unfold heckeT_n
...
operator. -/
theorem heckeT_n_cusp_eq_upperTri [NeZero p] (hpN : p ∣ N) :
    heckeT_n_cusp (N := N) k p = heckeSlashUpperTriCuspFormEnd k hpN := by
  unfold heckeT_n_cusp

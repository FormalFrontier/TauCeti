...
element of the stabiliser `⟨T, -I⟩` of `∞` in `SL(2, ℤ)`. Here the cusps are indexed by the orbits
of `⟨T⟩` alone (`CuspClass`), so a classical cusp of `𝒢` is one cusp class or two, according as
its `⟨T⟩`-orbit is stable under `-I` or not. That is the **full**-coset convention that
...
The **width** of a class is the period of that action: the least `m > 0` with `x⁻¹ T ^ m x ∈ 𝒢`
(`minimalPeriod_TSL_dvd_iff`, `cuspClassWidth_mk`). It is the classical width of the cusp
`x⁻¹ ∞` when `-I ∈ 𝒢`, and otherwise that width or twice it, in step with the same convention.
The widths sum to the index (`sum_cuspClassWidth`), and the class of the base coset — the cusp
`∞` — has width `Subgroup.integerCuspWidth 𝒢` (`cuspClassWidth_mk_one`).

## The mechanism

Over one class the coset factors are the integer translates of a single factor, so they multiply
...
classes, so its order at `∞` is the sum of the orders of `f` at the cusp classes. Nothing here
divides by an order that could be `⊤`: each factor of a nonzero form is nonzero
(`SlashInvariantForm.quotientFunc_ne_zero`), which is what the additivity of
`qExpansionOrderAtCusp` over products asks for.

## Main definitions

* `TauCeti.ModularForm.CuspClass`: the cusps of `𝒢`, as the `T`-orbits on `𝒮ℒ ⧸ 𝒢 ⊓ 𝒮ℒ`.
* `TauCeti.ModularForm.cuspClassWidth`: the width of a cusp class.
* `TauCeti.ModularForm.orderAtCuspClass`: the vanishing order of a form at a cusp class, read in
  that class's width.

## Main results

* `TauCeti.ModularForm.orderAtCuspClass_mk`: the order at a class may be read at any
  representative coset, so the `Quotient.out` of the definition is only a spelling.
* `TauCeti.ModularForm.qExpansionOrderAtCusp_one_norm_eq_sum_orderAtCuspClass`: the cusp
...
* `TauCeti.ModularForm.twelve_mul_orderAtCuspClass_le_weight_mul_index`: the mass at one cusp is
  bounded by the total mass.
...
/-- The **cusp classes** of `𝒢`: the orbits of left translation by `T` on the coset space
`𝒮ℒ ⧸ 𝒢 ⊓ 𝒮ℒ`. The class of the coset of `x` is the cusp `x⁻¹ ∞` of `𝒢`, indexed for the full
coset space rather than its projective quotient; see the module docstring. -/
abbrev CuspClass (𝒢 : Subgroup (GL (Fin 2) ℝ)) : Type :=
...
lemma quotientFunc_TSL_zpow_smul (f : F) (q : 𝒮ℒ ⧸ 𝒢.subgroupOf 𝒮ℒ) (j : ℤ) (τ : ℍ) :
  ...
  have hinv : (⟨((TSL ^ (-j) : 𝒮ℒ) : GL (Fin 2) ℝ), hmem⟩ : 𝒮ℒ)⁻¹ = TSL ^ j := by
    rw [show (⟨((TSL ^ (-j) : 𝒮ℒ) : GL (Fin 2) ℝ), hmem⟩ : 𝒮ℒ) = TSL ^ (-j) from rfl,
      ← zpow_neg, neg_neg]
...
/-- The **width** of a cusp class of `𝒢`: by `minimalPeriod_TSL_dvd_iff` and `cuspClassWidth_mk`,
the least `m > 0` with `x⁻¹ T ^ m x ∈ 𝒢` for any representative `x` of the class. -/
noncomputable def cuspClassWidth (c : CuspClass 𝒢) : ℕ := minimalPeriod (TSL • ·) c.out

private lemma exists_zpow_smul_eq_out (q : 𝒮ℒ ⧸ 𝒢.subgroupOf 𝒮ℒ) :
    ∃ j : ℤ, TSL ^ j • q = (⟦q⟧ : CuspClass 𝒢).out := by
  obtain ⟨h, hh⟩ := (orbitRel_apply (G := Subgroup.zpowers TSL)).mp
    (Quotient.eq.mp (Quotient.out_eq (⟦q⟧ : CuspClass 𝒢)))
  obtain ⟨j, hj⟩ := Subgroup.mem_zpowers_iff.mp h.2
  exact ⟨j, by rw [show (TSL ^ j : 𝒮ℒ) = (h : 𝒮ℒ) from hj]; exact hh⟩

/-- The width of a cusp class is the period of the `T`-action at any representative coset. -/
lemma cuspClassWidth_mk (q : 𝒮ℒ ⧸ 𝒢.subgroupOf 𝒮ℒ) :
    cuspClassWidth (⟦q⟧ : CuspClass 𝒢) = minimalPeriod (TSL • ·) q := by
  obtain ⟨j, hj⟩ := exists_zpow_smul_eq_out (𝒢 := 𝒢) q
  rw [cuspClassWidth, ← hj, minimalPeriod_zpow_smul]

/-- **What the period of a coset says about the group**: `T ^ n` fixes the coset of `x` exactly
when `x⁻¹ T ^ n x ∈ 𝒢`. So the period — and hence, by `cuspClassWidth_mk`, the width of the
class of `⟦x⟧` — is the least positive `n` with `x⁻¹ T ^ n x ∈ 𝒢`, the classical width of the
cusp `x⁻¹ ∞`, read for the full coset space rather than its projective quotient. -/
lemma minimalPeriod_TSL_dvd_iff (x : 𝒮ℒ) (n : ℕ) :
  ...
  rw [← MulAction.pow_smul_eq_iff_minimalPeriod_dvd, MulAction.Quotient.smul_mk, smul_eq_mul,
    ...
    show ((TSL ^ n * x : 𝒮ℒ)⁻¹ * x : 𝒮ℒ) = (x⁻¹ * TSL ^ n * x : 𝒮ℒ)⁻¹ by group,
    InvMemClass.coe_inv, Subgroup.inv_mem_iff]

instance [𝒢.IsFiniteRelIndex 𝒮ℒ] (c : CuspClass 𝒢) : NeZero (cuspClassWidth c) :=
  inferInstanceAs (NeZero (minimalPeriod (TSL • ·) c.out))

lemma cuspClassWidth_pos [𝒢.IsFiniteRelIndex 𝒮ℒ] (c : CuspClass 𝒢) : 0 < cuspClassWidth c :=
  Nat.pos_of_neZero _
...
/-- Each factor of the norm is periodic with the width of its cusp class as a period. -/
lemma periodic_quotientFunc_out (f : F) (c : CuspClass 𝒢) :
    Periodic (quotientFunc f c.out ∘ ofComplex) (cuspClassWidth c : ℝ) :=
  periodic_quotientFunc f c.out

/-- The product of the norm factors over one cusp class is the Galois product of that
class's factor at its own width. -/
lemma prod_cuspClass_quotientFunc [𝒢.IsFiniteRelIndex 𝒮ℒ] (f : F) (c : CuspClass 𝒢) (τ : ℍ) :
    ∏ x : ZMod (cuspClassWidth c),
        quotientFunc f (TSL ^ (ZMod.cast x : ℤ) • c.out) τ =
      galoisProd (cuspClassWidth c) (quotientFunc f c.out) τ := by
  rw [galoisProd_apply]
  refine Finset.prod_nbij' ZMod.val (fun j ↦ (j : ZMod (cuspClassWidth c)))
...
/-- A subgroup of finite relative index has finitely many cusp classes: they are the orbits of
an action on the finite coset space. -/
noncomputable instance fintypeCuspClass : Fintype (CuspClass 𝒢) := Fintype.ofFinite _

variable [SlashInvariantFormClass F 𝒢 k]

/-- **The norm, decomposed over the cusp classes**: the coset factors of one class multiply to
the Galois product of that class's factor, taken over a full period. -/
lemma slashInvariantForm_norm_apply_eq_prod_galoisProd (f : F) (τ : ℍ) :
    _root_.SlashInvariantForm.norm 𝒮ℒ f τ =
      ∏ c : CuspClass 𝒢, galoisProd (cuspClassWidth c) (quotientFunc f c.out) τ := by
  classical
  ...
  rw [← prod_cuspClass_quotientFunc f c τ]
...
/-- The **vanishing order of `f` at a cusp class**: the order of the width-`m` `q`-expansion of
the coset factor `f ∣[k] x⁻¹`, where `m` is the width of the class and `x` a representative.
It does not depend on the representative — `orderAtCuspClass_mk`. -/
noncomputable def orderAtCuspClass [SlashInvariantFormClass F 𝒢 k] (f : F) (c : CuspClass 𝒢) :
    ℤ :=
  qExpansionOrderAtCusp (cuspClassWidth c : ℝ) (quotientFunc f c.out)

omit [𝒢.IsFiniteRelIndex 𝒮ℒ] in
/-- The order at a cusp class is nonnegative: a modular form is holomorphic at every cusp. -/
lemma orderAtCuspClass_nonneg [SlashInvariantFormClass F 𝒢 k] (f : F) (c : CuspClass 𝒢) :
    0 ≤ orderAtCuspClass f c :=
  qExpansionOrderAtCusp_nonneg _ _

variable [ModularFormClass F 𝒢 k]

/-- **The order at a cusp class may be read at any representative coset**, in that coset's own
period; the `Quotient.out` in the definition is therefore only a choice of spelling. -/
lemma orderAtCuspClass_mk (f : F) (q : 𝒮ℒ ⧸ 𝒢.subgroupOf 𝒮ℒ) :
    orderAtCuspClass f (⟦q⟧ : CuspClass 𝒢) =
      qExpansionOrderAtCusp ((minimalPeriod (TSL • ·) q : ℕ) : ℝ) (quotientFunc f q) := by
  obtain ⟨j, hj⟩ := exists_zpow_smul_eq_out (𝒢 := 𝒢) q
  rw [orderAtCuspClass, cuspClassWidth_mk, ← hj,
    qExpansionOrderAtCusp_quotientFunc_TSL_zpow_smul]

private lemma analyticAt_cuspFunction_one_galoisProd (f : F) (c : CuspClass 𝒢) :
    AnalyticAt ℂ (cuspFunction 1 (galoisProd (cuspClassWidth c) (quotientFunc f c.out))) 0 :=
...
/-- The width-`1` order of the Galois product over a cusp class is the order of that class's
factor at its own width. -/
lemma qExpansion_one_galoisProd_cuspClass_order_eq (f : F) (c : CuspClass 𝒢) :
    (qExpansion 1 (galoisProd (cuspClassWidth c) (quotientFunc f c.out))).order =
      (qExpansion (cuspClassWidth c : ℝ) (quotientFunc f c.out)).order :=
  qExpansion_one_galoisProd_order_eq (cuspClassWidth_pos c) (periodic_quotientFunc_out f c)
    (SlashInvariantForm.isBoundedAtImInfty_quotientFunc f _)
    (SlashInvariantForm.mdifferentiable_quotientFunc f _)

private lemma qExpansion_cuspClass_ne_zero {f : F} (hf : (⇑f : ℍ → ℂ) ≠ 0) (c : CuspClass 𝒢) :
    qExpansion (cuspClassWidth c : ℝ) (quotientFunc f c.out) ≠ 0 :=
  mt (qExpansion_eq_zero_iff (mod_cast cuspClassWidth_pos c) (periodic_quotientFunc_out f c)
    (SlashInvariantForm.mdifferentiable_quotientFunc f _)
...
omit [𝒢.IsFiniteRelIndex 𝒮ℒ] in
/-- The class of the base coset is the cusp `∞`, whose width is the integer cusp width. -/
lemma cuspClassWidth_mk_one :
    cuspClassWidth (⟦(QuotientGroup.mk 1 : 𝒮ℒ ⧸ 𝒢.subgroupOf 𝒮ℒ)⟧ : CuspClass 𝒢) =
      Subgroup.integerCuspWidth 𝒢 := by
  rw [cuspClassWidth_mk, minimalPeriod_TSL_mk_one]

/-- **At the cusp `∞` the order is the one the `∞`-decomposition reads**: the order of `f`
itself at the integer cusp width. -/
lemma orderAtCuspClass_mk_one (f : F) :
    orderAtCuspClass f (⟦(QuotientGroup.mk 1 : 𝒮ℒ ⧸ 𝒢.subgroupOf 𝒮ℒ)⟧ : CuspClass 𝒢) =
      qExpansionOrderAtCusp ((Subgroup.integerCuspWidth 𝒢 : ℕ) : ℝ) (⇑f) := by
  rw [orderAtCuspClass_mk, minimalPeriod_TSL_mk_one,
    show quotientFunc f (QuotientGroup.mk 1 : 𝒮ℒ ⧸ 𝒢.subgroupOf 𝒮ℒ) = ⇑f by simp]

/-- **The order of the norm at `∞` is the sum of the orders of `f` at the cusp classes.** -/
theorem qExpansionOrderAtCusp_one_norm_eq_sum_orderAtCuspClass (f : F)
    (hf : (⇑f : ℍ → ℂ) ≠ 0) :
    qExpansionOrderAtCusp 1 ⇑(_root_.ModularForm.norm 𝒮ℒ f) =
      ∑ c : CuspClass 𝒢, orderAtCuspClass f c := by
  have hne : ∀ c : CuspClass 𝒢,
      analyticOrderAt (cuspFunction 1
        (galoisProd (cuspClassWidth c) (quotientFunc f c.out))) 0 ≠ ⊤ := fun c ↦ by
    rw [← qExpansion_order_eq_analyticOrderAt_cuspFunction
      (analyticAt_cuspFunction_one_galoisProd f c), qExpansion_one_galoisProd_cuspClass_order_eq,
      Ne, PowerSeries.order_eq_top]
    exact qExpansion_cuspClass_ne_zero hf c
  have hfun : (⇑(_root_.ModularForm.norm 𝒮ℒ f) : ℍ → ℂ) =
      ∏ c : CuspClass 𝒢, galoisProd (cuspClassWidth c) (quotientFunc f c.out) := by
    ext τ
  ...
  rw [orderAtCuspClass, qExpansionOrderAtCusp_def, qExpansionOrderAtCusp_def,
    qExpansion_one_galoisProd_cuspClass_order_eq]
...
/-- **The widths of the cusp classes sum to the index** `[SL(2, ℤ) : 𝒢 ⊓ SL(2, ℤ)]`. -/
lemma sum_cuspClassWidth [𝒢.IsFiniteRelIndex 𝒮ℒ] :
    ∑ c : CuspClass 𝒢, cuspClassWidth c = Nat.card (𝒮ℒ ⧸ 𝒢.subgroupOf 𝒮ℒ) :=
...
`Γ ≤ SL(2, ℤ)`, the interior orders — weighted by `2 / |Stab_Γ P|` — together with the orders at
the cusp classes add up to `k · [SL(2, ℤ) : Γ] / 12`.
...
theorem valence_formula_finiteIndex (f : F) (hf : (⇑f : ℍ → ℂ) ≠ 0) :
    (∑ᶠ o : orbitRel.Quotient (Γ : Subgroup (GL (Fin 2) ℝ)) ℍ,
        ...
        ∑ c : CuspClass (Γ : Subgroup (GL (Fin 2) ℝ)), (orderAtCuspClass f c : ℚ) =
      (k : ℚ) * Nat.card (𝒮ℒ ⧸ (Γ : Subgroup (GL (Fin 2) ℝ)).subgroupOf 𝒮ℒ) / 12 := by
  rw [← valence_formula_finiteIndex_norm f hf,
    qExpansionOrderAtCusp_one_norm_eq_sum_orderAtCuspClass f hf]
  push_cast
  ring
...
theorem twelve_mul_orderAtCuspClass_le_weight_mul_index (f : F) (hf : (⇑f : ℍ → ℂ) ≠ 0)
    (c : CuspClass (Γ : Subgroup (GL (Fin 2) ℝ))) :
    12 * orderAtCuspClass f c ≤
  ...
  have hsingle : (orderAtCuspClass f c : ℚ) ≤
      ∑ c' : CuspClass (Γ : Subgroup (GL (Fin 2) ℝ)), (orderAtCuspClass f c' : ℚ) :=
    Finset.single_le_sum (f := fun c' ↦ (orderAtCuspClass f c' : ℚ))
      (fun c' _ ↦ by exact_mod_cast orderAtCuspClass_nonneg f c') (Finset.mem_univ c)
  have htotal := valence_formula_finiteIndex f hf
  have hq : (12 : ℚ) * (orderAtCuspClass f c : ℚ) ≤

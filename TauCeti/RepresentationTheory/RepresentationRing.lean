...
The **representation ring** `R(G)` of a monoid `G` over a field `k` is the Grothendieck ring of the
finite-dimensional representations `FDRep k G`: its underlying additive group is generated by the
classes `[V]` of objects subject only to `[V ⊕ W] = [V] + [W]`, and its multiplication is induced
by the tensor product, so that `[V] · [W] = [V ⊗ W]` with unit the class of the trivial
...
its character — but that needs the freeness of `R(G)` on the isomorphism classes of irreducibles,
hence a Jordan-Hölder decomposition inside `FDRep k G`, and is not proved here.
...
* `TauCeti.character_tensorUnit`: the character of the unit of `FDRep k G` is the constant
  function `1`, the multiplicativity input to `TauCeti.repRingCharacter`.
...
/-- **The representation ring** `R(G)` of a monoid `G` over a field `k`: the Grothendieck ring of
`FDRep k G`, with addition induced by the direct sum and multiplication by the tensor product.
...
holds for a *split* extension. For a finite `G` whose order is invertible in `k` the two agree,
every short exact sequence of representations splitting by Maschke's theorem. -/
...
/-- The character of the unit of `FDRep k G`, the trivial representation on `k` itself, is the
constant function `1`. -/
...
`1` (`TauCeti.character_tensorUnit`). -/
noncomputable def repRingCharacter (k : Type u) (G : Type v) [Field k] [Monoid G] :
    repRing k G →+* (G → k) :=
  SplitK0.liftRingHom (characterInvariant k G) (character_tensorUnit k G) FDRep.char_tensor
...
of `TauCeti.repRingCharacter_mem_classFunction`. -/

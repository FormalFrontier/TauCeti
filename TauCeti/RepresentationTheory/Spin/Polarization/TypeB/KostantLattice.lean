...
simple-generator Kostant integral form: the subring
`TauCeti.UniversalEnvelopingAlgebra.kostantForm` generated by the divided powers of the
Bourbaki-numbered simple-root vectors, in both signs, and the binomial coefficients in the
simple coroots. As in
...
* `TauCeti.typeBRootGenerator`: the positive and negative numbered simple-root vectors as one
  family.
...
`typeBRootGenerator` and the binomial coefficients in `typeBSimpleCorootGenerator`. No comparison
with the all-root Kostant `ℤ`-form of type `B` is claimed. -/

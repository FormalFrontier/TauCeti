...
representation only when its carrier already lies there. A finite-dimensional carrier is however
always *equivalent* to one that does, because it is spanned by finitely many vectors over `k`;
`FDRep.ofShrink` performs that transport, and the lemmas beside it say that the transport changes
neither the dimension nor the character.

## Main definitions

* `FDRep.ofShrink`: a finite-dimensional representation on a carrier in an arbitrary universe, as
  an object of `FDRep k G`.
...
/-- **A finite-dimensional representation as an object of `FDRep k G`**, whatever universe its
carrier lives in. A finite-dimensional `k`-module is `Small.{u}` for `k : Type u`, so the carrier
may be replaced by `Shrink V` and the action conjugated across; `FDRep.ofShrinkEquiv` compares the
result with `ρ`. -/

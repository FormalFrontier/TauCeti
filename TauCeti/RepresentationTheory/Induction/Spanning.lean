...
surjectivity of `⨁_{C cyclic} R(C) → R(G)` after tensoring with `ℚ`.  **Nothing below says
...
descends `TauCeti.ClassFunction.cyclicInducedCharacters_eq_top` to the virtual-character lattice
...
/-- `TauCeti.ClassFunction.cyclicInduced` is induction from the family of cyclic subgroups.  The
definition does not unfold outside this module, so this is what a downstream proof rewrites with in
order to reach the lemmas stated for a general family. -/
theorem cyclicInduced_eq_inducedFrom :
    cyclicInduced k G = inducedFrom k G {S | IsCyclic S} := by
  rw [cyclicInduced]
...
/-- `TauCeti.ClassFunction.cyclicInducedCharacters` is induction from the family of cyclic
subgroups, as in `TauCeti.ClassFunction.cyclicInduced_eq_inducedFrom`. -/
theorem cyclicInducedCharacters_eq_inducedCharactersFrom :
    cyclicInducedCharacters k G = inducedCharactersFrom k G {S | IsCyclic S} := by
  rw [cyclicInducedCharacters]
...
The coefficients are arbitrary elements of `k`.  Artin's induction theorem asserts that they may be
taken rational, and `TauCeti.ClassFunction.exists_zsmul_eq_sum_zsmul_ind_ofCharacter` deduces that
from this statement by a lattice descent. -/

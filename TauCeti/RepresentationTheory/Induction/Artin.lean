...
def indVirtualCharacterSetCyclic : Set (G → k) :=
  {f | ∃ C : Subgroup G, IsCyclic C ∧ ∃ (n : ℕ) (ρ : Representation k C (Fin n → k)),
    ρ.IsIrreducible ∧ ((ind C (ofCharacter ρ) : ClassFunction k G) : G → k) = f}
...
theorem ind_ofCharacter_mem_indVirtualCharacterSpanCyclic (C : Subgroup G) (hC : IsCyclic C)
  ...
  AddSubgroup.subset_closure ⟨C, hC, n, ρ, hρ, rfl⟩
...
theorem indVirtualCharacterSpanCyclic_le_iff {H : AddSubgroup (G → k)} :
  ...
  constructor
  · intro h C hC n ρ hρ
    exact h ⟨C, hC, n, ρ, hρ, rfl⟩
  · rintro h - ⟨C, hC, n, ρ, hρ, rfl⟩
    exact h C hC n ρ hρ

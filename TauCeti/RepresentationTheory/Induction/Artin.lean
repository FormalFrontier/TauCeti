...
/-- The definition of `TauCeti.ClassFunction.indVirtualCharacterSpanCyclic`, exposed for consumers
that need its generating set. -/
theorem indVirtualCharacterSpanCyclic_def :
    indVirtualCharacterSpanCyclic k G = AddSubgroup.closure (indVirtualCharacterSetCyclic k G) :=
  by rw [indVirtualCharacterSpanCyclic]
...
theorem indVirtualCharacterSpanCyclic_le_iff {H : AddSubgroup (G → k)} :
  ...
  rw [indVirtualCharacterSpanCyclic_def, AddSubgroup.closure_le]
  constructor
  · intro h C hC n ρ hρ
...
private theorem exists_artin_induction_data [IsAlgClosed k] [CharZero k] :
  ...
  have hspan : Submodule.span k 𝒢 = ⊤ := by
    rw [← indCharacterSpan_def]
    exact indCharacterSpan_cyclic_eq_top

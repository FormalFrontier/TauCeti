...
*family* of characters takes its value at the identity is again a normal subgroup
(`FDRep.coe_iInf_ker`, `FDRep.normal_iInf_ker`).
...
`FDRep.coe_iInf_ker` together with `FDRep.normal_iInf_ker` that promotes that common kernel from a
set of character equations to a normal subgroup.
...
* `FDRep.coe_iInf_ker` and `FDRep.normal_iInf_ker`: **the locus where every member of a family of
  characters takes its value at the identity is a normal subgroup.**
...
/-- **The common kernel of a family of representations is normal.** Together with
`FDRep.coe_iInf_ker` this says that a subset of `G` cut out by the equations
`χᵢ(g) = χᵢ(1)` is a normal subgroup, which is how a character computation produces one. -/
theorem normal_iInf_ker (W : ι → FDRep ℂ G) : (⨅ i, (W i).ρ.ker : Subgroup G).Normal where
  conj_mem g hg h := by
    rw [Subgroup.mem_iInf] at hg ⊢
    exact fun i => (inferInstance : ((W i).ρ.ker).Normal).conj_mem g (hg i) h

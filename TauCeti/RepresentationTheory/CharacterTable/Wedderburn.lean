...
half, which is what the dimension arguments can see. The first of the two is supplied downstream, in
`TauCeti/RepresentationTheory/CharacterTable/BlockRepresentation.lean`, where each block is shown to
carry an irreducible representation and distinct blocks to carry inequivalent ones; the
independence of the degrees from the presentation is still not proved.

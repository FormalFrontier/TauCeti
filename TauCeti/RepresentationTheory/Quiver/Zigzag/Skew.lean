...
vertex by allowing a nonzero scalar ratio between each ordered pair of incident edges.  A parameter
...
/-- A skew-zigzag parameter assigns a nonzero scalar ratio to every ordered pair of adjacencies
...
/-- The skew-zigzag ideal is the two-sided span of its displayed relation family. -/
theorem skewZigzagIdeal_eq_span :
    skewZigzagIdeal k G c = TwoSidedIdeal.span {x | IsSkewZigzagRelator k G c x} :=
  by rw [skewZigzagIdeal]

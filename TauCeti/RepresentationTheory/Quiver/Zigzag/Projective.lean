...
`Z e_i`.  It is `@[expose]`d so that downstream files may apply the general theory of the left
ideal generated by an idempotent to it. -/
@[expose] noncomputable def zigzagProjective (i : V) : Ideal (nonisolatedZigzagQuotient k G) :=
...
/-- The distinguished generator `e_i` of the vertex projective.  Like `TauCeti.zigzagProjective`
it is `@[expose]`d, so that the general theory of the left ideal generated by an idempotent
recognizes it as that ideal's generator. -/
@[expose] noncomputable def zigzagProjectiveGenerator (i : V) : zigzagProjective k G i :=

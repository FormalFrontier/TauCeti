...
the **arrow ideal** `arrowIdeal k Q`: the elements with no trivial path in their support. It is the
ideal generated by the arrows, and the complement of the semisimple part of the path algebra.

For a **finite acyclic** quiver the filtration dies: no path is longer than the number of vertices,
so `pathSpan k Q (Nat.card Q) = ⊥` and every element of the arrow ideal is nilpotent. This pins the
Jacobson radical of the path algebra: it is exactly the arrow ideal
(`TauCeti.jacobson_pathAlgebra_eq_arrowIdeal`). Both inclusions use the finiteness and the
acyclicity, and both fail without them, as the one-loop quiver shows: there `kQ ≅ k[X]`
(`TauCeti.PathAlgebra.oneLoopAlgEquiv`) has zero radical while its arrow ideal `(X)` is nonzero.
...
* `TauCeti.mul_mem_pathSpan`: the length filtration is multiplicative.
...
`pathSpan` is a `k`-submodule, not an ideal: only its first step is a one-sided, let alone a
two-sided, ideal, and the multiplicativity statement is about the `k`-span in any case. The arrow
ideal is then built on the `k`-submodule `pathSpan k Q 1`, whose closure under multiplication by
the path algebra is the `m = 0` and the `n = 0` case of multiplicativity, `pathSpan k Q 0` being
everything.

Membership is described twice: `TauCeti.mem_pathSpan_iff` in terms of the coordinates for the path
basis, which is how the two inclusions of the radical theorem are proved, and
`TauCeti.ofPath_mem_pathSpan` on a basis path, which is how the ideal is recognized in practice.
...
theorem repr_single (x : Quiver.TotalPath Q) (c : k) :
...
/-- **The length filtration of a finite acyclic quiver dies at the number of vertices**: no path
of an acyclic quiver is longer than the number of vertices. -/
theorem pathSpan_eq_bot_of_isAcyclic (k : Type w) (Q : Type u) [Field k] [Quiver.{v} Q] [Finite Q]
    (h : Quiver.IsAcyclic Q) : pathSpan k Q (Nat.card Q) = ⊥ := by
...
theorem vertexIdempotent_mul_mul_vertexIdempotent (h : Quiver.IsAcyclic Q) (v : Q)
  ...
  | single x c =>
    obtain ⟨a, b, p⟩ := x
    rw [repr_single, vertexIdempotent_eq_single, smul_single, mul_one]
    by_cases hb : v = b
    · subst hb

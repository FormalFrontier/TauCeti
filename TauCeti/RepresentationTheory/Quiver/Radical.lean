...
* `TauCeti.mem_arrowIdeal_iff`: an element lies in the arrow ideal exactly when all its coordinates
  on the trivial paths vanish, and `TauCeti.ofPath_mem_arrowIdeal_iff` on a basis path.
...
multiplication by the path algebra on either side. Only the first step is packaged here, as
`arrowIdeal k Q`, that being the step the radical theorem is about; the higher steps are used as
submodules, through `TauCeti.pow_mem_pathSpan`.
...
`TauCeti.ofPath_mem_pathSpan` and `TauCeti.ofPath_mem_arrowIdeal_iff` recognize a basis path, which
is how the ideal is recognized in practice. `TauCeti.mem_arrowIdeal` is the bridge to the
`pathSpan` implementation, which consumers should not need.
...
exactly when every path carrying a nonzero coordinate has positive length, so that in particular
all of its coordinates on the trivial paths vanish. -/
@[simp]
...
theorem ofPath_mem_arrowIdeal_iff {x : Quiver.TotalPath Q} :
    (ofPath x : pathAlgebra k Q) ∈ arrowIdeal k Q ↔ 0 < x.2.2.length := by
  refine ⟨fun hx => mem_arrowIdeal_iff.1 hx x ?_, ofPath_mem_arrowIdeal⟩

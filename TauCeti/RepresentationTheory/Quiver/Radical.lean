...
is how the ideal is recognized in practice. At the level of the arrow ideal the length condition
collapses to the vanishing of the named coordinates on the trivial paths, which is
...
Only `TauCeti.ofPath_mem_arrowIdeal_iff` is `simp`: it is the one whose right-hand side is smaller
than its left, and simp-tagging a second membership `iff` whose left-hand side is the bare
`f ∈ arrowIdeal k Q` would rewrite that one's left-hand side away, which the `simpNF` linter
rejects.

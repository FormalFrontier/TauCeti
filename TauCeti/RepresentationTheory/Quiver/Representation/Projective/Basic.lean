...
* `TauCeti.isSplitEpi_of_app_eq_indecProjRepBasis_nil`: `Pᵢ` is **generated** by the basis vector
  of the trivial path, so a morphism into `Pᵢ` hitting that vector is a split epimorphism, and
  `TauCeti.eq_id_of_app_indecProjRepBasis_nil_eq_self`: an endomorphism of `Pᵢ` fixing it is the
  identity. `TauCeti.exists_eq_smul_indecProjRepBasis_nil`: when the trivial path is the only path
  ...
  `TauCeti.RepresentationTheory.Quiver.Representation.Projective.Cover`.

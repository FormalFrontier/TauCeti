...
positive length; over an acyclic quiver the two differ exactly at `i`, where the first is the line
through `x` and the second vanishes. That comparison is what forces a simple representation to be

...
the subrepresentation generated by `x`, and `n = 1` gives its radical; over an acyclic quiver the
two differ exactly at `i`, where the first is the line through `x` and the second vanishes. That
comparison is what forces a simple representation to be concentrated at one vertex.
...
* `TauCeti.QuiverSubrep.pathSpan_zero_self_of_isAcyclic` and
  `TauCeti.QuiverSubrep.pathSpan_one_self_of_isAcyclic`: over an acyclic quiver the two spans at the
  base vertex are the line through `x` and `⊥`.
...
the subrepresentation generated by `x`, and for `n = 1` its radical. -/
@[expose]
...
/-- At the base vertex of an acyclic quiver the span along all paths is the line through `x`: the
only path from `i` to itself is the trivial one. -/
theorem pathSpan_zero_self_of_isAcyclic (hQ : Quiver.IsAcyclic Q) (x : M.obj i) :
...
/-- At the base vertex of an acyclic quiver the span along the paths of positive length vanishes:
there is no such path from `i` to itself. -/
theorem pathSpan_one_self_of_isAcyclic (hQ : Quiver.IsAcyclic Q) (x : M.obj i) :
    (pathSpan x 1).toSubmodule i = ⊥ := by
  refine Submodule.span_eq_bot.2 ?_
  rintro y ⟨p, hp, rfl⟩
  rw [hQ.length_eq_zero p] at hp

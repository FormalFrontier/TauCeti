...
* `TauCeti.isSplitEpi_of_epi_comp_indecProjRepToSimpleRep` and
  ...
  over an acyclic quiver — in the sharper form that every `g : X ⟶ Pᵢ` whose composite with it
  is an epimorphism is already a *split* epimorphism — so `Pᵢ` is the projective cover of `Sᵢ`.

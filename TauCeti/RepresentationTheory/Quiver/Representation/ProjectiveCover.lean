...
quotient of the vertex projective. This file proves that over an **acyclic** quiver that surjection
is a *projective cover*: it is an essential epimorphism, so nothing smaller than `Pᵢ` maps onto
`Sᵢ`.

Acyclicity is genuinely needed, and it is exactly what is needed. For the quiver with one vertex
and one loop the path algebra is `k[X]`, the kernel of `Pᵢ ↠ Sᵢ` is the ideal `(X)`, and
`(X) + (X - 1) = k[X]` with `(X - 1)` proper, so that kernel is not superfluous and `Pᵢ ↠ Sᵢ` is
not a cover. What acyclicity buys is that the vertex space `(Pᵢ)ᵢ` is the *line* through the basis
vector of the trivial path, the trivial path being the only path `i → i`
(`TauCeti.exists_eq_smul_indecProjRepBasis_nil`). A morphism `g : X ⟶ Pᵢ` whose composite with
`Pᵢ ↠ Sᵢ` is onto must then hit that basis vector exactly, not merely up to the kernel.
...
* `TauCeti.exists_eq_smul_indecProjRepBasis_nil`: over an acyclic quiver `(Pᵢ)ᵢ` is the line
  spanned by the basis vector of the trivial path.
* `TauCeti.eq_id_of_app_indecProjRepBasis_nil`: an endomorphism of `Pᵢ` fixing that basis vector
  ...
  hitting it is a split epimorphism. Neither needs acyclicity.
* `TauCeti.eq_indecProjRepBasis_nil_of_app_eq_simpleRepGenerator` and
  `TauCeti.exists_app_eq_indecProjRepBasis_nil`: over an acyclic quiver `Pᵢ ↠ Sᵢ` separates that
  basis vector from the rest of `(Pᵢ)ᵢ`, so a morphism into `Pᵢ` that is onto `Sᵢ` after
  composing with it hits the basis vector.
* `TauCeti.isSplitEpi_of_epi_comp_indecProjRepToSimpleRep` and
  ...
  over an acyclic quiver, indeed a split one, so `Pᵢ` is the projective cover of `Sᵢ`.
...
`TauCetiRoadmap/RepresentationTheory/QuiverRepresentations/README.md`, for the vertex simples of
an acyclic quiver.
...
/-- **Over an acyclic quiver `(Pᵢ)ᵢ` is a line**: it is spanned by the basis vector of the trivial
path, which is then the only path `i → i`. This is the one place acyclicity enters the projective
cover below, and it fails for the one-loop quiver, where `(Pᵢ)ᵢ` is `k[X]`. -/
theorem exists_eq_smul_indecProjRepBasis_nil (h : Quiver.IsAcyclic Q) (i : Q)
...
/-- **The identity of `Pᵢ` is the morphism the universal property attaches to the basis vector of
the trivial path.** It is the instance of `TauCeti.indecProjRepHom_app_nil_self` at the identity,
whose action on that basis vector is the basis vector. -/
theorem indecProjRepHom_indecProjRepBasis_nil (i : Q) :
    indecProjRepHom i (indecProjRep k Q i) (indecProjRepBasis k i i Quiver.Path.nil)
      = 𝟙 (indecProjRep k Q i) :=
  indecProjRepHom_app_nil_self (𝟙 (indecProjRep k Q i))
...
theorem eq_id_of_app_indecProjRepBasis_nil {i : Q} (f : indecProjRep k Q i ⟶ indecProjRep k Q i)
  ...
  calc f = indecProjRepHom i (indecProjRep k Q i)
    ...
    _ = indecProjRepHom i (indecProjRep k Q i) (indecProjRepBasis k i i Quiver.Path.nil) := by
        rw [hf]
    _ = 𝟙 (indecProjRep k Q i) := indecProjRepHom_indecProjRepBasis_nil k i
...
/-- **Over an acyclic quiver `Pᵢ ↠ Sᵢ` pins down the basis vector of the trivial path**: it is the
only vector of `(Pᵢ)ᵢ` sent to the generator of the line `(Sᵢ)ᵢ`. Over a quiver with a cycle at
`i` this fails: any vector differing from it by a path of positive length `i → i` has the same
image. -/
theorem eq_indecProjRepBasis_nil_of_app_eq_simpleRepGenerator (h : Quiver.IsAcyclic Q) {i : Q}
    {v : (indecProjRep k Q i).obj i}
    (hv : (indecProjRepToSimpleRep k i).app ((Paths.of Q).obj i) v = simpleRepGenerator k i) :
    v = indecProjRepBasis k i i Quiver.Path.nil := by
  obtain ⟨c, rfl⟩ := exists_eq_smul_indecProjRepBasis_nil k h i v
...
/-- Over an acyclic quiver, a morphism `g : X ⟶ Pᵢ` whose composite with `Pᵢ ↠ Sᵢ` is an
...
`(Pᵢ)ᵢ`. -/
theorem exists_app_eq_indecProjRepBasis_nil (h : Quiver.IsAcyclic Q) {i : Q} {X : QuiverRep k Q}
...
/-- **`Pᵢ ↠ Sᵢ` is an essential epimorphism over an acyclic quiver, in the strong split form**: a
morphism into `Pᵢ` that becomes an epimorphism after composing with it is already a split
epimorphism. This is the projective-cover property of `Pᵢ` in its sharpest form: the composite
already being onto `Sᵢ` forces `g` to have a section, so `Pᵢ` is a direct summand of every source
that covers `Sᵢ` through it. -/
theorem isSplitEpi_of_epi_comp_indecProjRepToSimpleRep (h : Quiver.IsAcyclic Q) {i : Q}
...
/-- **`Pᵢ` is the projective cover of `Sᵢ` over an acyclic quiver**: `Pᵢ ↠ Sᵢ` is an epimorphism
from a projective object (`TauCeti.epi_indecProjRepToSimpleRep`,
...
theorem epi_of_epi_comp_indecProjRepToSimpleRep (h : Quiver.IsAcyclic Q) {i : Q}
    {X : QuiverRep k Q} (g : X ⟶ indecProjRep k Q i)
...
/-- **The cover is rigid**: over an acyclic quiver an endomorphism of `Pᵢ` commuting with
`Pᵢ ↠ Sᵢ` is the identity, not merely an isomorphism. This is the uniqueness half of "the"
projective cover, in its sharpest form: `Pᵢ ↠ Sᵢ` admits no automorphism over `Sᵢ` at all. -/
theorem eq_id_of_comp_indecProjRepToSimpleRep (h : Quiver.IsAcyclic Q) {i : Q}
...
/-- **The cover is minimal**: over an acyclic quiver every projective representation mapping onto
`Sᵢ` retracts onto `Pᵢ`, so `Pᵢ` is a direct summand of it. Projectivity lifts the map to `Pᵢ`
along the cover, and
`TauCeti.isSplitEpi_of_epi_comp_indecProjRepToSimpleRep` splits the lift. -/
theorem exists_isSplitEpi_of_epi_to_simpleRep (h : Quiver.IsAcyclic Q) {i : Q}

...
noncomputable def preprojectiveRelator : pathAlgebra k (Symmetrify Q) :=
  gaugedPreprojectiveRelator k fun _ _ _ => 1
...
noncomputable def gaugedPreprojectiveIdeal (ε : ∀ ⦃i j : Q⦄, (i ⟶ j) → k) :
...
noncomputable abbrev preprojectiveIdeal : TwoSidedIdeal (pathAlgebra k (Symmetrify Q)) :=
  gaugedPreprojectiveIdeal (Q := Q) k fun _ _ _ => 1
...
theorem preprojectiveRelator_mem_preprojectiveIdeal :
  ...
  rw [preprojectiveRelator, preprojectiveIdeal]
  exact gaugedPreprojectiveRelator_mem_gaugedPreprojectiveIdeal k _
...
theorem preprojectiveIdeal_eq_span_range_localPreprojectiveRelator :
  ...
  · rw [SetLike.mem_coe, gaugedPreprojectiveRelator_one, ← sum_localPreprojectiveRelator]
...
theorem gaugedPreprojectiveMk_surjective (ε : ∀ ⦃i j : Q⦄, (i ⟶ j) → k) :
    Function.Surjective (gaugedPreprojectiveMk k ε) :=
  Ideal.Quotient.mk_surjective

@[simp]
theorem gaugedPreprojectiveMk_eq_zero_iff (ε : ∀ ⦃i j : Q⦄, (i ⟶ j) → k)
    {f : pathAlgebra k (Symmetrify Q)} :
...
noncomputable def preprojectiveMk :
  ...
  gaugedPreprojectiveMk (Q := Q) k fun _ _ _ => 1
...
theorem preprojectiveMk_apply (f : pathAlgebra k (Symmetrify Q)) :
  ...
  rw [preprojectiveMk, gaugedPreprojectiveMk_apply]

theorem preprojectiveMk_surjective : Function.Surjective (preprojectiveMk k Q) :=
  gaugedPreprojectiveMk_surjective k (fun _ _ _ => (1 : k))

@[simp]
theorem preprojectiveMk_eq_zero_iff {f : pathAlgebra k (Symmetrify Q)} :
    preprojectiveMk k Q f = 0 ↔ f ∈ preprojectiveIdeal k Q := by
  simpa only [preprojectiveMk, preprojectiveIdeal] using
    (gaugedPreprojectiveMk_eq_zero_iff (Q := Q) k (fun _ _ _ => (1 : k)) (f := f))

@[simp]
theorem preprojectiveMk_preprojectiveRelator :
    preprojectiveMk k Q (preprojectiveRelator k Q) = 0 := by
  simpa only [preprojectiveMk, preprojectiveRelator] using
    (gaugedPreprojectiveMk_gaugedPreprojectiveRelator (Q := Q) k (fun _ _ _ => (1 : k)))
...
noncomputable def gaugedPreprojectiveLift (ε : ∀ ⦃i j : Q⦄, (i ⟶ j) → k)
    (f : pathAlgebra k (Symmetrify Q) →ₐ[k] B) (hf : f (gaugedPreprojectiveRelator k ε) = 0) :
...
noncomputable def preprojectiveLift (hf : f (preprojectiveRelator k Q) = 0) :
  ...
  gaugedPreprojectiveLift k (fun _ _ _ => 1) f hf
...
theorem preprojectiveLift_comp_preprojectiveMk (hf : f (preprojectiveRelator k Q) = 0) :
  ...
  exact gaugedPreprojectiveLift_comp_gaugedPreprojectiveMk k _ f hf

...
/-- The basis elements of a group element and of its inverse are mutually inverse.  Private: this
is group-algebra plumbing for the conjugation computations below. -/
private theorem single_mul_single_inv (g : G) :
    MonoidAlgebra.single g (1 : ℚ) * MonoidAlgebra.single g⁻¹ 1 = 1 := by
  rw [← MonoidAlgebra.of_apply, ← MonoidAlgebra.of_apply, ← map_mul, mul_inv_cancel, map_one]

/-- The basis elements of a group element and of its inverse are mutually inverse.  Private: this
is group-algebra plumbing for the conjugation computations below. -/
private theorem single_inv_mul_single (g : G) :
    MonoidAlgebra.single g⁻¹ (1 : ℚ) * MonoidAlgebra.single g 1 = 1 := by
  simpa using single_mul_single_inv g⁻¹
...
theorem youngSymmetrizer_relabel :
  ...
  simp only [mul_assoc]
  rw [← mul_assoc (MonoidAlgebra.single σ⁻¹ (1 : ℚ)), single_inv_mul_single, one_mul]
...
theorem mem_spechtIdeal_relabel_iff {x : MonoidAlgebra ℚ (Equiv.Perm (Fin μ.card))} :
  ...
  · rintro ⟨a, rfl⟩
    ...
    simp only [mul_assoc, single_inv_mul_single, mul_one]
  · rintro ⟨b, hb⟩
    refine ⟨b * MonoidAlgebra.single σ⁻¹ 1, ?_⟩
    rw [youngSymmetrizer_relabel]
    simp only [mul_assoc]
    rw [← mul_assoc (MonoidAlgebra.single σ⁻¹ (1 : ℚ)), single_inv_mul_single, one_mul,
      ← mul_assoc, hb, mul_assoc, single_mul_single_inv, mul_one]

noncomputable section

/-- Right multiplication by `σ` as a `ℚ`-linear equivalence from `ℚ[Sₙ] c_{σt}` to `ℚ[Sₙ] c_t`. -/
def spechtIdealRelabelEquiv :
    spechtIdeal (relabel σ t) ≃ₗ[ℚ] spechtIdeal t where
  toFun x := ⟨(x : MonoidAlgebra ℚ (Equiv.Perm (Fin μ.card))) * MonoidAlgebra.single σ 1,
    (mem_spechtIdeal_relabel_iff σ t).mp x.2⟩
  invFun y := ⟨(y : MonoidAlgebra ℚ (Equiv.Perm (Fin μ.card))) * MonoidAlgebra.single σ⁻¹ 1,
    (mem_spechtIdeal_relabel_iff σ t).mpr <| by
      rw [mul_assoc, single_inv_mul_single, mul_one]
      exact y.2⟩
  map_add' x y := by
  ...
  left_inv x := by
    refine Subtype.ext ?_
    simp only [mul_assoc, single_mul_single_inv, mul_one]
  right_inv y := by
    refine Subtype.ext ?_
    simp only [mul_assoc, single_inv_mul_single, mul_one]
...
`ℚ[Sₙ] c_t`: it commutes with left multiplication. -/
def spechtIdealRelabelRepEquiv :
    (spechtIdealRep (relabel σ t)).ρ.Equiv (spechtIdealRep t).ρ :=
  Representation.Equiv.mk (spechtIdealRelabelEquiv σ t) fun g =>
    LinearMap.ext fun x => Subtype.ext <| by
      simp only [LinearMap.coe_comp, Function.comp_apply, LinearEquiv.coe_coe, ofModule'_apply,
        Submodule.coe_smul, smul_eq_mul, spechtIdealRelabelEquiv_apply_coe, mul_assoc]
...
theorem finrank_spechtIdeal_relabel :
  ...
  (spechtIdealRelabelEquiv σ t).finrank_eq

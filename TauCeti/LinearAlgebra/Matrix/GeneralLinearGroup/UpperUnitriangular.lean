...
/-- A matrix unit is unipotent in its natural representation exactly when subtracting the
identity from its underlying matrix gives a nilpotent matrix. -/
theorem isUnipotent_toLin_iff (g : GL m R) :

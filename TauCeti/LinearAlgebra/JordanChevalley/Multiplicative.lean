...
* `LinearMap.GeneralLinearGroup.unipotentPart_eq_semisimplePart_inv_mul`: the unipotent factor is
  the product of the inverse semisimple factor with the original automorphism.
* `LinearMap.GeneralLinearGroup.coe_semisimplePart_mem_adjoin`: the semisimple factor belongs to
  the algebra generated by the original automorphism.
...
automorphism. -/
theorem unipotentPart_eq_semisimplePart_inv_mul (g : GeneralLinearGroup K V) :

...
* `LinearMap.GeneralLinearGroup.unipotentPart_eq_inv_mul`: the unipotent factor is the product
  of the inverse semisimple factor with the original automorphism.
* `LinearMap.GeneralLinearGroup.coe_semisimplePart_mem_adjoin` and
  `LinearMap.GeneralLinearGroup.coe_unipotentPart_mem_adjoin`: both factors belong to the algebra
  generated by the original automorphism.
...
/-- The unipotent factor is the product of the inverse semisimple factor with the original
automorphism. -/
theorem unipotentPart_eq_inv_mul (g : GeneralLinearGroup K V) :
...
/-- The unipotent factor of an automorphism belongs to the algebra generated by that
automorphism. -/
theorem coe_unipotentPart_mem_adjoin (g : GeneralLinearGroup K V) :
    (unipotentPart g : Module.End K V) ∈
      Algebra.adjoin K {(g : Module.End K V)} := by
  have hs_le :
      Algebra.adjoin K {(semisimplePart g : Module.End K V)} ≤
        Algebra.adjoin K {(g : Module.End K V)} :=
    Algebra.adjoin_le (Set.singleton_subset_iff.mpr (coe_semisimplePart_mem_adjoin g))
  have hs_inv : (((semisimplePart g)⁻¹ : GeneralLinearGroup K V) : Module.End K V) ∈
      Algebra.adjoin K {(g : Module.End K V)} :=
    hs_le (TauCeti.Units.coe_inv_mem_adjoin (semisimplePart g)
      (IsIntegral.of_finite K (semisimplePart g : Module.End K V)))
  rw [unipotentPart_eq_inv_mul, Units.val_mul]
  exact (Algebra.adjoin K {(g : Module.End K V)}).mul_mem hs_inv

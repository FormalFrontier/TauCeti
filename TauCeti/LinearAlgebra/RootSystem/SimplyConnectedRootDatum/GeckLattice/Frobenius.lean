...
Geck coordinate lattice, and `TauCeti.DynkinType.geckPointsMap` is the map its points inherit from
a homomorphism of value rings. This file reads that functoriality at the `p ^ k`-power Frobenius
...
theorem coe_geckFrobenius (g : t.geckPoints ht A) :
  ...
  t.coe_geckPointsMap ht (iterateFrobenius A p k) g
...
theorem geckFrobenius_zero : t.geckFrobenius ht p 0 A = MonoidHom.id _ := by
  rw [geckFrobenius, iterateFrobenius_zero, t.geckPointsMap_id ht]

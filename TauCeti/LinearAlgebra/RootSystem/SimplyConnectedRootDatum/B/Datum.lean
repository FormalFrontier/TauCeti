...
lemma typeB_sgn_mk_lt {a : ℕ} (h2 : a < 2 * n) (h : a < n) : sgn (⟨a, h2⟩ : Fin (2 * n)) = 1 :=
  if_pos h

lemma typeB_sgn_mk_ge {a : ℕ} (h2 : a < 2 * n) (h : n ≤ a) : sgn (⟨a, h2⟩ : Fin (2 * n)) = -1 :=
  if_neg (show ¬(a < n) by omega)

lemma typeB_axis_mk_lt {a : ℕ} (h2 : a < 2 * n) (h : a < n) : axis (⟨a, h2⟩ : Fin (2 * n)) = a :=
  if_pos h

lemma typeB_axis_mk_ge {a : ℕ} (h2 : a < 2 * n) (h : n ≤ a) :
    axis (⟨a, h2⟩ : Fin (2 * n)) = a - n :=
  if_neg (show ¬(a < n) by omega)

lemma typeB_wt_mk_lt {a : ℕ} (h2 : a < 2 * n) (h : a < n) :
    wt (⟨a, h2⟩ : Fin (2 * n)) = weight n a := by
  rw [wt, typeB_sgn_mk_lt h2 h, typeB_axis_mk_lt h2 h, one_smul]

lemma typeB_wt_mk_ge {a : ℕ} (h2 : a < 2 * n) (h : n ≤ a) :
    wt (⟨a, h2⟩ : Fin (2 * n)) = -weight n (a - n) := by
  rw [wt, typeB_sgn_mk_ge h2 h, typeB_axis_mk_ge h2 h, neg_one_smul]

lemma coe_shift_mk {a b : ℕ} (ha : a < 2 * n) (hb : b < n) :
    ((shift (⟨a, ha⟩ : Fin (2 * n)) ⟨b, hb⟩ : Fin (2 * n)) : ℕ) =
      if a + b < 2 * n then a + b else a + b - 2 * n := rfl

lemma shift_mk {a b c : ℕ} (ha : a < 2 * n) (hb : b < n) (hc : c < 2 * n)
    (h : (if a + b < 2 * n then a + b else a + b - 2 * n) = c) :
    shift (⟨a, ha⟩ : Fin (2 * n)) ⟨b, hb⟩ = ⟨c, hc⟩ :=
  Fin.ext (by rw [coe_shift_mk]; exact h)

lemma typeB_cwt_mk_lt {a : ℕ} (h2 : a < 2 * n) (h : a < n) :
    cwt (⟨a, h2⟩ : Fin (2 * n)) = coweight n a := by
  rw [cwt, typeB_sgn_mk_lt h2 h, typeB_axis_mk_lt h2 h, one_smul]

/-! ## The reflection permutation on the raw index type -/

/-- The reflection in the root indexed by `z`, acting on the raw index type. -/
def typeBReflIdx (z w : Fin (2 * n) × Fin n) : Fin (2 * n) × Fin n :=
  index (reflMap z.1 (shift z.1 z.2) w.1) (reflMap z.1 (shift z.1 z.2) (shift w.1 w.2))

lemma typeBReflIdx_involutive (z : Fin (2 * n) × Fin n) : Involutive (typeBReflIdx z) := by
  intro w
...
lemma rootIdx_typeBReflIdx (z w : Fin (2 * n) × Fin n) :
    rootIdx w - (rootIdx w ⬝ᵥ corootIdx z) • rootIdx z = rootIdx (typeBReflIdx z w) := by
  have hpq : IsPair z.1 (shift z.1 z.2) := isPair_shift _ _
  have hw : IsPair w.1 (shift w.1 w.2) := isPair_shift _ _
  have hRw := isPair_reflMap hpq hw
  rw [typeBReflIdx, rootIdx_index hRw, rootIdx, rootIdx, corootIdx]
...
lemma corootIdx_typeBReflIdx (z w : Fin (2 * n) × Fin n) :
...
lemma corootIdx_injective : Injective (corootIdx : Fin (2 * n) × Fin n → Fin n → ℤ) := by
  intro z z' h
  refine eq_of_forall_mem_iff fun m => ?_
...
lemma typeB_lt_two_mul_sq {i : ℕ} (hi : i < n) : i < 2 * n ^ 2 :=
...
def typeBRot (n : ℕ) : Equiv.Perm (Fin (2 * n)) where
...
lemma typeB_two_mul_sq (n : ℕ) : n * (2 * n) = 2 * n ^ 2 := by ring
...
def typeBEnum₀ (n : ℕ) : Fin (2 * n) × Fin n ≃ Fin (2 * n ^ 2) :=
...
def typeBSwapLast (n : ℕ) : Equiv.Perm (Fin (2 * n ^ 2)) :=
  if h : 0 < n then
...
def typeBSimplePair (i : Fin n) : Fin (2 * n) × Fin n :=
...
lemma typeBEnum_typeBSimplePair (i : Fin n) :
...
lemma typeBReflPerm_coroot (k l : Fin (2 * n ^ 2)) :
...
lemma rootIdx_typeBSimplePair (i : Fin n) :
  ...
  · have hshort : shift (⟨n - 1, by omega⟩ : Fin (2 * n)) (⟨0, by omega⟩ : Fin n) =
    ...
    rw [rootIdx, typeBSimplePair, dif_pos hlast, typeBShortPair]
    rw [hshort, rootOfPair_self, typeB_wt_mk_lt _ (show n - 1 < n by omega),
      weight_of_le (le_of_eq hlast.symm), sub_zero]
    congr 1
    omega
  · have hlt : (i : ℕ) + 1 < n := by omega
    ...
    have hne : (⟨n + (i : ℕ) + 1, by omega⟩ : Fin (2 * n)) ≠ ⟨(i : ℕ), by omega⟩ := by
      ...
      omega
    rw [rootIdx, typeBSimplePair, dif_neg hlast]
...
@[simp] theorem root_typeBSimpleIndex (i : Fin n) :
  ...
  funext k
  have hk := k.isLt
  simp only [weight, Pi.sub_apply, CartanMatrix.B, Matrix.of_apply, Fin.ext_iff]
  split_ifs <;> omega
...
@[simp] theorem coroot_typeBSimpleIndex (i : Fin n) :
  ...
  have hi := i.isLt
  rw [coroot_typeBSimplyConnectedRootDatum, typeBEnum_symm_typeBSimpleIndex, corootIdx]
  by_cases hlast : (i : ℕ) + 1 = n
  · have hshort : shift (⟨n - 1, by omega⟩ : Fin (2 * n)) (⟨0, by omega⟩ : Fin n) =
        ⟨n - 1, by omega⟩ := shift_eq_self rfl
    ...
    funext k
    have hk := k.isLt
    simp only [coweight, Pi.single_apply, Fin.ext_iff]
    split_ifs <;> omega
  · have hlt : (i : ℕ) + 1 < n := by omega
    have hv : shift (⟨n + (i : ℕ) + 1, by omega⟩ : Fin (2 * n)) (⟨n - 1, by omega⟩ : Fin n) =
        ⟨(i : ℕ), by omega⟩ :=
...
`2 (e₀ + ⋯ + e_j)`. -/
def typeBDualVec (n j : ℕ) : Fin n → ℤ := ∑ b ∈ Finset.range (j + 1), coweight n b

lemma weight_dotProduct_typeBDualVec {a j : ℕ} (ha : a < n) :
...
lemma typeBSimpleRoot_dotProduct_typeBDualVec (i j : Fin n) :
  ...
  by_cases hlast : (i : ℕ) + 1 = n
  · rw [weight_of_le (le_of_eq hlast.symm), zero_dotProduct]
...
lemma linearIndependent_typeBSimpleRoot (n : ℕ) :
...
def typeBSimpleSupport (n : ℕ) : Finset (Fin (2 * n ^ 2)) :=
  Finset.univ.map ⟨typeBSimpleIndex n, typeBSimpleIndex_injective⟩

lemma mem_typeBSimpleSupport {k : Fin (2 * n ^ 2)} :
...
lemma image_root_typeBSimpleSupport :
...
lemma image_coroot_typeBSimpleSupport :
...
lemma typeB_sub_mem_closure {M : Type*} [AddCommGroup M] (f : ℕ → M) {a b : ℕ}
    (hb : b ≤ n) (hab : a ≤ b) :
...
lemma typeB_weight_mem_closure {a : ℕ} (ha : a ≤ n) :
    weight n a ∈ AddSubmonoid.closure
      (range fun i : Fin n => weight n (i : ℕ) - weight n ((i : ℕ) + 1)) := by
  have h := typeB_sub_mem_closure (n := n) (weight n) le_rfl ha
  rwa [weight_of_le le_rfl, sub_zero] at h

lemma typeB_wt_add_wt_mem_closure (u v : Fin (2 * n)) (hu : sgn u = 1) (hv : sgn v = 1) :
    wt u + wt v ∈ AddSubmonoid.closure
      (range fun i : Fin n => weight n (i : ℕ) - weight n ((i : ℕ) + 1)) := by
  rw [wt, wt, hu, hv, one_smul, one_smul]
  exact AddSubmonoid.add_mem _ (typeB_weight_mem_closure (le_of_lt (axis_lt u)))
    (typeB_weight_mem_closure (le_of_lt (axis_lt v)))

lemma typeB_wt_sub_wt_mem_closure (u v : Fin (2 * n)) (hu : sgn u = 1) (hv : sgn v = -1)
    (hle : axis u ≤ axis v) :
    wt u + wt v ∈ AddSubmonoid.closure
      (range fun i : Fin n => weight n (i : ℕ) - weight n ((i : ℕ) + 1)) := by
  rw [wt, wt, hu, hv, one_smul, neg_one_smul, ← sub_eq_add_neg]
  exact typeB_sub_mem_closure (n := n) (weight n) (le_of_lt (axis_lt v)) hle

lemma typeB_mem_closure_single {w : Fin n → ℤ} (hw : ∀ j, 0 ≤ w j) :
...
lemma half_apply_last {u v : Fin (2 * n)} {j : Fin n} (hj : (j : ℕ) + 1 = n) :
    half u v j = if sgn u = sgn v then sgn u else 0 := if_pos hj

lemma half_apply_of_ne {u v : Fin (2 * n)} {j : Fin n} (hj : (j : ℕ) + 1 ≠ n) :
    half u v j = sgn u * (if axis u ≤ (j : ℕ) then 1 else 0) +
      sgn v * (if axis v ≤ (j : ℕ) then 1 else 0) := if_neg hj

lemma typeB_half_nonneg_same {u v : Fin (2 * n)} (hsu : sgn u = 1) (hsv : sgn v = 1) (j : Fin n) :
    0 ≤ half u v j := by
  by_cases hj : (j : ℕ) + 1 = n
...
lemma typeB_half_nonpos_same {u v : Fin (2 * n)} (hsu : sgn u = -1) (hsv : sgn v = -1)
    (j : Fin n) : half u v j ≤ 0 := by
  by_cases hj : (j : ℕ) + 1 = n
  · rw [half_apply_last hj, hsu, hsv, if_pos rfl]
    omega
  · rw [half_apply_of_ne hj, hsu, hsv]
...
lemma typeB_half_nonneg_mixed {u v : Fin (2 * n)} (hsu : sgn u = 1) (hsv : sgn v = -1)
...
lemma typeB_half_nonpos_mixed {u v : Fin (2 * n)} (hsu : sgn u = 1) (hsv : sgn v = -1)
...
def typeBSimplyConnectedBase (n : ℕ) : (typeBSimplyConnectedRootDatum n).Base where
  ...
  coroot_mem_or_neg_mem k := by
    ...
    rcases sgn_eq_one_or_neg_one u with hsu | hsu <;>
      rcases sgn_eq_one_or_neg_one v with hsv | hsv
    · exact Or.inl (typeB_mem_closure_single (typeB_half_nonneg_same hsu hsv))
    · rcases le_total (axis u) (axis v) with hle | hle
      · exact Or.inl (typeB_mem_closure_single (typeB_half_nonneg_mixed hsu hsv hle))
      · exact Or.inr (typeB_mem_closure_single fun j =>
          neg_nonneg.mpr (typeB_half_nonpos_mixed hsu hsv hle j))
    · rw [half_comm]
      rcases le_total (axis v) (axis u) with hle | hle
      · exact Or.inl (typeB_mem_closure_single (typeB_half_nonneg_mixed hsv hsu hle))
      · exact Or.inr (typeB_mem_closure_single fun j =>
          neg_nonneg.mpr (typeB_half_nonpos_mixed hsv hsu hle j))
    · exact Or.inr (typeB_mem_closure_single fun j =>
        neg_nonneg.mpr (typeB_half_nonpos_same hsu hsv j))

...
The body is exposed because a downstream statement about Geck's construction on
`TauCeti.DynkinType.rationalBase` has to be readable as a statement about this carrier. -/
@[expose] def lieAlgebra : LieSubalgebra ℚ (Matrix (t.GeckIndex ht) (t.GeckIndex ht) ℚ) :=

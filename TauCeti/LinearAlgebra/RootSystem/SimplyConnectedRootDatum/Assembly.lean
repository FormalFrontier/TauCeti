...
* `TauCeti.DynkinType.simpleIndex_A` and its eight siblings: the simple-root index of the
  dispatcher is the family module's own simple-root index, one family at a time.
...
/-! Each family module enumerates its own roots and names its own simple-root index. The nine
...
theorem simpleIndex_A (n : ℕ) (ht : (A n).Valid) (i : Fin n) :

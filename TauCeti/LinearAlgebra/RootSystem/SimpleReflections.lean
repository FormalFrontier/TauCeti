...
* `TauCeti.simpleWordProd` multiplies out a word in the simple reflections of a base.
* `TauCeti.exists_simpleWordProd_eq` writes every Weyl-group element as a product of simple
  reflections.
...
noncomputable def simpleWordProd (b : P.Base) (l : List b.support) : P.weylGroup :=
  (l.map fun i : b.support => RootPairing.weylGroup.ofIdx P (i : ι)).prod

variable {P}

@[simp]
lemma simpleWordProd_nil (b : P.Base) : simpleWordProd P b [] = 1 :=
  List.prod_nil

@[simp]
lemma simpleWordProd_cons (b : P.Base) (i : b.support) (l : List b.support) :
    simpleWordProd P b (i :: l) = RootPairing.weylGroup.ofIdx P (i : ι) * simpleWordProd P b l :=
  List.prod_cons

@[simp]
lemma simpleWordProd_append (b : P.Base) (l l' : List b.support) :
    simpleWordProd P b (l ++ l') = simpleWordProd P b l * simpleWordProd P b l' := by
  rw [simpleWordProd, List.map_append, List.prod_append]
  rfl
...
theorem exists_simpleWordProd_eq (w : P.weylGroup) :
    ∃ l : List b.support, simpleWordProd P b l = w := by
  have hw :
  ...
  refine ⟨l.toList, ?_⟩
  rw [simpleWordProd, ← FreeMonoid.lift_apply]
  exact hl

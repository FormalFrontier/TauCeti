...
/-- A chain, evaluated at a weight affine in the position: the row vanishes except at the far end,
where it is `(n + 1) c`. This is `TauCeti.sum_range_chainEntry_mul_affine`, whose two
...
private theorem sum_fin_chainEntry_mul_linear {n : ℕ} (m : Fin n) (c : ℚ) :
  ...
  rw [Finset.sum_congr rfl fun w _ ↦ hcongr w,
    Fin.sum_univ_eq_sum_range
      (fun w ↦ ((chainEntry (m : ℕ) w : ℤ) : ℚ) * (c * (w : ℚ) + c)) n,

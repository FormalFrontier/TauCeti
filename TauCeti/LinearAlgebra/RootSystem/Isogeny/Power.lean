...
`TauCeti/LinearAlgebra/RootSystem/Isogeny/Basic.lean` to the special isogenies of the pinned `B₂`,
`G₂` and `F₄` root data. Their squares are scalings by two, three and two, respectively, so their
even powers are scalings and an odd power is a scaling composed with the original special isogeny.
...
* `TauCeti.DynkinType.b2SpecialIsogeny_pow_mul_pow` and its `G₂` and `F₄` counterparts: the
  square relation for every power of the special isogeny.
* `TauCeti.DynkinType.b2SpecialIsogeny_pow_odd_weightMap_root` and its `G₂` and `F₄`
  counterparts: the action on roots; the corresponding `coroot_coweightMap` lemmas give the dual
  action.
* `TauCeti.DynkinType.b2SpecialIsogeny_pow_odd_exponent_at_lengthPerm_of_isLongSimpleRoot` and its
  companion: the root-datum exponent at a length-exchanged simple index of `B₂`.
...
/-! ## Odd powers of the special isogeny of `B₂` -/

/-- **The even powers of the special isogeny of `B₂` are Frobenius scalings.** -/
theorem b2SpecialIsogeny_pow_two_mul (m : ℕ) :
    b2SpecialIsogeny ^ (2 * m) = smulId (typeBSimplyConnectedRootDatum 2) (2 ^ m) :=
  pow_two_mul_of_mul_self b2SpecialIsogeny_mul_self m

/-- **Every power of the special isogeny of `B₂` squares to a Frobenius scaling.** At
`k = 2 * m + 1` this is the root-datum form of `steinberg(m) ^ 2 = Frob_(2 ^ (2m+1))`, the relation
whose group-scheme counterpart cuts out the Suzuki groups `²B₂(2 ^ (2m+1))`. Nothing about a group
is proved here. -/
theorem b2SpecialIsogeny_pow_mul_pow (k : ℕ) :
    b2SpecialIsogeny ^ k * b2SpecialIsogeny ^ k =
      smulId (typeBSimplyConnectedRootDatum 2) (2 ^ k) :=
  pow_mul_pow_of_mul_self b2SpecialIsogeny_mul_self k

/-- An odd power of the special isogeny of `B₂` permutes the eight roots exactly as the isogeny
itself does.
...
theorem b2SpecialIsogeny_pow_odd_indexEquiv_apply (m : ℕ) (i : Fin (2 * 2 ^ 2)) :
    (b2SpecialIsogeny ^ (2 * m + 1)).indexEquiv i = b2SpecialIsogenyIndexEquiv i := by
  rw [pow_odd_indexEquiv_of_mul_self_eq_smulId b2SpecialIsogeny_mul_self,
    b2SpecialIsogeny_indexEquiv_apply]

/-- **The root-datum exponent of an odd power of the special isogeny of `B₂`** is the exponent
of the special isogeny at the source root, times `2 ^ m`. -/
@[simp] theorem b2SpecialIsogeny_pow_odd_exponent (m : ℕ) (i : Fin (2 * 2 ^ 2)) :
    (b2SpecialIsogeny ^ (2 * m + 1)).exponent i = b2SpecialIsogenyExponent i * 2 ^ m := by
  rw [pow_odd_exponent_of_mul_self_eq_smulId b2SpecialIsogeny_mul_self,
    b2SpecialIsogeny_exponent]
  norm_num

/-- **The defining relation of an odd power of the special isogeny of `B₂` on the roots.** The
displayed coefficient is the root-datum exponent at the source index `i`. -/
theorem b2SpecialIsogeny_pow_odd_weightMap_root (m : ℕ) (i : Fin (2 * 2 ^ 2)) :
    (b2SpecialIsogeny ^ (2 * m + 1)).weightMap
        ((typeBSimplyConnectedRootDatum 2).root i) =
      (b2SpecialIsogenyExponent i * 2 ^ m) •
        (typeBSimplyConnectedRootDatum 2).root (b2SpecialIsogenyIndexEquiv i) := by
  rw [root_weightMap_pow_odd_of_mul_self_eq_smulId b2SpecialIsogeny_mul_self,
    b2SpecialIsogeny_exponent, b2SpecialIsogeny_indexEquiv_apply]
  norm_num

/-- **The defining relation of an odd power of the special isogeny of `B₂` on the coroots.** -/
theorem b2SpecialIsogeny_pow_odd_coroot_coweightMap (m : ℕ) (i : Fin (2 * 2 ^ 2)) :
    (b2SpecialIsogeny ^ (2 * m + 1)).coweightMap
        ((typeBSimplyConnectedRootDatum 2).coroot (b2SpecialIsogenyIndexEquiv i)) =
      (b2SpecialIsogenyExponent i * 2 ^ m) • (typeBSimplyConnectedRootDatum 2).coroot i := by
  rw [← b2SpecialIsogeny_indexEquiv_apply]
  rw [coroot_coweightMap_pow_odd_of_mul_self_eq_smulId b2SpecialIsogeny_mul_self,
    b2SpecialIsogeny_exponent]
  norm_num

/-- At the source index length-exchanged from a long simple node of `B₂`, the root-datum exponent
of the `(2m+1)`-st power is `2 ^ m`. -/
theorem b2SpecialIsogeny_pow_odd_exponent_at_lengthPerm_of_isLongSimpleRoot (m : ℕ) {i : Fin 2}
    (hi : (B 2).IsLongSimpleRoot i) :
    (b2SpecialIsogeny ^ (2 * m + 1)).exponent
        (typeBSimpleIndex 2 (lengthPermRankTwo i)) = 2 ^ m := by
  have hshort : ¬ (B 2).IsLongSimpleRoot (lengthPermRankTwo i) := by simpa using hi
  rw [b2SpecialIsogeny_pow_odd_exponent,
    (b2SpecialIsogenyExponent_typeBSimpleIndex_eq_one_iff _).mpr hshort, one_mul]

/-- At the source index length-exchanged from a short simple node of `B₂`, the root-datum exponent
of the `(2m+1)`-st power is `2 ^ (m + 1)`. -/
theorem b2SpecialIsogeny_pow_odd_exponent_at_lengthPerm_of_not_isLongSimpleRoot (m : ℕ)
    {i : Fin 2} (hi : ¬ (B 2).IsLongSimpleRoot i) :
    (b2SpecialIsogeny ^ (2 * m + 1)).exponent
        (typeBSimpleIndex 2 (lengthPermRankTwo i)) = 2 ^ (m + 1) := by
  have hlong : (B 2).IsLongSimpleRoot (lengthPermRankTwo i) := by simpa using hi
  have hlength := (isLongSimpleRoot_iff_b2Length_eq_two (lengthPermRankTwo i)).mp hlong
  rw [b2SpecialIsogeny_pow_odd_exponent, b2SpecialIsogenyExponent_typeBSimpleIndex,
    ← b2Length_castLE, hlength, pow_succ]
  ring

/-- **The two root-datum exponents of a power of the special isogeny of `B₂`, at a root and at
its image, multiply to `2 ^ k`.** -/
theorem b2SpecialIsogeny_pow_exponent_mul_exponent (k : ℕ) (i : Fin (2 * 2 ^ 2)) :
    (b2SpecialIsogeny ^ k).exponent i *
        (b2SpecialIsogeny ^ k).exponent ((b2SpecialIsogeny ^ k).indexEquiv i) = 2 ^ k := by
  rw [pow_exponent_mul_exponent_indexEquiv_of_mul_self_eq_smulId
    b2SpecialIsogeny_mul_self]
  norm_num

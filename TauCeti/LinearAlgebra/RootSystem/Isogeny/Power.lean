...
indices are `p ^ m` and `p ^ (m + 1)`. The exponent stored at a root index is the root-subgroup
parameter attached to its image under the length-exchanging permutation. Thus the root-datum
exponent is `p ^ (m + 1)` at a long source node and `p ^ m` at a short source node, while the
root-subgroup parameter is `p ^ m` at a long node and `p ^ (m + 1)` at a short node. The simple-node
lemmas below state the latter convention by evaluating the root-datum exponent at the
length-exchanged index.

Squaring any power returns a scaling,
`τ ^ k * τ ^ k = smulId P (p ^ k)`. At `k = 2 * m + 1` this is the root-datum form of the relation
`τ ^ (2m+1)` squares to `Frob_{p ^ (2m+1)}` that cuts out the Suzuki and Ree groups over the field
with `p ^ (2m+1)` elements.
...
* `TauCeti.DynkinType.g2SpecialIsogeny_pow_odd_exponent_at_lengthPerm_of_isLongSimpleRoot` and its
  three companions: the root-subgroup parameter of an odd power at a simple node.
...
theorem g2SpecialIsogeny_pow_odd_indexEquiv_apply (m : ℕ) (i : Fin 12) :
  ...
  rw [pow_odd_indexEquiv_of_mul_self_eq_smulId g2SpecialIsogeny_comp_self,
...
@[simp] theorem g2SpecialIsogeny_pow_odd_exponent (m : ℕ) (i : Fin 12) :
  ...
  rw [pow_odd_exponent_of_mul_self_eq_smulId g2SpecialIsogeny_comp_self,
    g2SpecialIsogeny_exponent]
  norm_num

/-- **The defining relation of an odd power of the special isogeny of `G₂` on the roots.** The
displayed coefficient at the source index `i` is the root-subgroup parameter attached to the
length-exchanged target index `g2SpecialIsogenyIndex i`. -/
theorem g2SpecialIsogeny_pow_odd_weightMap_root (m : ℕ) (i : Fin 12) :
  ...
  rw [root_weightMap_pow_odd_of_mul_self_eq_smulId g2SpecialIsogeny_comp_self,
...
theorem g2SpecialIsogeny_pow_odd_coroot_coweightMap (m : ℕ) (i : Fin 12) :
  ...
  rw [coroot_coweightMap_pow_odd_of_mul_self_eq_smulId g2SpecialIsogeny_comp_self,
    g2SpecialIsogeny_exponent]
  norm_num

/-- At a long simple root-subgroup node of `G₂`, the parameter of the `(2m+1)`-st power is `3 ^ m`.
The root-datum exponent is evaluated at the length-exchanged source index. -/
...
/-- At a short simple root-subgroup node of `G₂`, the parameter of the `(2m+1)`-st power is
`3 ^ (m + 1)`. The root-datum exponent is evaluated at the length-exchanged source index. -/
...
theorem f4SpecialIsogeny_pow_odd_indexEquiv_apply (m : ℕ) (i : Fin 48) :
  ...
  rw [pow_odd_indexEquiv_of_mul_self_eq_smulId f4SpecialIsogeny_comp_self,
...
@[simp] theorem f4SpecialIsogeny_pow_odd_exponent (m : ℕ) (i : Fin 48) :
  ...
  rw [pow_odd_exponent_of_mul_self_eq_smulId f4SpecialIsogeny_comp_self,
    f4SpecialIsogeny_exponent]
  norm_num

/-- **The defining relation of an odd power of the special isogeny of `F₄` on the roots.** The
displayed coefficient at the source index `i` is the root-subgroup parameter attached to the
length-exchanged target index `f4SpecialIsogenyIndex i`. -/
theorem f4SpecialIsogeny_pow_odd_weightMap_root (m : ℕ) (i : Fin 48) :
  ...
  rw [root_weightMap_pow_odd_of_mul_self_eq_smulId f4SpecialIsogeny_comp_self,
...
theorem f4SpecialIsogeny_pow_odd_coroot_coweightMap (m : ℕ) (i : Fin 48) :
  ...
  rw [coroot_coweightMap_pow_odd_of_mul_self_eq_smulId f4SpecialIsogeny_comp_self,
    f4SpecialIsogeny_exponent]
  norm_num

/-- At a long simple root-subgroup node of `F₄`, the parameter of the `(2m+1)`-st power is `2 ^ m`.
The root-datum exponent is evaluated at the length-exchanged source index. -/
...
/-- At a short simple root-subgroup node of `F₄`, the parameter of the `(2m+1)`-st power is
`2 ^ (m + 1)`. The root-datum exponent is evaluated at the length-exchanged source index. -/

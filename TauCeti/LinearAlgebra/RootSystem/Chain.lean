...
* `TauCeti.sum_range_chainEntry_mul`: the row of a chain at a position, evaluated at a weight `g`.
  Away from the two ends it is the second difference `2 g m - g (m - 1) - g (m + 1)`, so a weight
  that is affine in the position is annihilated there.
...
/-- **The row of a chain, evaluated at a weight.** Along `n` positions `0, …, n - 1`, the entries at
the position `m` collect `2 g m - g (m - 1) - g (m + 1)`, with the preceding term absent at `m = 0`
and the following term absent at the far end. An affine weight is therefore annihilated away from
the two ends.
...
theorem sum_range_chainEntry_mul {R : Type*} [Ring R] {n m : ℕ} (hm : m < n) (g : ℕ → R) :
    ∑ s ∈ Finset.range n, (chainEntry m s : R) * g s
      = 2 * g m - (if m = 0 then 0 else g (m - 1))
        - (if m + 1 = n then 0 else g (m + 1)) := by
  simpa [chainEntry] using (sum_range_chainBEntry_mul (L := 0) hm g)
...
theorem sum_range_chainEntry_mul_affine {R : Type*} [Ring R] {n m : ℕ} (hm : m < n)
  ...
  · subst m
    ...
    · subst n
      simp
    · simp only [Nat.cast_zero, mul_zero, zero_add, hend, ite_false, Nat.cast_one, mul_one,
        ite_true]
      noncomm_ring
  · have hpos : 1 ≤ m := Nat.one_le_iff_ne_zero.mpr hzero
    ...
    · subst n
      ...
      noncomm_ring
    · simp only [hzero, hend, ite_false, Nat.cast_sub hpos, Nat.cast_add, Nat.cast_one]
      noncomm_ring

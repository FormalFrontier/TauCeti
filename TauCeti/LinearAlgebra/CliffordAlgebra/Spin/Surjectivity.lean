...
The spin action lands in the special orthogonal group. On a finite free module, the key parity
calculation is carried out on the Lipschitz group: grade involution acts on a Lipschitz element by
the determinant of its twisted-conjugation action. Thus an even element has determinant one, while
a determinant-one Pin lift is even. Outside the finite-free case, Mathlib defines the determinant
to be one, so the codomain restriction remains canonical. Combining the finite-dimensional parity
calculation with Pin surjectivity gives Spin surjectivity.
...
theorem spinToSpecialOrthogonal_surjective (Q : QuadraticForm K V) (hQ : Q.Nondegenerate) :
  ...
  have hdet : lipschitzDet Q l =
      LinearEquiv.det
        (((pinToOrthogonal Q p : QuadraticMap.orthogonalGroup Q) : V ≃ₗ[K] V)) :=
    lipschitzDet_pinToLipschitz Q p
  have hdet_one :
      LinearEquiv.det
  ...
  have hinv := involute_eq_det_smul_of_mem_lipschitz Q l.2
  rw [hlcoe, hdet, hdet_one, Units.val_one, map_one, one_mul] at hinv
  let s : spinGroup Q := ⟨p, p.2, mem_even_of_involute_eq Q hinv⟩
  refine ⟨s, ?_⟩
  apply Subtype.ext
  -- Reduce equality in the special orthogonal subgroup to the underlying linear equivalences.
  change (((spinToOrthogonal Q s : QuadraticMap.orthogonalGroup Q) : V ≃ₗ[K] V)) = g

...
* `TauCeti.ReducedTensorWords.iSup_range_of`: the positive tensor powers generate reduced tensor
  words.
* `TauCeti.ReducedTensorWords.linearMap_ext`: a linear map out of reduced tensor words is
  determined by its restrictions to the positive tensor powers.
...
/-- Reduced tensor words are generated by the images of the positive tensor powers.

This is `DFinsupp.iSup_range_lsingle` restated for `of`, whose body is not exposed, so the
`DFinsupp` form does not apply to goals phrased with `of`. -/
theorem iSup_range_of : ⨆ n : {n : ℕ // 0 < n}, LinearMap.range (of R M n) = ⊤ :=
  DFinsupp.iSup_range_lsingle

/-- Two linear maps out of reduced tensor words agree as soon as they agree on every positive
tensor power.

This is `DirectSum.linearMap_ext` restated for `of`, whose body is not exposed, so the
`DirectSum` form does not apply to goals phrased with `of`. -/
theorem linearMap_ext {N : Type uN} [AddCommMonoid N] [Module R N]
    {f g : ReducedTensorWords R M →ₗ[R] N} (h : ∀ n, f ∘ₗ of R M n = g ∘ₗ of R M n) : f = g :=
  DirectSum.linearMap_ext R h
...
theorem deconcatenation_natural (f : M →ₗ[R] N) :
  ...
  refine linearMap_ext R M fun n ↦ LinearMap.ext fun z ↦ ?_

...
theorem iSup_range_of : ⨆ n : {n : ℕ // 0 < n}, LinearMap.range (of R M n) = ⊤ := by
  have h : ∀ x : ReducedTensorWords R M,
      x ∈ ⨆ n : {n : ℕ // 0 < n}, LinearMap.range (of R M n) := by
    intro x
    induction x using DirectSum.induction_on with
    | zero => exact Submodule.zero_mem _
    | of n y => exact Submodule.mem_iSup_of_mem n ⟨y, by simp [of, DirectSum.lof_eq_of]⟩
    | add u v hu hv => exact Submodule.add_mem _ hu hv
  exact eq_top_iff.mpr fun x _ ↦ h x

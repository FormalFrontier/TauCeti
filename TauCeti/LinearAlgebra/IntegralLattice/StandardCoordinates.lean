...
An integral symmetric matrix `G` of size `n` presents an integral lattice on `Fin n → ℚ`, namely
`ofGramMatrix (Pi.basisFun ℚ (Fin n)) G hG`, whose carrier is the standard integral lattice `ℤⁿ`
...
variable {n : ℕ} (G : Matrix (Fin n) (Fin n) ℤ) (hG : G.IsSymm)

/-- The form of the lattice presented by `G` on `Fin n → ℚ`, expanded in coordinates. -/
theorem form_ofGramMatrix_basisFun_apply (x y : Fin n → ℚ) :
    (ofGramMatrix (Pi.basisFun ℚ (Fin n)) G hG).form x y =
...
theorem form_ofGramMatrix_basisFun_right (x : Fin n → ℚ) (i : Fin n) :
    (ofGramMatrix (Pi.basisFun ℚ (Fin n)) G hG).form x (Pi.basisFun ℚ (Fin n) i) =
      ∑ j, ((G i j : ℤ) : ℚ) * x j := by
  rw [form_ofGramMatrix_basisFun_apply]
  have h : ∀ k : Fin n, (∑ j, ((G k j : ℤ) : ℚ) * (Pi.basisFun ℚ (Fin n) i) j)
      = ((G k i : ℤ) : ℚ) := by
    intro k
  ...
  rw [show G k i = G i k from hG.apply i k]
  exact mul_comm _ _

/-- The Gram matrix of a Gram-matrix lattice in its standard coordinate basis is the given
matrix. -/
theorem form_ofGramMatrix_basisFun_basisFun (i j : Fin n) :
    (ofGramMatrix (Pi.basisFun ℚ (Fin n)) G hG).form
        (Pi.basisFun ℚ (Fin n) i) (Pi.basisFun ℚ (Fin n) j) = ((G i j : ℤ) : ℚ) := by
...
integers. -/
theorem mem_ofGramMatrix_basisFun_carrier_iff (x : Fin n → ℚ) :
    x ∈ (ofGramMatrix (Pi.basisFun ℚ (Fin n)) G hG).carrier ↔ ∀ i, ∃ z : ℤ, (z : ℚ) = x i := by
...
theorem mem_ofGramMatrix_basisFun_dualCarrier_iff (x : Fin n → ℚ) :
    x ∈ (ofGramMatrix (Pi.basisFun ℚ (Fin n)) G hG).dualCarrier ↔
      ∀ i, ∃ z : ℤ, (z : ℚ) = ∑ j, ((G i j : ℤ) : ℚ) * x j := by
  rw [dualCarrier, LinearMap.BilinForm.mem_dualSubmodule]
  constructor
  · intro hx i
    have hi := hx (Pi.basisFun ℚ (Fin n) i)
  ...
  · intro h y hy
    ...
    | add a b _ _ ha hb =>
        ...
        exact add_mem ha hb
    | smul c a _ ha =>
        rw [show (ofGramMatrix (Pi.basisFun ℚ (Fin n)) G hG).form x (c • a) =
            c • (ofGramMatrix (Pi.basisFun ℚ (Fin n)) G hG).form x a from map_zsmul _ _ _]

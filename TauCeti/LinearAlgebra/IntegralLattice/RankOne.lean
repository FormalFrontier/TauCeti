...
`m = 0` form is the same construction with no discriminant group at all.  The two rank-one
lattices already present as Layer 1 acceptance examples are the two smallest members,
...
@[expose] noncomputable def rankOneDualGen : (rankOne m).dualCarrier :=
...
@[expose] noncomputable def rankOneClass : (rankOne m).DiscriminantGroup :=
...
`e / (2m)`. -/
@[expose] noncomputable def rankOneDiscriminantEquiv :
...
compatibility check, made directly against the two computed tables rather than through the general
theorem `TauCeti.IntegralLattice.polar_discriminantQuadraticMap`. -/
theorem polar_discriminantQuadraticMap_zsmul_rankOneClass (k l : ℤ) :
    QuadraticMap.polar ((rankOne m).discriminantQuadraticMap (isEven_rankOne m))
      ...
      ((k * l / (2 * m) : ℚ) : AddCircle (1 : ℚ)) := by
  have h2m := rankOne_two_mul_ne_zero m
  rw [QuadraticMap.polar, ← add_smul, discriminantQuadraticMap_zsmul_rankOneClass,
    discriminantQuadraticMap_zsmul_rankOneClass, discriminantQuadraticMap_zsmul_rankOneClass,
    ← AddCircle.coe_sub, ← AddCircle.coe_sub]
  congr 1
  push_cast
  field_simp
  ring
...
theorem isPosDef_rankOne_iff (m : ℤ) [NeZero m] : (rankOne m).IsPosDef ↔ 0 < m := by
  refine ⟨fun h ↦ ?_, isPosDef_rankOne⟩
  by_contra hm
  have hneg : m < 0 := lt_of_le_of_ne (not_lt.mp hm) (NeZero.ne m)
  have hone := (rankOne m).isPosDef_iff.mp h 1 one_ne_zero
  rw [rankOne_form_apply] at hone
  have hmq : (m : ℚ) < 0 := by exact_mod_cast hneg
  nlinarith

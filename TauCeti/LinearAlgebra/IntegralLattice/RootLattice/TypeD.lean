...
value and are told apart by their mutual pairing `b(s, c) = (n - 2) / 4`.
...
* `TauCeti.IntegralLattice.checkerboardDiscriminantGroup_cases`: the four classes exhaust the
  discriminant group.
...
* `TauCeti.IntegralLattice.discriminantPairing_checkerboardSpinorClass_checkerboardCospinorClass`:
  the pairing `(n - 2) / 4` which separates the two even-rank spinor classes.
...
theorem single_two_mem_checkerboardCarrier (i : Fin n) :
...
theorem single_sub_single_mem_checkerboardCarrier (i j : Fin n) :
...
theorem checkerboardCarrier_le_intSpan :
...
theorem checkerboardCarrier_fg : (checkerboardCarrier n).FG := by
...
theorem checkerboardCarrier_span_eq_top :
...
theorem checkerboardDiscriminantGroup_cases (g : (checkerboardLattice n).DiscriminantGroup) :
...
theorem natCard_discriminantGroup_checkerboardLattice :
  ...
  have huniv : (Set.univ : Set (checkerboardLattice n).DiscriminantGroup) =
    ...
    exact checkerboardDiscriminantGroup_cases g
...
values, distinguishes the two spinor classes of an even-rank checkerboard lattice. -/
...
noncomputable def zmodFourAddEquivCheckerboardDiscriminantGroup (hn : Odd n) :
    ZMod 4 ≃+ (checkerboardLattice n).DiscriminantGroup :=
  zmodAddEquivOfGenerator (g := checkerboardSpinorClass n)
    (fun x ↦ by
      have hc : (-1 : ℤ) • checkerboardSpinorClass n = checkerboardCospinorClass n := by
        ...
        abel
      rcases checkerboardDiscriminantGroup_cases x with h | h | h | h
...
theorem zmodFourAddEquivCheckerboardDiscriminantGroup_apply_one (hn : Odd n) :
    zmodFourAddEquivCheckerboardDiscriminantGroup n hn 1 = checkerboardSpinorClass n :=
...
noncomputable def zmodTwoProdAddEquivCheckerboardDiscriminantGroup (hn : Even n) :
  ...
  rcases checkerboardDiscriminantGroup_cases g with h | h | h | h
  · exact ⟨(0, 0), by simp [h]⟩
  · exact ⟨(1, 0), by

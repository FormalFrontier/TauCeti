...
* `TauCeti.SpecialLinear.groupScheme`, `TauCeti.SpecialLinear.groupSchemeι`, and
  `TauCeti.SpecialLinear.groupSchemeιGeneralLinear`: the kernel group scheme and its closed
  immersions into the general-linear Hopf spectrum and named general-linear group scheme.
...
* `TauCeti.SpecialLinear.schemePointsMulEquiv`: the corresponding equivalence on scheme-valued
  points.
...
/-- **The determinant coordinate map agrees with its counit modulo `det - 1`.** On the group
algebra of `ℤ`, the difference between the image of an element and the scalar its counit records
lies in the principal ideal generated by the generic determinant minus one. -/
private lemma determinantCoordinateMap_sub_algebraMap_counit_mem_span
  ...
  -- On a generator `det ^ m` this is `groupLike_zpow_sub_one_mem_span`; linearity does the rest.
...
/-- The harder inclusion of `TauCeti.SpecialLinear.definingHopfIdeal_toIdeal`: the determinant
kernel Hopf ideal is contained in the principal ideal generated by `det - 1`. -/
private lemma definingHopfIdeal_toIdeal_le :
  ...
  intro x hx
  rw [Ideal.mem_comap]
  -- The counit vanishes on the kernel Hopf ideal, so the image is the difference of the
  -- previous lemma.
  ...
  simpa [hx0] using determinantCoordinateMap_sub_algebraMap_counit_mem_span R n x
...
theorem definingHopfIdeal_toIdeal :
    (definingHopfIdeal R n).toIdeal =
      Ideal.span
        {(GeneralLinear.determinantGroupLike R n :
            GeneralLinear.coordinateHopfAlgebra R n) - 1} := by
  refine le_antisymm (definingHopfIdeal_toIdeal_le R n) ?_
  rw [Ideal.span_singleton_le_iff_mem]
  -- The generator is the image of `t - 1` for `t` the degree-one basis vector, whose counit
  -- vanishes, so it lies in the kernel Hopf ideal.
  ...
  have ht : Coalgebra.counit (R := R) (t - 1) = 0 := by
    simp [t]
  have hmem := HopfIdeal.mem_toIdeal.mpr
    (CommHopfAlgCat.mem_kernelHopfIdeal_of_mem_augmentation
...
/-- The named special-to-general-linear inclusion is the kernel inclusion followed by transport
to the named general-linear presentation. -/
theorem groupSchemeιGeneralLinear_def :
    groupSchemeιGeneralLinear R n =
      groupSchemeι R n ≫ eqToHom (GeneralLinear.groupScheme_def R n).symm := by
  unfold groupSchemeιGeneralLinear
  rfl
...
noncomputable def schemePointsMulEquiv :
  ...
  (groupSchemePointMulEquiv (R := R) n A).symm.trans
...
lemma schemePointsMulEquiv_symm_apply (g : Matrix.SpecialLinearGroup (Fin n) A) :
    (schemePointsMulEquiv (R := R) n A).symm g =

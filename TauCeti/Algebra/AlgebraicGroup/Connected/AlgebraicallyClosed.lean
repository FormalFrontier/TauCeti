...
* `TauCeti.geometricallyConnectedCommHopfAlgProperty_iff_isAlgClosed`: geometric connectedness
  is equivalent to connectedness after every algebraically closed field extension.
...
The reverse implication passes from an arbitrary extension `K / k` to `AlgebraicClosure K`.
The induced map on coordinate rings is injective, so the absence of nontrivial idempotents in
the algebraically closed base change descends to the original base change. -/

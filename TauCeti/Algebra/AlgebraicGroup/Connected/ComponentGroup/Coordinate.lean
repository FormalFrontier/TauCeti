...
/-- The idempotent of the identity component of `Spec H` is the idempotent of the connected
component of the augmentation point. -/
private theorem connectedComponentsIdempotent_one (H : FiniteTypeCommHopfAlgCat.{u, u} k) :
    PrimeSpectrum.connectedComponentsIdempotent (1 : ConnectedComponents (PrimeSpectrum H)) =
      PrimeSpectrum.connectedComponentIdempotent
        (Bialgebra.augmentationPoint k H : PrimeSpectrum H) :=
  (congrArg PrimeSpectrum.connectedComponentsIdempotent
    (rationalKernelPoint_one_component H).symm).trans
    (PrimeSpectrum.connectedComponentsIdempotent_mk _)

/-- The kernel of the component morphism lies in the identity-component Hopf ideal: every
idempotent-weighted term of the component coordinate map is a multiple of the complementary
idempotent of the identity component. -/
private theorem kernelHopfIdeal_componentCoordinateHom_le (H : FiniteTypeCommHopfAlgCat.{u, u} k) :
  ...
  have he_one : PrimeSpectrum.connectedComponentsIdempotent
      (1 : ConnectedComponents (PrimeSpectrum H)) = PrimeSpectrum.connectedComponentIdempotent z :=
    connectedComponentsIdempotent_one H
...
theorem kernelHopfIdeal_componentCoordinateHom
    ...
    CommHopfAlgCat.kernelHopfIdeal (componentCoordinateHom H) =
      HopfAlgebra.identityComponentHopfIdeal (k := k) (H := H) := by
  classical
  refine le_antisymm (kernelHopfIdeal_componentCoordinateHom_le H) ?_
  let C := ConnectedComponents (PrimeSpectrum H)
  let z : PrimeSpectrum H := Bialgebra.augmentationPoint k H
  have he_one : PrimeSpectrum.connectedComponentsIdempotent
      (1 : ConnectedComponents (PrimeSpectrum H)) = PrimeSpectrum.connectedComponentIdempotent z :=

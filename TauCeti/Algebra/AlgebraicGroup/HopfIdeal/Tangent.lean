...
* `TauCeti.HopfIdeal.mem_lieSubalgebra_iff`,
  `TauCeti.HopfIdeal.mem_lieSubalgebra_iff_span`: membership means vanishing on the Hopf ideal,
  or just on a chosen set of ideal generators.
...
theorem quotientLieHom_apply (I : HopfIdeal R H)
  ...
  by
    ...
    intro x
    rw [quotientLinearMap, derivationCompLinear_apply, derivationComp_apply]
    simp only [quotientLieHom, derivationCompLieHom_apply, derivationComp_apply]
    rfl
...
theorem derivation_eq_zero_of_mem_span
...
theorem mem_lieSubalgebra_iff_span (I : HopfIdeal R H) {S : Set H}
  ...
  · intro hd x hx
    apply derivation_eq_zero_of_mem_span d (hd := hd) (hx := hI ▸ hx)
    intro y hy

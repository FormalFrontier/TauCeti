...
* `TauCeti.HopfIdeal.quotientLinearMap`, `TauCeti.HopfIdeal.quotientLieHom`: the differential of
  the closed-subgroup inclusion.
* `TauCeti.HopfIdeal.lieSubalgebra`: its image in the ambient Lie algebra.
* `TauCeti.HopfIdeal.mem_lieSubalgebra_iff`,
  `TauCeti.HopfIdeal.mem_lieSubalgebra_iff_of_toIdeal_eq_span`: membership means vanishing on the
...
/-- The coefficient-linear differential of the closed-subgroup inclusion presented by a Hopf
ideal `I`. On coordinate rings it is precomposition with the quotient bialgebra morphism. -/
noncomputable def quotientLinearMap (I : HopfIdeal R H) :
    Derivation R (H ⧸ I.toIdeal)
        (Bialgebra.CounitAlgebra R (H ⧸ I.toIdeal) B) →ₗ[B]
      Derivation R H (Bialgebra.CounitAlgebra R H B) :=
  derivationCompLinear (B := B) (Bialgebra.Quotient.mkBialgHom I.toIdeal)

/-- The coefficient-linear closed-subgroup differential acts by precomposition with the quotient
map. -/
@[simp]
theorem quotientLinearMap_apply (I : HopfIdeal R H)
    (d : Derivation R (H ⧸ I.toIdeal)
      (Bialgebra.CounitAlgebra R (H ⧸ I.toIdeal) B)) :
    quotientLinearMap I d =
      derivationComp (B := B) (Bialgebra.Quotient.mkBialgHom I.toIdeal) d := by
  rw [quotientLinearMap, derivationCompLinear_apply]
...
/-- The Lie and coefficient-linear forms of the closed-subgroup differential have the same
underlying function. -/
theorem quotientLieHom_apply (I : HopfIdeal R H)
    (d : Derivation R (H ⧸ I.toIdeal)
      (Bialgebra.CounitAlgebra R (H ⧸ I.toIdeal) B)) :
    quotientLieHom I d = quotientLinearMap I d :=
  by
    rw [quotientLinearMap_apply, quotientLieHom, derivationCompLieHom_apply]
...
private def derivationVanishingIdeal
  ...
  smul_mem' := by
    rintro a x ⟨hxε, hxd⟩
    constructor
    · -- An ideal's scalar action is multiplication; expose it before applying the counit.
      change Coalgebra.counit (R := R) (a * x) = 0
      simp [hxε]
    · -- The same definitional unfolding exposes the derivation's Leibniz rule.
      change d (a * x) = 0
      rw [d.leibniz]
...
theorem derivation_apply_eq_zero_of_mem_span
  ...
  have hle : Ideal.span S ≤ derivationVanishingIdeal d := by
    rw [Ideal.span_le]
...
theorem mem_lieSubalgebra_iff_of_toIdeal_eq_span (I : HopfIdeal R H) {S : Set H}
    (hI : I.toIdeal = Ideal.span S)
    (d : Derivation R H (Bialgebra.CounitAlgebra R H B)) :
    d ∈ lieSubalgebra (B := B) I ↔ ∀ x ∈ S, d x = 0 := by
  rw [mem_lieSubalgebra_iff]
  constructor
  · intro hd x hx
    apply hd x
    rw [hI]
  ...
  · intro hd x hx
    apply derivation_apply_eq_zero_of_mem_span d (hd := hd) (hx := hI ▸ hx)
    intro y hy

...
not the zero module. -/
instance finiteDimensional_glIrreducible : FiniteDimensional K (glIrreducible K mu) :=
...
theorem lieSpan_glIrreducibleGenerator_eq_top (hmu : IsGlDominantIntegral mu) :
    LieSubmodule.lieSpan K (Matrix (Fin N) (Fin N) K) {glIrreducibleGenerator K mu} = ⊤ := by

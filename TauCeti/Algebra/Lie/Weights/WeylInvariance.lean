...
theorem finrank_weightSpace_neg_sub_zsmul_add {α : Weight K H L} {χ : H → K} {n : ℤ}
  ...
  -- Cut the string off beyond both indices `k` and `-k - n`: a finite-dimensional module has only
  -- finitely many weights, so the weight spaces vanish far enough along any nonzero direction.
  have hcutoff : ∀ β : H → K, β ≠ 0 → ∀ b : ℤ,
      ∃ j : ℤ, b < j ∧ genWeightSpace M (j • β + χ) = ⊥ := by
    intro β hβ b
    obtain ⟨j, hj, hj'⟩ :=
  ...
  obtain ⟨q, hqb, hq⟩ := hcutoff (⇑α) hα B
  obtain ⟨r, hrb, hr⟩ := hcutoff (-⇑α) (neg_ne_zero.2 hα) B
  ...
  -- The string is stable under the three elements of the triple, hence a module over the `sl₂`
  -- subalgebra they generate.
  have hstabh : ∀ m ∈ N.toSubmodule, ⁅h, m⁆ ∈ N.toSubmodule := by
    intro m hm
    rw [LieSubmodule.mem_toSubmodule] at hm ⊢
    rw [hh, ← LieSubalgebra.coe_bracket_of_module]
    exact N.lie_mem hm
  have hstabe : ∀ m ∈ N.toSubmodule, ⁅e, m⁆ ∈ N.toSubmodule := by
    intro m hm
    rw [LieSubmodule.mem_toSubmodule] at hm ⊢
    exact lie_mem_genWeightSpaceChain_of_genWeightSpace_eq_bot_right M (⇑α) χ p q hq he hm
  have hstabf : ∀ m ∈ N.toSubmodule, ⁅f, m⁆ ∈ N.toSubmodule := by
    intro m hm
    rw [LieSubmodule.mem_toSubmodule] at hm ⊢
    exact lie_mem_genWeightSpaceChain_of_genWeightSpace_eq_bot_left M (⇑α) χ p q hp hf hm
  set N' : LieSubmodule K (ht.toLieSubalgebra K) M :=
    ht.lieSubmoduleOfStable N.toSubmodule hstabh hstabe hstabf with hN'def
  have hN'S : N'.toSubmodule = N.toSubmodule := by
    rw [hN'def, IsSl2Triple.lieSubmoduleOfStable_toSubmodule]
  have hA : ∀ v ∈ N'.toSubmodule, A v ∈ N'.toSubmodule := by
    rw [hN'S]
    intro v hv
  ...
  have hop : toEnd K (ht.toLieSubalgebra K) N' ⟨h, ht.h_mem_toLieSubalgebra⟩ = A.restrict hA := by
    ...
    simp only [toEnd_apply_apply, LieSubmodule.coe_bracket, LieSubalgebra.coe_bracket_of_module,
      hAdef, hh]
  ...
  have hNsup : N'.toSubmodule =
    ...
    rw [hN'S, hNdef, genWeightSpaceChain_def]
...
@[simp] theorem finrank_weightSpace_sub_apply_coroot_smul (α : Weight K H L) (χ : H → K) :
  ...
  · -- Neither `χ` nor its reflection is a weight, so both weight spaces vanish.
    ...
    have hbot : ∀ ψ : H → K, ψ (IsKilling.coroot α) = χ (IsKilling.coroot α) ∨
      ...
      have hz' : ψ (IsKilling.coroot α) = (z : K) := by simpa using hz
      rcases hψ with hψ | hψ
      · exact hint z (by rw [← hψ, hz'])
      · exact hint (-z) (by rw [Int.cast_neg, ← hz', hψ, neg_neg])

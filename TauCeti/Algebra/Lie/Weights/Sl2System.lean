...
fixed-point-free involution on the roots. The resulting family satisfies the normalisation on the
nose, in both directions, because an `sl₂` triple stays one when `e` and `f` are exchanged and `h`
is negated, and `(-α)^∨ = -α^∨`.
...
theorem exists_isSl2System : ∃ x : Weight K H L → L, IsSl2System x := by
  ...
  -- representatives for the equivalence relation generated by `α ↦ -α`.
  have hne : ∀ α : Weight K H L, α.IsNonZero → α ≠ -α := by
    intro α hα h
  ...
  set s : Set (Weight K H L) := Set.range fun q : Quotient S ↦ q.out
  have hmem : ∀ α : Weight K H L, α ∈ s ↔ (Quotient.mk S α).out = α := by
    refine fun α ↦ ⟨?_, fun h ↦ ⟨Quotient.mk S α, h⟩⟩
    rintro ⟨q, rfl⟩
    rw [Quotient.out_eq]
  have hmk : ∀ α : Weight K H L, Quotient.mk S (-α) = Quotient.mk S α :=
    fun α ↦ Quotient.sound (Or.inr (neg_neg α).symm)
  have hone : ∀ α : Weight K H L, α ∈ s ∨ (-α) ∈ s := by
    intro α
    rcases Quotient.mk_out (s := S) α with h | h
    · exact Or.inl ((hmem α).mpr h.symm)
    · refine Or.inr ((hmem (-α)).mpr ?_)
      rw [hmk α]
      exact (neg_eq_iff_eq_neg.mpr h).symm
  have hnot : ∀ α : Weight K H L, α.IsNonZero → ¬(α ∈ s ∧ (-α) ∈ s) := by
    rintro α hα ⟨h₁, h₂⟩
    rw [hmem] at h₁ h₂
    refine hne α hα ?_
    calc α = (Quotient.mk S α).out := h₁.symm
      _ = (Quotient.mk S (-α)).out := by rw [hmk α]
      _ = -α := h₂
  ...
  · rw [ite_eq_left hα, ite_eq_left hα.neg]
    ...
    · have h' : (-α) ∈ s := (hone α).resolve_left h

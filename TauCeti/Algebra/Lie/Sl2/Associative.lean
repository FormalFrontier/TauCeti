...
and already the characteristic, instance of Kostant's normal-ordering formula, and it is why the
subring generated by divided powers of root vectors and by binomial coefficients of Cartan
...
* `TauCeti.Sl2.h_mul_pow_of_lie_eq_zsmul`: an eigenvector `X` of `⁅H, ·⁆` with integer eigenvalue
  `c` shifts `H` by `n * c` when `H` is moved past `Xⁿ`. This is the form the root vectors of a
  split semisimple Lie algebra satisfy, so it covers every root vector and not only the two
  members of an `sl₂` triple; `TauCeti.Sl2.h_mul_f_pow` and `TauCeti.Sl2.h_mul_e_pow` are its
  `c = ∓2` specialisations.
...
variable {A : Type*} [Ring A] {H E F X : A} {c : ℤ}

/-- If `X` is an eigenvector of `⁅H, ·⁆` with integer eigenvalue `c`, then moving `H` past `Xⁿ`
translates it by `n * c`.

For the root vectors of a split semisimple Lie algebra this is the relation `⁅h, xₐ⁆ = α(h) • xₐ`
with `α(h)` an integer, so this lemma covers every root vector at once and not only the two
members of an `sl₂` triple. -/
theorem h_mul_pow_of_lie_eq_zsmul (hHX : H * X - X * H = c • X) (n : ℕ) :
    H * X ^ n = X ^ n * H + ((n : ℤ) * c) • X ^ n := by
  have hX : H * X = X * H + c • X := by
    rw [← hHX]; abel
  induction n with
  | zero => simp
  | succ n ih =>
    calc H * X ^ (n + 1) = H * X ^ n * X := by rw [pow_succ, ← mul_assoc]
      _ = (X ^ n * H + ((n : ℤ) * c) • X ^ n) * X := by rw [ih]
      _ = X ^ n * (H * X) + ((n : ℤ) * c) • X ^ (n + 1) := by
          rw [add_mul, mul_assoc, smul_mul_assoc, ← pow_succ]
      _ = X ^ n * (X * H + c • X) + ((n : ℤ) * c) • X ^ (n + 1) := by rw [hX]
      _ = X ^ (n + 1) * H + (c + (n : ℤ) * c) • X ^ (n + 1) := by
          rw [mul_add, ← mul_assoc, ← pow_succ, mul_smul_comm, ← pow_succ, add_assoc, ← add_smul]
      _ = X ^ (n + 1) * H + (((n + 1 : ℕ) : ℤ) * c) • X ^ (n + 1) := by push_cast; ring_nf
...
theorem h_mul_f_pow (hhf : H * F - F * H = -(2 • F)) (n : ℕ) :
  ...
  rw [h_mul_pow_of_lie_eq_zsmul hhf' n, hc, neg_zsmul, natCast_zsmul, mul_sub, nsmul_eq_mul']
  abel
...
theorem h_mul_e_pow (hhe : H * E - E * H = 2 • E) (n : ℕ) :
  ...
  have hc : ((n : ℤ) * 2) = ((2 * n : ℕ) : ℤ) := by push_cast; ring
  rw [h_mul_pow_of_lie_eq_zsmul hhe' n, hc, natCast_zsmul, mul_add, nsmul_eq_mul']
...
theorem h_mul_f_dividedPower (hhf : H * F - F * H = -(2 • F)) (n : ℕ) :
  ...
  rw [dividedPower_def, mul_smul_comm, smul_mul_assoc, h_mul_f_pow hhf]
...
theorem h_mul_e_dividedPower (hhe : H * E - E * H = 2 • E) (n : ℕ) :
  ...
  rw [dividedPower_def, mul_smul_comm, smul_mul_assoc, h_mul_e_pow hhe]
...
reason a `ℤ`-form generated by divided powers is closed under multiplication. -/
...
/-- The canonical map to the universal enveloping algebra turns a Lie bracket into a ring
commutator. -/
theorem ι_mul_sub_mul (x y : L) : ι R x * ι R y - ι R y * ι R x = ι R ⁅x, y⁆ := by
  rw [(ι R).map_lie x y, LieRing.of_associative_ring_bracket]

/-- The image of the bracket relation `⁅e, f⁆ = h` of an `sl₂` triple. -/
theorem ι_lie_e_f (t : IsSl2Triple h e f) : ι R e * ι R f - ι R f * ι R e = ι R h := by
  rw [ι_mul_sub_mul, t.lie_e_f]

/-- The image of the bracket relation `⁅h, e⁆ = 2 • e` of an `sl₂` triple. -/
theorem ι_lie_h_e (t : IsSl2Triple h e f) : ι R h * ι R e - ι R e * ι R h = 2 • ι R e := by
  rw [ι_mul_sub_mul, t.lie_h_e_nsmul, map_nsmul]

/-- The image of the bracket relation `⁅h, f⁆ = -(2 • f)` of an `sl₂` triple. -/
theorem ι_lie_h_f (t : IsSl2Triple h e f) : ι R h * ι R f - ι R f * ι R h = -(2 • ι R f) := by
  rw [ι_mul_sub_mul, t.lie_h_f_nsmul, map_neg, map_nsmul]
...
theorem ι_e_mul_ι_f_pow (t : IsSl2Triple h e f) (n : ℕ) :
  ...
  e_mul_f_pow (ι_lie_e_f R t) (ι_lie_h_f R t) n
...
theorem ι_f_mul_ι_e_pow (t : IsSl2Triple h e f) (n : ℕ) :
  ...
  f_mul_e_pow (ι_lie_e_f R t) (ι_lie_h_e R t) n
...
theorem ι_e_mul_dividedPower_ι_f (t : IsSl2Triple h e f) (n : ℕ) :
  ...
  e_mul_f_dividedPower (ι_lie_e_f ℚ t) (ι_lie_h_f ℚ t) n
...
theorem ι_f_mul_dividedPower_ι_e (t : IsSl2Triple h e f) (n : ℕ) :
  ...
  f_mul_e_dividedPower (ι_lie_e_f ℚ t) (ι_lie_h_e ℚ t) n
...
theorem ι_h_mul_dividedPower_ι_f (t : IsSl2Triple h e f) (n : ℕ) :
  ...
  h_mul_f_dividedPower (ι_lie_h_f ℚ t) n
...
theorem ι_h_mul_dividedPower_ι_e (t : IsSl2Triple h e f) (n : ℕ) :
  ...
  h_mul_e_dividedPower (ι_lie_h_e ℚ t) n

...
so that its coordinates are still directly available, and `TauCeti.Sl2Std.smul_apply` and
`TauCeti.Sl2Std.zero_apply` restate the two pointwise operations that `Pi` lemmas can no longer
reach through it.
...
theorem raise_apply (v : Sl2Std K n) (i : Fin (n + 1)) :
    raise K n v i =
      if h : (i : ℕ) < n then (((i : ℕ) : K) + 1) * v ⟨(i : ℕ) + 1, by omega⟩ else 0 := rfl

/-- The lowering operator in coordinates. -/
theorem lower_apply (v : Sl2Std K n) (i : Fin (n + 1)) :
    lower K n v i =
      if h : 0 < (i : ℕ) then ((n : K) - (i : ℕ) + 1) * v ⟨(i : ℕ) - 1, by omega⟩ else 0 := rfl

/-- The Cartan operator in coordinates. -/
theorem diag_apply (v : Sl2Std K n) (i : Fin (n + 1)) :
    diag K n v i = ((n : K) - 2 * (i : ℕ)) * v i := rfl

/-- Raising after lowering is diagonal, with eigenvalue `(i + 1)(n - i)` at index `i`. Both ends
of the string are covered without a case split, since the coefficient vanishes there. -/
...
theorem lie_raise_lower : ⁅raise K n, lower K n⁆ = diag K n := by
  ...
  change raise K n (lower K n v) i - lower K n (raise K n v) i = diag K n v i
  rw [raise_lower_apply, lower_raise_apply, diag_apply]
  ring

/-- The `sl₂` relation `⁅h, e⁆ = 2e` for the ladder operators. -/
theorem lie_diag_raise : ⁅diag K n, raise K n⁆ = (2 : K) • raise K n := by
  refine LinearMap.ext fun v => funext fun i => ?_
  change diag K n (raise K n v) i - raise K n (diag K n v) i = 2 * raise K n v i
  rw [diag_apply (raise K n v), raise_apply (diag K n v), raise_apply v]
  by_cases h : (i : ℕ) < n
...
theorem lie_diag_lower : ⁅diag K n, lower K n⁆ = -((2 : K) • lower K n) := by
  refine LinearMap.ext fun v => funext fun i => ?_
  change diag K n (lower K n v) i - lower K n (diag K n v) i = -(2 * lower K n v i)
  rw [diag_apply (lower K n v), lower_apply (diag K n v), lower_apply v]
  by_cases h : 0 < (i : ℕ)
  · rw [dif_pos h, dif_pos h, diag_apply]
    rw [Nat.cast_sub h, Nat.cast_one]
    ring
...
theorem basis_apply (i j : Fin (n + 1)) : basis K n i j = if j = i then 1 else 0 :=
...
theorem lie_slFinTwoBasis_zero (v : Sl2Std K n) : ⁅slFinTwoBasis K 0, v⁆ = raise K n v := by
  rw [show ⁅slFinTwoBasis K 0, v⁆ = rep K n (slFinTwoBasis K 0) v from rfl, rep_apply_basis]
  rfl
...
theorem lie_slFinTwoBasis_one (v : Sl2Std K n) : ⁅slFinTwoBasis K 1, v⁆ = lower K n v := by
  rw [show ⁅slFinTwoBasis K 1, v⁆ = rep K n (slFinTwoBasis K 1) v from rfl, rep_apply_basis]
  rfl
...
theorem lie_slFinTwoBasis_two (v : Sl2Std K n) : ⁅slFinTwoBasis K 2, v⁆ = diag K n v := by
  rw [show ⁅slFinTwoBasis K 2, v⁆ = rep K n (slFinTwoBasis K 2) v from rfl, rep_apply_basis]
  rfl
...
theorem raise_pow_apply_eq_zero (k : ℕ) (v : Sl2Std K n) (i : Fin (n + 1))
  ...
  | succ k ih =>
    ...
    by_cases hi : (i : ℕ) < n
    · rw [dif_pos hi, ih ⟨(i : ℕ) + 1, by omega⟩ (by change n < (i : ℕ) + 1 + k; omega), mul_zero]
    · rw [dif_neg hi]
...
theorem eq_smul_basis_zero_of_raise_eq_zero {v : Sl2Std K n} (h : raise K n v = 0) :
  ...
  · rw [if_neg hi, mul_zero]
    ...
    have hi0 : 0 < (i : ℕ) := by
      ...
      · exact h0
    have hlt : (i : ℕ) - 1 < n := by omega
    have h' : raise K n v ⟨(i : ℕ) - 1, by omega⟩ = 0 := by rw [h]; rfl
    have hidx : (⟨(i : ℕ) - 1 + 1, by omega⟩ : Fin (n + 1)) = i := by ext; simp; omega
...
theorem eq_top_of_raise_mem_of_lower_mem (N : Submodule K (Sl2Std K n)) (hN : N ≠ ⊥)
  ...
  have hex : ∃ k, ((raise K n) ^ k) v = 0 := ⟨n + 1, by rw [raise_pow_eq_zero]; rfl⟩
...
instance isIrreducible_toLieSubalgebra :
  ...
  refine (LieSubmodule.toSubmodule_eq_top N).1 (eq_top_of_raise_mem_of_lower_mem _
    (fun h => hN ((LieSubmodule.toSubmodule_eq_bot N).1 h)) (fun w hw => ?_) (fun w hw => ?_))
  · have h1 := N.lie_mem (x := ⟨slFinTwoBasis K 0, he⟩) hw
    rwa [show ⁅(⟨slFinTwoBasis K 0, he⟩ : t.toLieSubalgebra K), w⁆ = raise K n w from
      lie_slFinTwoBasis_zero w] at h1
  · have h1 := N.lie_mem (x := ⟨slFinTwoBasis K 1, hf⟩) hw
    rwa [show ⁅(⟨slFinTwoBasis K 1, hf⟩ : t.toLieSubalgebra K), w⁆ = lower K n w from
      lie_slFinTwoBasis_one w] at h1

/-
Copyright (c) 2026 The Tau Ceti contributors. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
-/
module

public import TauCeti.Algebra.Lie.Sl2.Classification
public import TauCeti.Algebra.Lie.Sl2.CompleteReducibility
public import TauCeti.Algebra.Lie.Submodule.Atom

/-!
# Every finite-dimensional `sl₂`-module is a direct sum of the `V(n)`

`TauCeti/Algebra/Lie/Sl2/CompleteReducibility.lean` proves complete reducibility in its
complement form: every Lie submodule of a finite-dimensional module over a Lie algebra generated
by an `sl₂` triple has a complement. `TauCeti/Algebra/Lie/Sl2/Classification.lean` proves that the
finite-dimensional irreducibles are exactly the standard modules `TauCeti.Sl2Std K n = V(n)`. This
file joins the two into the decomposition itself: such a module is an internal direct sum of
finitely many irreducible Lie submodules, and over `LieAlgebra.SpecialLinear.sl (Fin 2) K` each
summand is a `V(n)`.

The passage from complements to a decomposition is lattice-theoretic and runs entirely in
`LieSubmodule K L M`, which Mathlib already knows to be a complete, modular, compactly generated
lattice, well-founded for `>` once `M` is Noetherian. Complements make it a `ComplementedLattice`,
hence atomistic (`isAtomistic_of_complementedLattice`), hence the supremum of an independent set of
atoms (`exists_sSupIndep_of_sSup_atoms_eq_top`), which well-foundedness makes finite
(`WellFoundedGT.finite_of_sSupIndep`). `TauCeti.isIrreducible_iff_isAtom` reads the atoms as the
irreducible submodules, and `LieSubmodule.iSupIndep_toSubmodule` transports the independence to the
underlying submodules, where `DirectSum.IsInternal` lives.

The decomposition is indexed by `Fin k` rather than by the set of atoms: a `Fin k` index carries
`DecidableEq`, which `DirectSum.IsInternal` needs, and it is what the dimension count below sums
over.

## Main results

* `TauCeti.complementedLattice_lieSubmodule`: the lattice of Lie submodules of a finite-dimensional
  module over a Lie algebra generated by an `sl₂` triple is complemented.
* `TauCeti.exists_isInternal_isIrreducible`: **complete reducibility as a decomposition.** Such a
  module is the internal direct sum of finitely many irreducible Lie submodules.
* `TauCeti.Sl2Std.exists_isInternal_lieModuleEquiv`: **every finite-dimensional `sl₂`-module is
  `⨁ V(nᵢ)`.** Over `LieAlgebra.SpecialLinear.sl (Fin 2) K` the summands are the standard modules.
* `TauCeti.Sl2Std.finrank_eq_sum`: the dimension of the module is `∑ᵢ (nᵢ + 1)`.

## References

This closes the complete-reducibility item of Layer 0 of
`TauCetiRoadmap/RepresentationTheory/LieHighestWeight/README.md`, which asks that every
finite-dimensional `sl₂`-module be exhibited as a direct sum of the `V(nᵢ)`.

* [J. E. Humphreys, *Introduction to Lie Algebras and Representation Theory*][humphreys1972],
  §6.3 and §7.2.
-/

public section

namespace TauCeti

open LieModule Module

section General

variable {K : Type*} [Field K] [CharZero K] [IsAlgClosed K]
variable {L : Type*} [LieRing L] [LieAlgebra K L]
variable {M : Type*} [AddCommGroup M] [Module K M] [LieRingModule L M] [LieModule K L M]
variable {h e f : L} {t : IsSl2Triple h e f}

/-- **The Lie submodules of an `sl₂`-module form a complemented lattice.** This is
`TauCeti.exists_isCompl_of_toLieSubalgebra_eq_top` packaged as the lattice-theoretic hypothesis
that the decomposition below consumes. It is a theorem rather than an instance because the
generation hypothesis `htop` is not a class. -/
theorem complementedLattice_lieSubmodule (htop : t.toLieSubalgebra K = ⊤) [FiniteDimensional K M] :
    ComplementedLattice (LieSubmodule K L M) :=
  ⟨exists_isCompl_of_toLieSubalgebra_eq_top htop⟩

variable (M) in
/-- **Complete reducibility as a decomposition.** A finite-dimensional module over a Lie algebra
generated by an `sl₂` triple, over an algebraically closed field of characteristic zero, is the
internal direct sum of a finite family of irreducible Lie submodules.

The family is produced as an independent family of atoms of `LieSubmodule K L M`: complements make
that lattice complemented, hence atomistic, and it is well-founded for `>` because `M` is
finite-dimensional, which bounds the family. -/
theorem exists_isInternal_isIrreducible (htop : t.toLieSubalgebra K = ⊤)
    [FiniteDimensional K M] :
    ∃ (k : ℕ) (N : Fin k → LieSubmodule K L M),
      DirectSum.IsInternal (fun i ↦ (N i).toSubmodule) ∧
        ∀ i, LieModule.IsIrreducible K L (N i) := by
  have := complementedLattice_lieSubmodule (M := M) htop
  obtain ⟨S, hindep, hSsup, hSatom⟩ :=
    exists_sSupIndep_of_sSup_atoms_eq_top (α := LieSubmodule K L M) sSup_atoms_eq_top
  have : Fintype S := (WellFoundedGT.finite_of_sSupIndep hindep).fintype
  refine ⟨Fintype.card S, fun i ↦ ((Fintype.equivFin S).symm i : LieSubmodule K L M), ?_,
    fun i ↦ (isIrreducible_iff_isAtom _).2 (hSatom ((Fintype.equivFin S).symm i).2)⟩
  refine DirectSum.isInternal_submodule_of_iSupIndep_of_iSup_eq_top ?_ ?_
  · rw [LieSubmodule.iSupIndep_toSubmodule]
    exact ((sSupIndep_iff S).1 hindep).comp (Fintype.equivFin S).symm.injective
  · rw [LieSubmodule.iSup_toSubmodule_eq_top, (Fintype.equivFin S).symm.iSup_comp
      (g := fun x : S ↦ (x : LieSubmodule K L M)), ← sSup_eq_iSup']
    exact hSsup

end General

namespace Sl2Std

open LieAlgebra

variable {K : Type*} [Field K] [CharZero K] [IsAlgClosed K]
variable {M : Type*} [AddCommGroup M] [Module K M]
  [LieRingModule (SpecialLinear.sl (Fin 2) K) M] [LieModule K (SpecialLinear.sl (Fin 2) K) M]
  [FiniteDimensional K M]

variable (K M) in
/-- **Every finite-dimensional `sl₂`-module is `⨁ V(nᵢ)`.** Over an algebraically closed field of
characteristic zero, a finite-dimensional `LieAlgebra.SpecialLinear.sl (Fin 2) K`-module is the
internal direct sum of a finite family of Lie submodules, the `i`-th of which is equivalent to the
standard irreducible `V(nᵢ)`.

This is `TauCeti.exists_isInternal_isIrreducible` for the standard triple, whose summands
`TauCeti.Sl2Std.existsUnique_nonempty_lieModuleEquiv` names. -/
theorem exists_isInternal_lieModuleEquiv :
    ∃ (k : ℕ) (N : Fin k → LieSubmodule K (SpecialLinear.sl (Fin 2) K) M) (n : Fin k → ℕ),
      DirectSum.IsInternal (fun i ↦ (N i).toSubmodule) ∧
        ∀ i, Nonempty (N i ≃ₗ⁅K,SpecialLinear.sl (Fin 2) K⁆ Sl2Std K (n i)) := by
  obtain ⟨k, N, hint, hirr⟩ := exists_isInternal_isIrreducible M
    (toLieSubalgebra_isSl2Triple_single_eq_top (R := K) (zero_ne_one : (0 : Fin 2) ≠ 1))
  have hex : ∀ i, ∃ n : ℕ,
      Nonempty (N i ≃ₗ⁅K,SpecialLinear.sl (Fin 2) K⁆ Sl2Std K n) := fun i ↦
    have := hirr i
    (existsUnique_nonempty_lieModuleEquiv (M := N i)).exists
  choose n hn using hex
  exact ⟨k, N, n, hint, hn⟩

omit [CharZero K] [IsAlgClosed K] [LieModule K (SpecialLinear.sl (Fin 2) K) M] in
/-- **The dimension of a finite-dimensional `sl₂`-module from its decomposition.** If the module is
the internal direct sum of Lie submodules equivalent to `V(n₀), …, V(n_{k-1})`, its dimension is
`∑ᵢ (nᵢ + 1)`, each `V(nᵢ)` having dimension `nᵢ + 1`. -/
theorem finrank_eq_sum {k : ℕ} {N : Fin k → LieSubmodule K (SpecialLinear.sl (Fin 2) K) M}
    {n : Fin k → ℕ} (hint : DirectSum.IsInternal fun i ↦ (N i).toSubmodule)
    (hn : ∀ i, Nonempty (N i ≃ₗ⁅K,SpecialLinear.sl (Fin 2) K⁆ Sl2Std K (n i))) :
    finrank K M = ∑ i, (n i + 1) := by
  have hequiv :=
    LinearEquiv.ofBijective (DirectSum.coeLinearMap fun i ↦ (N i).toSubmodule) hint
  rw [← hequiv.finrank_eq, Module.finrank_directSum]
  refine Finset.sum_congr rfl fun i _ ↦ ?_
  change finrank K (N i) = n i + 1
  rw [(hn i).some.toLinearEquiv.finrank_eq, finrank_eq]

end Sl2Std

end TauCeti

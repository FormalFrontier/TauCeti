...
* `TauCeti.Sl2Std.rankOneTorusMatrix`: the matrix equation `diag(s, s⁻¹)`.
...
torus. -/
noncomputable abbrev rankOneGroupScheme :
...
noncomputable abbrev rankOneGroupSchemeι :
...
noncomputable abbrev rankOneRootSubgroup (i : Fin 2) :
...
noncomputable abbrev rankOneWeightTorus :
...
noncomputable abbrev rankOneTorusMatrix {A : Type*} [CommRing A] :

...
and — once `L` is generated by the triple and the base field is algebraically closed — the
eigenspaces **span** `M`, so `h` acts diagonalizably.
...
`h` is a Lie submodule (`e`, `f` and `h` shift the eigenvalue by `2`, `-2` and `0`), so complete
reducibility, `TauCeti.exists_isCompl_of_toLieSubalgebra_eq_top`, supplies a complementary Lie
submodule; over an algebraically closed field a nonzero complement would contain an eigenvector of
`h`, which already lies in the sum. Hence the complement vanishes and the eigenspaces exhaust `M`.
Only here are `IsAlgClosed` and the hypothesis that the triple generates `L` needed.
...
* `TauCeti.iSup_eigenspace_toEnd_eq_top`: **diagonalizability.** Over an algebraically closed field
  of characteristic zero, the eigenspaces of the Cartan element of a generating `sl₂` triple span
  the module.
...
under the subalgebra they generate; the hypothesis `htop` says that subalgebra is all of `L`.
...
def eigenspaceSup (htop : t.toLieSubalgebra K = ⊤) : LieSubmodule K L M where
  ...
  lie_mem := by
    ...
    intro x y hy
    have hx : x ∈ t.toLieSubalgebra K := by rw [htop]; trivial
...
@[simp]
theorem eigenspaceSup_toSubmodule (htop : t.toLieSubalgebra K = ⊤) :
    (eigenspaceSup t htop).toSubmodule = ⨆ ν : K, (toEnd K L M h).eigenspace ν := by
  simp only [eigenspaceSup]

theorem mem_eigenspaceSup_of_lie_h_eq_smul (htop : t.toLieSubalgebra K = ⊤)
    (hv : ⁅h, v⁆ = μ • v) : v ∈ eigenspaceSup t htop := by
  rw [← LieSubmodule.mem_toSubmodule, eigenspaceSup_toSubmodule]
...
zero, the eigenspaces of the Cartan element of an `sl₂` triple generating `L` span every
finite-dimensional `L`-module.

The eigenspaces span a Lie submodule (`TauCeti.eigenspaceSup`), which by complete reducibility
(`TauCeti.exists_isCompl_of_toLieSubalgebra_eq_top`) has a complement. A nonzero complement is
itself a finite-dimensional module over an algebraically closed field, so `h` has an eigenvector in
it — but every eigenvector already lies in the span, and the two meet only in `0`. So the
complement is trivial. -/
theorem iSup_eigenspace_toEnd_eq_top (htop : t.toLieSubalgebra K = ⊤) :
    ⨆ μ : K, (toEnd K L M h).eigenspace μ = ⊤ := by
  obtain ⟨N, hN⟩ :=
    exists_isCompl_of_toLieSubalgebra_eq_top (M := M) htop (eigenspaceSup t htop)
  have hNbot : N = ⊥ := by
    by_contra hne
    have : Nontrivial N := (LieSubmodule.nontrivial_iff_ne_bot K L M).2 hne
    obtain ⟨μ, hμ⟩ :=
      LieModule.IsTriangularizable.exists_hasEigenvalue (R := K) (L := L) (M := ↥N) h
    obtain ⟨v, hv, hv0⟩ := hμ.exists_hasEigenvector
    have hvM : ⁅h, (v : M)⁆ = μ • (v : M) := by
      have hv' : ⁅h, v⁆ = μ • v := by simpa using Module.End.mem_eigenspace_iff.1 hv
      simpa using congrArg (fun w : ↥N ↦ (w : M)) hv'
    have hmem : (v : M) ∈ eigenspaceSup t htop ⊓ N :=
      ⟨mem_eigenspaceSup_of_lie_h_eq_smul t htop hvM, v.2⟩
    rw [hN.inf_eq_bot] at hmem
    exact hv0 (Subtype.ext (by simpa using (LieSubmodule.mem_bot _).1 hmem))
  have hsup : eigenspaceSup (M := M) t htop = ⊤ := by
    have := hN.sup_eq_top
    rwa [hNbot, sup_bot_eq] at this
  refine eq_top_iff.2 fun x _ ↦ ?_
  have hx : x ∈ eigenspaceSup (M := M) t htop := by rw [hsup]; trivial
  rwa [← LieSubmodule.mem_toSubmodule, eigenspaceSup_toSubmodule] at hx
...
theorem iSup_eigenspace_toEnd_intCast_eq_top (htop : t.toLieSubalgebra K = ⊤) :
    ⨆ z : ℤ, (toEnd K L M h).eigenspace (z : K) = ⊤ := by
  classical
  rw [← iSup_eigenspace_toEnd_eq_top (t := t) htop]

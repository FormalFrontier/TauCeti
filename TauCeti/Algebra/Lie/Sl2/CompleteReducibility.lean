...
submodule has a complement (`TauCeti.exists_isCompl_of_isSl2Triple`). With the classification of the
irreducibles already in `TauCeti/Algebra/Lie/Sl2/Classification.lean`, this says that every such
module is `⨁ V(nᵢ)`.
...
* **Step one, the invariant vector** (`TauCeti.exists_invariant_notMem`). If `L` carries `M` into a
  ...
  nontrivially on `N` — otherwise the action would compose to zero, and `L`, being perfect, would
  act trivially on `M` — so the Casimir operator is injective on `N` while its range lies in `N`; it
...
* **Step two, the invariant projection** (`TauCeti.exists_isCompl_of_isSl2Triple`). For an arbitrary
  submodule `N ≤ M`, apply step one inside the module `M →ₗ[K] M`, to the submodule of
...
* `TauCeti.lie_eq_zero_of_lie_lie_eq_zero`: a Lie algebra generated by an `sl₂` triple is perfect,
  in the form that an action composing to zero is zero.
* `TauCeti.exists_invariant_notMem`: an invariant vector outside a proper submodule containing
  `⁅L, M⁆`.
* `TauCeti.exists_isCompl_of_isSl2Triple`: **complete reducibility.** Every Lie submodule of a
  finite-dimensional module over a Lie algebra generated by an `sl₂` triple has a complement.
...
complete reducibility is `TauCeti.exists_isCompl_of_isSl2Triple`.
...
/-! ### Perfectness of the algebra generated by a triple -/
  ...
  | succ d ih =>
    ...
    · -- `N` is irreducible: the Casimir operator supplies the invariant vector.
      ...
      have hactN : ∃ (x : L) (z : N), ⁅x, z⁆ ≠ 0 := by
        ...
        simpa using hval
      have hinjN := injective_sl2Casimir (M := N) (t := t) htop hactN
...
theorem exists_isCompl_of_isSl2Triple (htop : t.toLieSubalgebra K = ⊤) [FiniteDimensional K M]
    (N : LieSubmodule K L M) : ∃ N' : LieSubmodule K L M, IsCompl N N' := by
  rcases eq_or_ne N ⊤ with rfl | hNtop
  ...
  -- Step one, inside the endomorphism module.
  ...
  -- Rescale to an equivariant projection of `M` onto `N`.
  obtain ⟨ψ, hψmem, hψid, hψlie⟩ : ∃ ψ : M →ₗ[K] M, (∀ m, ψ m ∈ N) ∧ (∀ n ∈ N, ψ n = n) ∧
    ...
    · have hzero : ⁅x, (φ : M →ₗ[K] M)⁆ = 0 := by
      ...
      simp [← happ]
  -- The kernel of that projection is the complement.
  refine ⟨{ carrier := {m : M | ψ m = 0}
            zero_mem' := by simp
            add_mem' := fun {a b} ha hb ↦ by
              simp only [Set.mem_ofPred_eq] at ha hb ⊢
              rw [map_add, ha, hb, add_zero]
            smul_mem' := fun a m hm ↦ by
              simp only [Set.mem_ofPred_eq] at hm ⊢
              rw [map_smul, hm, smul_zero]
            lie_mem := fun {x m} hm ↦ by
              simp only [Set.mem_ofPred_eq] at hm ⊢
              rw [hψlie, hm, lie_zero] }, ?_, ?_⟩
  · rw [disjoint_iff, eq_bot_iff]
    intro m hm
    rw [LieSubmodule.mem_inf] at hm
    have hker : ψ m = 0 := hm.2
    rw [LieSubmodule.mem_bot, ← hψid m hm.1, hker]
  · rw [codisjoint_iff, eq_top_iff]
    intro m _
    refine (LieSubmodule.mem_sup _ _ _).2 ⟨ψ m, hψmem m, m - ψ m, ?_, by abel⟩
    have : ψ (m - ψ m) = 0 := by rw [map_sub, hψid _ (hψmem m), sub_self]
    exact this
...
`TauCeti.exists_isCompl_of_isSl2Triple` for the standard triple, which generates `sl (Fin 2) K` by

...
* `TauCeti.UniversalEnvelopingAlgebra.representation_eq_smul_of_mem_center_of_lieSpan_eq_top`: a
  central element acting by a scalar on a cyclic generator acts by that scalar everywhere.
...
/-- A central element of the universal enveloping algebra that acts by a scalar on a cyclic
Lie-module generator acts by that scalar on the whole module. -/
theorem representation_eq_smul_of_mem_center_of_lieSpan_eq_top
    {u : U} (hu : u ∈ Subalgebra.center R U) {v : M} {c : R}
    (hv : representation R L M u v = c • v)
    (hgen : LieSubmodule.lieSpan R L {v} = ⊤) (m : M) :
  ...
  have htop : f.ker = ⊤ := by
    rw [← top_le_iff, ← hgen]
    exact LieSubmodule.lieSpan_le.mpr (by simpa using hvker)
  have hm : m ∈ f.ker := by rw [htop]; exact LieSubmodule.mem_top m
  rw [LieModuleHom.mem_ker] at hm
  simpa [f, sub_eq_zero] using hm

...
* `TauCeti.integralDividedPower`: a divided-power operator restricted to an invariant additive
  subgroup.
* `TauCeti.baseChangeExp`: the finite divided-power exponential on `R ⊗[ℤ] M`.
...
* `TauCeti.UniversalEnvelopingAlgebra.baseChangeKostantExpHom`: the specialization to a
  Kostant-stable lattice in a representation.
...
noncomputable def integralDividedPower (x : Module.End ℚ V) (M : AddSubgroup V)
    (hM : ∀ n, ∀ v ∈ M, Associative.dividedPower n x v ∈ M) (n : ℕ) : Module.End ℤ M where
  toFun v := ⟨Associative.dividedPower n x v, hM n v v.2⟩
  map_add' a b := by
    apply Subtype.ext
...
noncomputable def baseChangeExp (x : Module.End ℚ V) (M : AddSubgroup V)
  ...
  ∑ n ∈ range (nilpotencyClass x),
    t ^ n • (integralDividedPower x M hM n).baseChange R
...
private theorem baseChange_integralDividedPower_eq_zero_of_le
    ...
    (hM : ∀ n, ∀ v ∈ M, Associative.dividedPower n x v ∈ M)
    {k n : ℕ} (hk : x ^ k = 0) (hkn : k ≤ n) :
    (integralDividedPower x M hM n).baseChange R = 0 := by
  rw [integralDividedPower_eq_zero_of_le x M hM hk hkn, LinearMap.baseChange_zero]
...
noncomputable def baseChangeExpLinearEquiv (x : Module.End ℚ V) (M : AddSubgroup V)
  ...
  toLinearMap := baseChangeExp x M hM t
  invFun := baseChangeExp x M hM (-t)
  left_inv v := by
...
divided-power-stable lattice by the integral nilpotent exponential. -/
...
/-! ## Root subgroup actions from a Kostant form -/
...
/-- Every designated root vector's divided powers preserve a subgroup stable under the whole
Kostant form. -/

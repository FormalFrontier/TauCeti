...
When `CM` is the Cartan matrix of a root datum, this subring coincides with the canonical Kostant
integral `ℤ`-form (generated by divided powers of all root vectors), providing the explicit
presentation-level integral enveloping algebra used in the Chevalley--Demazure construction.
For an arbitrary integer matrix, it defines the Serre-generator candidate subring.
...
/-- The **Kostant integral subring of the Serre presentation**: the subring generated by every
divided power of a simple raising or lowering generator and every binomial coefficient in a
Cartan generator. When `CM` is a Cartan matrix, this generates the canonical Kostant `ℤ`-form. -/

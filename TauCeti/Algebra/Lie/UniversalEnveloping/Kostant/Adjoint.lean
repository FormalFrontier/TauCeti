  ...
  have hle : rootCorootSpan x ≤ (rootCorootSpan x).comap f := by
    ...
    · rw [Submodule.mem_comap, hfapply]
      refine hx.mem_chevalleyLieLattice_iff.1
        (hx.ringChoose_ad_coroot_apply_mem α n
          (chainBotCoeff (α : H → ℚ) β - chainTopCoeff (α : H → ℚ) β)
          (hx.rootVector_mem_chevalleyLieLattice β) ?_)
      rw [LieAlgebra.ad_apply, hx.toIsSl2System.lie_coroot β α, apply_coroot_eq_cast α β]
    · rw [Submodule.mem_comap, hfapply]
      refine hx.mem_chevalleyLieLattice_iff.1
        (hx.ringChoose_ad_coroot_apply_mem α n 0 (hx.coroot_mem_chevalleyLieLattice β) ?_)
      rw [LieAlgebra.ad_apply]
      have : ⁅(coroot α : L), (coroot β : L)⁆ = 0 := by
        have := trivial_lie_zero (↥H) (↥H) (coroot α) (coroot β)
        simpa using congrArg (Subtype.val : H → L) this
      rw [this]
      simp
...
theorem chevalleyKostantForm_le_stabilizer :
  ...
  TauCeti.UniversalEnvelopingAlgebra.kostantForm_le_stabilizer _ _ _ _
    ...
    (fun α n _ hy => hx.ringChoose_representation_mem α n hy)

...
* `TauCeti.UniversalEnvelopingAlgebra.ringChoose_ι_add_intCast_mem_kostantForm`: integer
  translates of designated Cartan binomial coefficients belong to the Kostant form.
* `TauCeti.UniversalEnvelopingAlgebra.ringChoose_ι_sub_intCast_mem_kostantForm`: subtracting an
  integer from a designated Cartan argument also preserves membership in the Kostant form.
...
* `TauCeti.UniversalEnvelopingAlgebra.dividedPower_mem_map_kostantForm`: divided powers of mapped
  root vectors lie in the mapped Kostant form.
...
theorem ringChoose_ι_add_intCast_mem_kostantForm (e : ι → L) (h : κ → L) (i : κ)
    (z : ℤ) (n : ℕ) :
    Ring.choose (_root_.UniversalEnvelopingAlgebra.ι ℚ (h i) +
      (z : _root_.UniversalEnvelopingAlgebra ℚ L)) n ∈ kostantForm e h := by
  exact Ring.choose_add_intCast_mem (fun k _ => ringChoose_mem_kostantForm e h i k) z
...
theorem ringChoose_ι_sub_intCast_mem_kostantForm (e : ι → L) (h : κ → L) (i : κ)
    (z : ℤ) (n : ℕ) :
    Ring.choose (_root_.UniversalEnvelopingAlgebra.ι ℚ (h i) -
      (z : _root_.UniversalEnvelopingAlgebra ℚ L)) n ∈ kostantForm e h := by
...
theorem span_kostantForm_eq_top (e : ι → L) (h : κ → L)
  ...
  rw [← Subring.toSubmodule_adjoin_coe (S := ℚ), hadjoin, _root_.Algebra.top_toSubmodule]

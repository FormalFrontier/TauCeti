...
because multiplication by arbitrary rational scalars would destroy the integral lattice. A scoped
...
It is `scoped` rather than `local` so that the downstream files which reason about the Cartan
generators of `kostantForm` elaborate `Ring.choose` through the very same instance. -/
noncomputable scoped instance moduleNNRat :

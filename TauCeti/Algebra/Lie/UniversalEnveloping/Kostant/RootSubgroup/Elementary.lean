...
is the elementary Chevalley group of the lattice `M` over `A`. For a Chevalley system on a
semisimple Lie algebra and a finite free admissible lattice this is the Chevalley group of Carter,
*Simple Groups of Lie Type*, §4.4; over a field it is the whole group of points of the
Chevalley--Demazure group scheme, and that identification is a separate theorem which this file
does not claim.
...
@[expose] noncomputable def kostantRootSubgroupParam (A : CommAlgCat.{w} ℤ) :
...
@[expose] noncomputable def kostantElementarySubgroup (A : CommAlgCat.{w} ℤ) :
...
@[expose] noncomputable def kostantElementaryMap {A B : CommAlgCat.{w} ℤ} (φ : A ⟶ B) :
    kostantElementarySubgroup e h ρ M hM hnil A →*
...
@[expose] noncomputable def kostantElementaryFunctor :

...
* `TauCeti.UniversalEnvelopingAlgebra.unitsMap_toMatrixAlgEquiv_kostantRootSubgroupParam`: in basis
  coordinates, a parametrized root-subgroup element is its represented root-subgroup matrix.
...
@[simp]
theorem unitsMap_toMatrixAlgEquiv_kostantRootSubgroupParam
    {n : ℕ} (b : Module.Basis (Fin n) ℤ M)
    (A : Type w) [CommRing A]

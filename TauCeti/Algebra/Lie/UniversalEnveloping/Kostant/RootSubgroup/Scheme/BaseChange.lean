...
`GLₙ` over `A` itself, cut out by the Hopf ideal `kostantGeneratedGeneralLinearIdeal`, and each
...
* `TauCeti.UniversalEnvelopingAlgebra.kostantGeneratedGeneralLinearIdeal`: the Hopf ideal of the
  general-linear coordinate algebra *over `A`* which presents the base-changed carrier, with
  `mem_kostantGeneratedGeneralLinearIdeal_iff` and
  `mem_kostantGeneratedGeneralLinearIdeal_of_mem` for its members.
* `TauCeti.UniversalEnvelopingAlgebra.kostantGeneratedGeneralLinearIso`: that presentation is the
  base change of the carrier over `ℤ`.
* `TauCeti.UniversalEnvelopingAlgebra.kostantRootSubgroupGeneralLinearCoordinateMap` and
  `TauCeti.UniversalEnvelopingAlgebra.kostantRootSubgroupGeneralLinearGeneratedCoordinateMap`: the
  base-changed root subgroups over `A`, before and after factoring through the carrier.
* `TauCeti.UniversalEnvelopingAlgebra.kostantGeneratedGeneralLinearIdeal_le_commonKernelHopfIdeal`:
  over `A` too, the carrier generated by the root subgroups lies inside the base-changed carrier.
...
noncomputable def kostantGeneratedGeneralLinearIdeal :
...
theorem mem_kostantGeneratedGeneralLinearIdeal_iff
    {x : GeneralLinear.coordinateHopfAlgebra A n} :
    x ∈ kostantGeneratedGeneralLinearIdeal e h ρ M hM hnil b A ↔
      (GeneralLinear.coordinateHopfAlgebraBaseChangeIso ℤ A n).symm.hom.hom x ∈
        kostantGeneratedBaseChangeIdeal e h ρ M hM hnil b A :=
  HopfIdeal.mem_comap

/-- Every scalar multiple of a defining equation of the Chevalley carrier over `ℤ` is a defining
equation over `A`: the carrier over `A` is cut out by the base-changed equations, not by new
ones. -/
theorem mem_kostantGeneratedGeneralLinearIdeal_of_mem (s : A)
    ...
    (GeneralLinear.coordinateHopfAlgebraBaseChangeIso ℤ A n).hom.hom (s ⊗ₜ[ℤ] y) ∈
      kostantGeneratedGeneralLinearIdeal e h ρ M hM hnil b A := by
  rw [mem_kostantGeneratedGeneralLinearIdeal_iff]
  have hround : (GeneralLinear.coordinateHopfAlgebraBaseChangeIso ℤ A n).symm.hom.hom
      ((GeneralLinear.coordinateHopfAlgebraBaseChangeIso ℤ A n).hom.hom (s ⊗ₜ[ℤ] y)) =
        s ⊗ₜ[ℤ] y := by
    change ((GeneralLinear.coordinateHopfAlgebraBaseChangeIso ℤ A n).hom ≫
      (GeneralLinear.coordinateHopfAlgebraBaseChangeIso ℤ A n).inv).hom (s ⊗ₜ[ℤ] y) = _
    rw [Iso.hom_inv_id]
    rfl
  rw [hround, kostantGeneratedBaseChangeIdeal_def]
...
noncomputable def kostantGeneratedGeneralLinearPresentationIso :
    CommHopfAlgCat.quotient (GeneralLinear.coordinateHopfAlgebra A n)
        (kostantGeneratedGeneralLinearIdeal e h ρ M hM hnil b A) ≅
      CommHopfAlgCat.quotient
        (CommHopfAlgCat.baseChange (K := A) (GeneralLinear.coordinateHopfAlgebra ℤ n))
        (kostantGeneratedBaseChangeIdeal e h ρ M hM hnil b A) :=
...
@[simp]
theorem mkQuotient_comp_kostantGeneratedGeneralLinearPresentationIso_hom :
    CommHopfAlgCat.mkQuotient (GeneralLinear.coordinateHopfAlgebra A n)
          (kostantGeneratedGeneralLinearIdeal e h ρ M hM hnil b A) ≫
        (kostantGeneratedGeneralLinearPresentationIso e h ρ M hM hnil b A).hom =
...
carrier over `ℤ`. -/
noncomputable def kostantGeneratedGeneralLinearIso :
    CommHopfAlgCat.quotient (GeneralLinear.coordinateHopfAlgebra A n)
        (kostantGeneratedGeneralLinearIdeal e h ρ M hM hnil b A) ≅
      CommHopfAlgCat.baseChange (K := A)
        (CommHopfAlgCat.quotient (GeneralLinear.coordinateHopfAlgebra ℤ n)
          (kostantGeneratedDefiningIdeal e h ρ M hM hnil b)) :=
  kostantGeneratedGeneralLinearPresentationIso e h ρ M hM hnil b A ≪≫
...
@[simp]
theorem mkQuotient_comp_kostantGeneratedGeneralLinearIso_hom :
    CommHopfAlgCat.mkQuotient (GeneralLinear.coordinateHopfAlgebra A n)
          (kostantGeneratedGeneralLinearIdeal e h ρ M hM hnil b A) ≫
        (kostantGeneratedGeneralLinearIso e h ρ M hM hnil b A).hom =
      (GeneralLinear.coordinateHopfAlgebraBaseChangeIso ℤ A n).inv ≫
        CommHopfAlgCat.baseChangeMap
          (CommHopfAlgCat.mkQuotient (GeneralLinear.coordinateHopfAlgebra ℤ n)
            (kostantGeneratedDefiningIdeal e h ρ M hM hnil b)) := by
  rw [kostantGeneratedGeneralLinearIso, Iso.trans_hom, ← Category.assoc,
    mkQuotient_comp_kostantGeneratedGeneralLinearPresentationIso_hom, Category.assoc,
    mkQuotient_comp_kostantGeneratedBaseChangeIso_hom]
...
noncomputable def kostantRootSubgroupGeneralLinearCoordinateMap (i : I) :
...
noncomputable def kostantRootSubgroupGeneralLinearGeneratedCoordinateMap (i : I) :
    CommHopfAlgCat.quotient (GeneralLinear.coordinateHopfAlgebra A n)
        (kostantGeneratedGeneralLinearIdeal e h ρ M hM hnil b A) ⟶
...
@[simp]
theorem mkQuotient_comp_kostantRootSubgroupGeneralLinearGeneratedCoordinateMap (i : I) :
    CommHopfAlgCat.mkQuotient (GeneralLinear.coordinateHopfAlgebra A n)
          (kostantGeneratedGeneralLinearIdeal e h ρ M hM hnil b A) ≫
        kostantRootSubgroupGeneralLinearGeneratedCoordinateMap e h ρ M hM hnil b A i =
      kostantRootSubgroupGeneralLinearCoordinateMap e h ρ M hM hnil b A i := by
  rw [kostantRootSubgroupGeneralLinearGeneratedCoordinateMap, ← Category.assoc,
    mkQuotient_comp_kostantGeneratedGeneralLinearPresentationIso_hom, Category.assoc,
    ← Category.assoc (CommHopfAlgCat.mkQuotient _ _),
    mkQuotient_comp_kostantRootSubgroupBaseChangeCoordinateMap,
    kostantRootSubgroupGeneralLinearCoordinateMap]
...
theorem kostantGeneratedGeneralLinearIdeal_toIdeal_le_ker (i : I) :
    (kostantGeneratedGeneralLinearIdeal e h ρ M hM hnil b A).toIdeal ≤
      RingHom.ker
        (kostantRootSubgroupGeneralLinearCoordinateMap e h ρ M hM hnil b A
          i).hom.toAlgHom.toRingHom := by
  intro x hx
  have hbase := kostantGeneratedBaseChangeIdeal_toIdeal_le_ker e h ρ M hM hnil b A i
    (HopfIdeal.mem_toIdeal.mpr (HopfIdeal.mem_comap.mp hx))
  rw [RingHom.mem_ker] at hbase ⊢
  rw [kostantRootSubgroupGeneralLinearCoordinateMap]
  change ((AdditiveGroup.coordinateHopfAlgebraBaseChangeIso ℤ A).hom.hom.toAlgHom.toRingHom)
      (((CommHopfAlgCat.baseChangeMap
            (kostantRootSubgroupCoordinateMap e h ρ M hM i (hnil i) b)).hom.toAlgHom.toRingHom)
        ((GeneralLinear.coordinateHopfAlgebraBaseChangeIso ℤ A n).symm.hom.hom x)) = 0
  rw [hbase, map_zero]
...
theorem kostantGeneratedGeneralLinearIdeal_le_commonKernelHopfIdeal :
    kostantGeneratedGeneralLinearIdeal e h ρ M hM hnil b A ≤
      CommHopfAlgCat.commonKernelHopfIdeal
        (kostantRootSubgroupGeneralLinearCoordinateMap e h ρ M hM hnil b A) :=
  (CommHopfAlgCat.le_commonKernelHopfIdeal_iff _ _).mpr
    (kostantGeneratedGeneralLinearIdeal_toIdeal_le_ker e h ρ M hM hnil b A)

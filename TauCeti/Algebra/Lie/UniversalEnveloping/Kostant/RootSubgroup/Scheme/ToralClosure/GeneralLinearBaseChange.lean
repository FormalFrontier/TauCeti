...
* `hom_mkQuotient_comp_kostantWeightTorusToralBaseChangePresentationCoordinateMap`: its
  underlying bialgebra morphism is the direct weight-torus morphism over `A`.
...
/-- The underlying bialgebra morphism of the factored transported weight-torus map is the direct
diagonal-representation morphism constructed over `A`. -/
theorem hom_mkQuotient_comp_kostantWeightTorusToralBaseChangePresentationCoordinateMap :
    (CommHopfAlgCat.mkQuotient (GeneralLinear.coordinateHopfAlgebra A n)
          (kostantToralBaseChangePresentationIdeal e h ρ M hM hnil b wt A) ≫
        kostantWeightTorusToralBaseChangePresentationCoordinateMap
          e h ρ M hM hnil b wt A).hom =
      GeneralLinear.weightTorusCoordinateBialgHom (S := A) wt := by
  rw [mkQuotient_comp_kostantWeightTorusToralBaseChangePresentationCoordinateMap,
...
/-- The defining ideal of the transported toral carrier is killed by the direct weight-torus
bialgebra morphism constructed over `A`. -/
theorem kostantToralBaseChangePresentationIdeal_toIdeal_le_direct_torus_ker :
    (kostantToralBaseChangePresentationIdeal e h ρ M hM hnil b wt A).toIdeal ≤
      RingHom.ker
        (GeneralLinear.weightTorusCoordinateBialgHom (S := A)
          wt).toAlgHom.toRingHom := by
  rw [← GeneralLinear.hom_weightTorusBaseChangeCoordinateMap ℤ A wt]

...
theorem kostantTorusSubsystemGroupScheme_hom_ext {Y : _root_.CommHopfAlgCat.{0} ℤ}
  ...
  have hroot' : ∀ (i : I) (hi : i ∈ S), (hff.preimage φ).unop ≫
      kostantRootSubgroupTorusSubsystemCoordinateMap e h ρ M hM b wt S hnilS hi =
      (hff.preimage ψ).unop ≫
        kostantRootSubgroupTorusSubsystemCoordinateMap e h ρ M hM b wt S hnilS hi := by
    intro i hi
    have hmap : (hopfSpec (CommRingCat.of ℤ)).map
          ((kostantRootSubgroupTorusSubsystemCoordinateMap
              e h ρ M hM b wt S hnilS hi).op ≫ hff.preimage φ) =
        (hopfSpec (CommRingCat.of ℤ)).map
          ((kostantRootSubgroupTorusSubsystemCoordinateMap
              e h ρ M hM b wt S hnilS hi).op ≫ hff.preimage ψ) := by
  ...
  have htorus' : (hff.preimage φ).unop ≫
      kostantWeightTorusTorusSubsystemCoordinateMap e h ρ M hM b wt S hnilS =
      (hff.preimage ψ).unop ≫
        kostantWeightTorusTorusSubsystemCoordinateMap e h ρ M hM b wt S hnilS := by
    have hmap : (hopfSpec (CommRingCat.of ℤ)).map
          ((kostantWeightTorusTorusSubsystemCoordinateMap
              e h ρ M hM b wt S hnilS).op ≫ hff.preimage φ) =
        (hopfSpec (CommRingCat.of ℤ)).map
          ((kostantWeightTorusTorusSubsystemCoordinateMap
              e h ρ M hM b wt S hnilS).op ≫ hff.preimage ψ) := by
      rw [Functor.map_comp, Functor.map_comp, hff.map_preimage, hff.map_preimage]
    ...
    have hop := hff.map_injective hmap
    simpa using congrArg Quiver.Hom.unop hop
  have hpre := kostantTorusSubsystemCoordinate_hom_ext e h ρ M hM b wt S hnilS

...
private noncomputable abbrev kostantTorusSubsystemGeneratorCodomain
    (S : Set I) (j : Sum S Unit) : _root_.CommHopfAlgCat ℤ :=
  match j with
...
private noncomputable def kostantTorusSubsystemGeneratorMap (S : Set I)
    ...
    GeneralLinear.coordinateHopfAlgebra ℤ n ⟶
      kostantTorusSubsystemGeneratorCodomain (κ := κ) S j :=
  match j with
  | .inl i => kostantRootSubgroupCoordinateMap e h ρ M hM i.1 (hnilS i) b
  | .inr _ => GeneralLinear.weightTorusCoordinateMap wt

@[simp]
private theorem kostantTorusSubsystemGeneratorMap_inl (S : Set I)
    (hnilS : ∀ i : S, IsNilpotent
      (ρ (_root_.UniversalEnvelopingAlgebra.ι ℚ (e i.1)))) (i : S) :
    kostantTorusSubsystemGeneratorMap e h ρ M hM b wt S hnilS (.inl i) =
      kostantRootSubgroupCoordinateMap e h ρ M hM i.1 (hnilS i) b := by
  rfl

@[simp]
private theorem kostantTorusSubsystemGeneratorMap_inr (S : Set I)
    (hnilS : ∀ i : S, IsNilpotent
...
theorem le_kostantTorusSubsystemDefiningIdeal_iff (S : Set I)
  ...
  · intro hJ
    refine ⟨?_, ?_⟩
    · intro i hi
      simpa only [kostantTorusSubsystemGeneratorMap_inl] using
        hJ (Sum.inl (β := Unit) (⟨i, hi⟩ : S))
    · simpa only [kostantTorusSubsystemGeneratorMap_inr] using
        hJ (Sum.inr (α := S) ())
  · rintro ⟨hroot, htorus⟩ (i | u)
    · simpa only [kostantTorusSubsystemGeneratorMap_inl] using hroot i.1 i.2
    · rcases u with ⟨⟩
      simpa only [kostantTorusSubsystemGeneratorMap_inr] using htorus

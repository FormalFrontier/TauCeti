...
-- Comparing a parametrized root-subgroup element with its matrix does not involve the represented
-- `GLₙ` presentation, so it is stated for an arbitrary finite basis index.
...
@[simp]
theorem basisMatrix_kostantRootSubgroupParam (b : Module.Basis η ℤ M)
    (A : Type v) [CommRing A]

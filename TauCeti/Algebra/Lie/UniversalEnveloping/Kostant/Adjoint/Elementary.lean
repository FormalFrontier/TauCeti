...
no role there. -/
noncomputable def adjointLattice : AddSubgroup L :=
  hx.chevalleyLieLattice.toSubmodule.toAddSubgroup

-- The public membership lemma below cannot unfold `adjointLattice` itself: the module system only
-- lets an exported theorem unfold exposed definitions.
private theorem mem_adjointLattice_iff_def {y : L} :
    y ∈ hx.adjointLattice ↔ y ∈ hx.chevalleyLieLattice :=
  Iff.rfl
...
theorem kostantForm_apply_mem_adjointLattice :
  ...
  intro u hu v hv
  rw [mem_adjointLattice_iff] at hv ⊢

...
The Cartan--Cartan normal-ordering rule is `TauCeti.choose_mul_choose`: it expands a product of two
binomial coefficients in one Cartan vector as an integral linear combination of binomial
coefficients in that vector. Consequently `TauCeti.ringChooseSpan` for any designated Cartan
vector lies in the Kostant form.
...
* `TauCeti.UniversalEnvelopingAlgebra.ringChooseSpan_le_kostantForm`: the one-Cartan integral
  binomial span lies in the Kostant form.
...
/-- The integral span of the binomial coefficients in one designated Cartan vector is contained
in the Kostant form. -/
theorem ringChooseSpan_le_kostantForm (e : ι → L) (h : κ → L) (i : κ) :
    ringChooseSpan (_root_.UniversalEnvelopingAlgebra.ι ℚ (h i)) ≤
      (kostantForm e h).toAddSubgroup :=
  ringChooseSpan_le_iff.2 fun n =>
    Subring.mem_toAddSubgroup.2 (ringChoose_mem_kostantForm e h i n)

...
One limitation carries over from the file this one builds on: the carrier is not identified with the
symplectic group scheme. Over a field, its matrix points are nevertheless the standard symplectic
group by `TauCeti.SpStd.points_eq_GLSymplecticFin`; this file works over a general commutative ring
and describes the fixed group as the carrier's points over the fixed subring. No statement here
restricts to the elementary subgroup generated by the root subgroups. Nothing below asserts that
the carrier is reductive, or that a group in sight is perfect, simple, or a named finite group of
Lie type.
...
Frobenius-fixed points of `TauCeti.SlStd` with `SL_(r+1)` over the fixed subfield. For type `C`,
`TauCeti.SpStd.points_eq_GLSymplecticFin` identifies the carrier's points over every field with the
standard symplectic group. The general-ring construction here retains the description by carrier
points over the fixed subring, as in
...
simplicity statement is claimed. The field-point equality
`TauCeti.SpStd.points_eq_GLSymplecticFin` identifies the source of the fixed-point isomorphism with
the standard symplectic group over the Frobenius-fixed field, but this theorem records only
finiteness and does not package the resulting isomorphism with `Sp_(2n+2)(q)`. -/

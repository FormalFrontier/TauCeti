...
Both definitions are `@[expose]`d: each is a repackaging that changes no data, and the equations
`TauCeti.lieSubmoduleOfEqTop_toSubmodule` and `TauCeti.lieModuleEquivOfEqTop_apply` recording that
are `rfl`, which a public theorem may only assert about exposed definitions. Those two equations
are the whole elimination API; nothing needs to unfold further.
-/

...
* `TauCeti.E7Minuscule.rootGeneratorWeight`: the root character of each numbered subgroup.
...
generators. -/
def rootGeneratorWeight : Fin 7 ⊕ Fin 7 → Fin 7 → ℤ
  | .inl i => CartanMatrix.E 7 i
  | .inr i => -(CartanMatrix.E 7 i)

/-- The character of a positive simple root generator is the corresponding Cartan-matrix row. -/
@[simp]
theorem rootGeneratorWeight_inl (i j : Fin 7) :
    rootGeneratorWeight (.inl i) j = CartanMatrix.E 7 i j := (rfl)

/-- The character of a negative simple root generator is the negative Cartan-matrix row. -/
@[simp]
theorem rootGeneratorWeight_inr (i j : Fin 7) :
    rootGeneratorWeight (.inr i) j = -CartanMatrix.E 7 i j := (rfl)

/-- The type-`E₇` Cartan matrix is symmetric. -/
private theorem cartanMatrix_apply_comm (i j : Fin 7) :
    CartanMatrix.E 7 i j = CartanMatrix.E 7 j i := by
...
theorem lie_cartanGenerator_rootGenerator (k : Fin 7 ⊕ Fin 7) (j : Fin 7) :
  ...
  | inl i =>
      rw [TauCeti.serreRootGenerator_inl, rootGeneratorWeight,
        TauCeti.lie_serreH_serreE, cartanMatrix_apply_comm]
      simp only [Int.cast_smul_eq_zsmul]
  | inr i =>
      rw [TauCeti.serreRootGenerator_inr, rootGeneratorWeight,
        TauCeti.lie_serreH_serreF, cartanMatrix_apply_comm]

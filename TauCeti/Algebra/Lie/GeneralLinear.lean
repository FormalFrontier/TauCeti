...
ideal, and neither piece is trivial. This file identifies both pieces.
...
commutative ring, provided the index type is nonempty: one inclusion is the vanishing of the trace
of a commutator, and the other writes a trace-zero matrix as a combination of the commutators
...
* `TauCeti.slIdeal R n`: the trace-zero matrices, as a `LieIdeal R (Matrix n n R)`. Mathlib's
  ...
  `TauCeti.slIdeal_toLieSubalgebra` identifies the two.

## Main results

* `TauCeti.mem_center_matrix_iff`: an element of `gl n R` is central exactly when it is a scalar
  matrix, and `TauCeti.center_matrix_toSubmodule` records the centre as the span of `1`.
* `TauCeti.derivedSeries_one_eq_slIdeal`: the derived ideal of `gl n R` is `TauCeti.slIdeal R n`, so
  ...
  `TauCeti.derivedSeries_one_toLieSubalgebra` reads this as `LieAlgebra.SpecialLinear.sl n R`.
...
* `TauCeti.not_hasTrivialRadical_matrix`: `gl n R` is never semisimple, since its centre is a
  nonzero abelian ideal (`TauCeti.center_matrix_ne_bot`).
...
theorem center_matrix_toSubmodule :
...
subalgebra; `TauCeti.slIdeal_toLieSubalgebra` identifies the two. The ideal packaging is what the
derived series of `gl n R` lives in. -/
def slIdeal : LieIdeal R (Matrix n n R) where
  ...
  lie_mem {X A} _ := by
    simp [LieRing.of_associative_ring_bracket, Matrix.trace_mul_comm X A]
...
theorem slIdeal_toLieSubalgebra :
...
vanishes. -/
theorem derivedSeries_one_eq_slIdeal [Nonempty n] :
  ...
  · rw [hderived, LieSubmodule.lie_le_iff]
    intro X _ Y _
    simp [LieRing.of_associative_ring_bracket, Matrix.trace_mul_comm X Y]
...
theorem derivedSeries_one_toLieSubalgebra [Nonempty n] :
    (derivedSeries R (Matrix n n R) 1 : LieSubalgebra R (Matrix n n R))
      = LieAlgebra.SpecialLinear.sl n R := by
  rw [derivedSeries_one_eq_slIdeal R n, slIdeal_toLieSubalgebra R n]
...
theorem isCompl_center_derivedSeries_one_matrix [Nonempty n] [Invertible (Fintype.card n : R)] :
  ...
  · rw [Submodule.disjoint_def]
    ...
    rw [derivedSeries_one_eq_slIdeal R n] at hA'
    have htr : (r • (1 : Matrix n n R)).trace = 0 := hA'
    have hr : (Fintype.card n : R) * r = 0 := by
      simpa [Matrix.trace_smul, Matrix.trace_one, mul_comm] using htr
    rw [show r = 0 by
      simpa [← mul_assoc, invOf_mul_self] using congrArg (⅟(Fintype.card n : R) * ·) hr, zero_smul]
  · rw [codisjoint_iff_le_sup]
    ...
    rw [derivedSeries_one_eq_slIdeal R n]
    change (A - r • (1 : Matrix n n R)).trace = 0
...
variable (R n) in
/-- `gl n R` is not semisimple: its centre is a nonzero abelian ideal, so its radical is nonzero.
This is why the highest-weight theory of `gl n R` cannot be read off Mathlib's `IsKilling`
machinery, and has to be developed through the reductive decomposition instead. -/

...
(`TauCeti.coeff_formalCharacter_mul_weylDenominator_eq_one_of_isHighestWeightVector`): the top
weight space of a highest weight module is a line and the constant term of `Δ` is `1`, and the
positive root cone is pointed, so `lam` admits only the one decomposition.
...
* `TauCeti.coeff_formalCharacter_mul_weylDenominator_eq_one_of_isHighestWeightVector`: and its
  **coefficient at `lam` is `1`**.
...
theorem formalCharacter_coeff_eq_one_of_isHighestWeightVector :
...
of which are `1`. -/
theorem coeff_formalCharacter_mul_weylDenominator_eq_one_of_isHighestWeightVector :
  ...
  have hchar : (formalCharacter K H M).coeff lam = 1 :=
    formalCharacter_coeff_eq_one_of_isHighestWeightVector hv hgen

...
base, that description is not the one to build from. The equivalent presentation used here is the
quotient of `U(L)` by the left ideal it generates on the kernel of the character of `K_lam`:
...
The two agree because `ι x - lam h = (ι h - lam h) + ι n` for a decomposition `x = h + n` of an
element of `𝔟`, which is the content of
`TauCeti.exists_ι_sub_algebraMap_mem_vermaIdeal_of_mem_borelSubalgebra` below: the generating set is
the image of `ker (K_lam)` under `ι`, up to the left ideal it generates.
...
relations, that is the left ideal generated by the kernel of the character `K_lam` of the Borel
subalgebra. -/
...
/-- **The Verma ideal contains the kernel of the Borel character**: an element `x = h + n` of the
Borel subalgebra contributes the relation `ι x - lam h`, which is the sum of the two kinds of
generator. This is the identification of the presentation used here with the classical
`U(L) ⊗_{U(𝔟)} K_lam`. -/
...
theorem existsUnique_lieModuleHom_apply_vermaGenerator (v : N)
  ...
  have hg : ∀ u : U, g (vermaMk b lam u) = u • v := fun u => hf u

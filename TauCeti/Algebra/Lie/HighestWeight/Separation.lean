...
This file proves the two nonvanishing statements that make that scalar separate things.
...
The second is **separation of the weights of one module**: on a finite-dimensional module generated
by a highest weight vector of weight `lam`, the scalars `c(mu)` attached to the weights `mu` of the
module are all different from `c(lam)` except at `mu = lam` itself
...
of a finite-dimensional module are Weyl stable
...
* `TauCeti.casimirScalar_ne_casimirScalar_of_genWeightSpace_ne_bot_of_isHighestWeightVector`:
  **the Casimir scalar of a weight of a highest weight module differs from that of the highest
  weight, except at the highest weight itself.**
...
/-- **Every weight of a highest weight module has a dominant integral Weyl conjugate, again a
...
theorem exists_weylGroup_smul_isDominantIntegral_of_genWeightSpace_ne_bot
    (hv : IsHighestWeightVector base lam v) (hgen : LieSubmodule.lieSpan K L {v} = ⊤)
    {mu : Module.Dual K H} (hmu : genWeightSpace M (mu : H → K) ≠ ⊥) :
    ∃ w : (IsKilling.rootSystem H).weylGroup,
      genWeightSpace M ((w • mu : Module.Dual K H) : H → K) ≠ ⊥ ∧
        IsDominantIntegral base (w • mu) := by
  classical
  ...
  have hcoroot' : ∀ (i : H.root) (chi : Module.Dual K H),
  ...
  have hweyl : ∀ (w : (IsKilling.rootSystem H).weylGroup) (chi : Module.Dual K H),
      genWeightSpace M ((w • chi : Module.Dual K H) : H → K) = ⊥ ↔
  ...
  set S : Set (Module.Dual K H) :=
    {chi : Module.Dual K H | genWeightSpace M (chi : H → K) ≠ ⊥}
  have hcone : ∀ chi ∈ S, lam - chi ∈ posRootCone (IsKilling.rootSystem H) base :=
    fun chi hchi ↦
      sub_mem_posRootCone_of_genWeightSpace_ne_bot_of_isHighestWeightVector_of_lieSpan_eq_top
        hv hgen hchi
  have hint : ∀ chi ∈ S, ∀ i ∈ base.support,
  ...
  have hrefl : ∀ chi ∈ S, ∀ i ∈ base.support,
  ...
  obtain ⟨w, hwS, hwdom⟩ :=
    exists_weylGroup_smul_dominant_of_forall_reflection_mem_of_sub_mem_posRootCone base hcone hint
      hrefl hmu
...
weights**: on a finite-dimensional module generated by a highest weight vector of weight `lam`, the
scalar `⟨mu + ρ, mu + ρ⟩` attached to a weight `mu ≠ lam` differs from `⟨lam + ρ, lam + ρ⟩`.

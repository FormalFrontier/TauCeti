...
* `Ω` is **injective** on a nontrivial finite-dimensional irreducible module
  (`TauCeti.representation_casimirElement_injective_of_isIrreducible`): such a module is generated
  by a highest weight vector of a dominant integral weight `λ`
  (`TauCeti.exists_isHighestWeightVector_and_lieSpan_eq_top`), and `λ ≠ 0` because a highest
  weight vector of weight zero generates a trivial module
...
* `TauCeti.representation_casimirElement_injective_of_isIrreducible`: the Casimir element acts
  injectively on a finite-dimensional irreducible module with a nontrivial action.
...
theorem representation_casimirElement_injective_of_isIrreducible
    [LieModule.IsIrreducible K L M] (hact : ∃ (x : L) (m : M), ⁅x, m⁆ ≠ 0) :
  ...
  have hlam : lam ≠ 0 := by
    rintro rfl
    obtain ⟨x, m, hm⟩ := hact
    exact hm ((isTrivial_of_isHighestWeightVector_weight_zero_of_isIrreducible hv).trivial x m)
...
theorem hasInvariantOutsideIrreducible_of_isKilling :
  ...
  have hactN : ∃ (x : L) (z : N), ⁅x, z⁆ ≠ 0 := by
    ...
    push Not at hcon
    obtain ⟨x, m, hm⟩ := hact
    refine hm ((isTrivial_of_derivedSeries_one_eq_top
    ...
    simpa using hval
  have hinjN := representation_casimirElement_injective_of_isIrreducible (K := K) (↥N) hactN
  have hinj : ∀ w : M, w ∈ N →

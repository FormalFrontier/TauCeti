...
`TauCeti.Algebra.Coalgebra.Comodule.MatrixCoefficient.Basic`: if `M` has a group-like coaction
...
theorem trivial_eq_groupLike_one : (trivial (R := R) (C := C) (M := M) : Comodule R C M) =

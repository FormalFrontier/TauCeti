...
the bookkeeping objects needed to state the faithful-representation criterion in the
reductive-groups roadmap: for an affine group scheme represented by a Hopf algebra `A`, a
representation is faithful exactly when its matrix coefficients generate `A`.

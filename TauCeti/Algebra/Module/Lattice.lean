...
domain rationalizes to its ambient vector space over the fraction field. The file also records the
generic order correspondence between a submodule interval and submodules of the associated
quotient.

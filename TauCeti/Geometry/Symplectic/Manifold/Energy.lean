...
4. **Integrated energy.** For a curve $u : \mathbb{R}^2 \to M$ and a measure $\mu$, its energy is
   $$E(u) = \int^\ast \frac{1}{2} e(du_z) \, d\mu(z).$$
   For an almost-everywhere pseudoholomorphic curve, $E(u)$ equals the lower integral of the
   symplectic area $\int^\ast \omega_{u(z)}(du_z(\partial_s), du_z(\partial_t)) \, d\mu(z)$. Under
   tameness, zero energy detects curves whose manifold derivative vanishes almost everywhere.

This completes the energy package of Lane F2.1 of the analytic Heegaard Floer roadmap.
...
* `TauCeti.SmoothTwoForm.stdComplexLineEnergy`: the integrated energy of a curve
  $u : \mathbb{R}^2 \to M$.
...
* `TauCeti.SmoothTwoForm.stdComplexLineEnergy_eq_lintegral_twoForm`: equality of integrated energy
  and symplectic area for almost-everywhere pseudoholomorphic curves.
* `TauCeti.SmoothTwoForm.stdComplexLineEnergy_eq_zero_of_ae_mfderiv_eq_zero`: zero energy for
  curves with almost everywhere vanishing derivative.
...
lemma stdComplexLineEnergyDensity_zero (form : SmoothTwoForm I M)
  ...
  have h1 : (0 : (ℝ × ℝ) →ₗ[ℝ] TangentSpace I x) stdComplexLineReal = 0 := rfl
  have h2 : (0 : (ℝ × ℝ) →ₗ[ℝ] TangentSpace I x) stdComplexLineImag = 0 := rfl
  rw [h1, h2]
  have hz : form x 0 (J x 0) = 0 := by
    have h : form x 0 (J x 0) = (form.toContMDiffSection x 0) (J x 0) := rfl
    rw [h, ContinuousLinearMap.map_zero, _root_.zero_apply]
  rw [hz, add_zero]
...
lemma stdComplexLineEnergyDensity_eq_two_mul_twoForm
  ...
  have hneg_right : form x (F stdComplexLineImag) (-F stdComplexLineReal) =
      - form x (F stdComplexLineImag) (F stdComplexLineReal) :=
    (form.toContMDiffSection x (F stdComplexLineImag)).map_neg (F stdComplexLineReal)
  have halt : - form x (F stdComplexLineImag) (F stdComplexLineReal) =
      form x (F stdComplexLineReal) (F stdComplexLineImag) := by
    have h := LinearMap.IsAlt.neg (form.isAlt_bilinFormAt x)
      (F stdComplexLineImag) (F stdComplexLineReal)
    simpa only [form.bilinFormAt_apply] using h
  rw [hneg_right, halt]
  ring
...
/-- Under tameness, the manifold derivative's standard pointwise energy density is nonnegative. -/
lemma mfderiv_stdComplexLineEnergyDensity_nonneg (htame : form.Tames J)
    (u : ℝ × ℝ → M) (z : ℝ × ℝ) :
    0 ≤ form.stdComplexLineEnergyDensity J (u z)
      (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z).toLinearMap :=
  htame.stdComplexLineEnergyDensity_nonneg (u z) _

/-- Under tameness, the manifold derivative's standard energy density vanishes exactly when the
manifold derivative is zero. -/
lemma mfderiv_stdComplexLineEnergyDensity_eq_zero_iff (htame : form.Tames J)
    (u : ℝ × ℝ → M) (z : ℝ × ℝ) :
    form.stdComplexLineEnergyDensity J (u z)
        (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z).toLinearMap = 0 ↔
      mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z = 0 :=
  htame.stdComplexLineEnergyDensity_toLinearMap_eq_zero_iff (u z)
    (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z)

/-- Under tameness, the manifold derivative's standard energy density is positive exactly when the
manifold derivative is nonzero. -/
lemma mfderiv_stdComplexLineEnergyDensity_pos_iff (htame : form.Tames J)
    (u : ℝ × ℝ → M) (z : ℝ × ℝ) :
    0 < form.stdComplexLineEnergyDensity J (u z)
        (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z).toLinearMap ↔
      mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z ≠ 0 :=
  htame.stdComplexLineEnergyDensity_toLinearMap_pos_iff (u z)
    (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z)

/-- Under tameness, a curve with nonzero manifold derivative has positive energy density. -/
lemma mfderiv_stdComplexLineEnergyDensity_pos (htame : form.Tames J)
    (u : ℝ × ℝ → M) (z : ℝ × ℝ)
    (hderiv : mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z ≠ 0) :
    0 < form.stdComplexLineEnergyDensity J (u z)
        (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z).toLinearMap :=
  (mfderiv_stdComplexLineEnergyDensity_pos_iff htame u z).mpr hderiv
...
/-- Under tameness, the within-set manifold derivative's energy density is nonnegative. -/
lemma mfderivWithin_stdComplexLineEnergyDensity_nonneg (htame : form.Tames J)
    (u : ℝ × ℝ → M) (s : Set (ℝ × ℝ)) (z : ℝ × ℝ) :
    0 ≤ form.stdComplexLineEnergyDensity J (u z)
      (mfderivWithin (modelWithCornersSelf ℝ (ℝ × ℝ)) I u s z).toLinearMap :=
  htame.stdComplexLineEnergyDensity_nonneg (u z) _

/-- Under tameness, the within-set manifold derivative's energy density vanishes exactly when the
derivative within the set is zero. -/
lemma mfderivWithin_stdComplexLineEnergyDensity_eq_zero_iff (htame : form.Tames J)
    (u : ℝ × ℝ → M) (s : Set (ℝ × ℝ)) (z : ℝ × ℝ) :
    form.stdComplexLineEnergyDensity J (u z)
        (mfderivWithin (modelWithCornersSelf ℝ (ℝ × ℝ)) I u s z).toLinearMap = 0 ↔
      mfderivWithin (modelWithCornersSelf ℝ (ℝ × ℝ)) I u s z = 0 :=
  htame.stdComplexLineEnergyDensity_toLinearMap_eq_zero_iff (u z)
    (mfderivWithin (modelWithCornersSelf ℝ (ℝ × ℝ)) I u s z)

/-- Under tameness, the within-set manifold derivative's energy density is positive exactly when
the derivative within the set is nonzero. -/
lemma mfderivWithin_stdComplexLineEnergyDensity_pos_iff (htame : form.Tames J)
    (u : ℝ × ℝ → M) (s : Set (ℝ × ℝ)) (z : ℝ × ℝ) :
    0 < form.stdComplexLineEnergyDensity J (u z)
        (mfderivWithin (modelWithCornersSelf ℝ (ℝ × ℝ)) I u s z).toLinearMap ↔
      mfderivWithin (modelWithCornersSelf ℝ (ℝ × ℝ)) I u s z ≠ 0 :=
...
/-- Under tameness, a curve with nonzero within-set manifold derivative has positive energy
density. -/
lemma mfderivWithin_stdComplexLineEnergyDensity_pos (htame : form.Tames J)
    (u : ℝ × ℝ → M) (s : Set (ℝ × ℝ)) (z : ℝ × ℝ)
    (hderiv : mfderivWithin (modelWithCornersSelf ℝ (ℝ × ℝ)) I u s z ≠ 0) :
    0 < form.stdComplexLineEnergyDensity J (u z)
        (mfderivWithin (modelWithCornersSelf ℝ (ℝ × ℝ)) I u s z).toLinearMap :=
  (mfderivWithin_stdComplexLineEnergyDensity_pos_iff htame u s z).mpr hderiv
...
lemma Invariant.stdComplexLineEnergyDensity_sub_two_mul_twoForm (hinv : form.Invariant J)
  ...
  have h_right : form x (v + J x w) (J x v - w) =
    ...
    (form.toContMDiffSection x (v + J x w)).map_sub (J x v) w
  have h_left1 : form x (v + J x w) (J x v) = form x v (J x v) + form x (J x w) (J x v) :=
    (form.toContMDiffSection x).map_add₂ v (J x w) (J x v)
  have h_left2 : form x (v + J x w) w = form x v w + form x (J x w) w :=
    (form.toContMDiffSection x).map_add₂ v (J x w) w
  have hinv_step : form x (J x w) (J x v) = form x w v := hinv x w v
  have halt1 : - form x w v = form x v w := by
    have h := LinearMap.IsAlt.neg (form.isAlt_bilinFormAt x) w v
    simpa only [form.bilinFormAt_apply] using h
  have halt2 : - form x (J x w) w = form x w (J x w) := by
    have h := LinearMap.IsAlt.neg (form.isAlt_bilinFormAt x) (J x w) w
    simpa only [form.bilinFormAt_apply] using h
  linarith
...
/-- The Wirtinger inequality for the manifold derivative of any curve into an almost complex
manifold. -/
lemma Compatible.two_mul_twoForm_mfderiv_le_stdComplexLineEnergyDensity
    (hcompat : form.Compatible J) (u : ℝ × ℝ → M) (z : ℝ × ℝ) :
    2 * form (u z)
        (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z stdComplexLineReal)
        (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z stdComplexLineImag) ≤
      form.stdComplexLineEnergyDensity J (u z)
        (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z).toLinearMap :=
  hcompat.two_mul_twoForm_le_stdComplexLineEnergyDensity (u z) _

/-- The Wirtinger inequality for the within-set manifold derivative of any curve into an almost
complex manifold. -/
lemma Compatible.two_mul_twoForm_mfderivWithin_le_stdComplexLineEnergyDensity
    (hcompat : form.Compatible J) (u : ℝ × ℝ → M) (s : Set (ℝ × ℝ)) (z : ℝ × ℝ) :
    2 * form (u z)
        (mfderivWithin (modelWithCornersSelf ℝ (ℝ × ℝ)) I u s z stdComplexLineReal)
        (mfderivWithin (modelWithCornersSelf ℝ (ℝ × ℝ)) I u s z stdComplexLineImag) ≤
      form.stdComplexLineEnergyDensity J (u z)
        (mfderivWithin (modelWithCornersSelf ℝ (ℝ × ℝ)) I u s z).toLinearMap :=
  hcompat.two_mul_twoForm_le_stdComplexLineEnergyDensity (u z) _
...
/-- The integrated energy of a curve from the standard complex line into a manifold carrying a
smooth two-form and a smooth almost complex structure: the lower Lebesgue integral of half the
pointwise energy density of the manifold derivative `mfderiv`. -/
noncomputable def stdComplexLineEnergy (form : SmoothTwoForm I M)
    (J : SmoothAlmostComplexStructure I M) (u : (ℝ × ℝ) → M) (μ : Measure (ℝ × ℝ)) : ℝ≥0∞ :=
  ∫⁻ z, ENNReal.ofReal (form.stdComplexLineEnergyDensity J (u z)
    (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z).toLinearMap / 2) ∂μ
...
lemma stdComplexLineEnergy_def (form : SmoothTwoForm I M)
    ...
    form.stdComplexLineEnergy J u μ =
      ∫⁻ z, ENNReal.ofReal (form.stdComplexLineEnergyDensity J (u z)
        (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z).toLinearMap / 2) ∂μ :=
  rfl
...
lemma stdComplexLineEnergy_mono_measure {μ ν : Measure (ℝ × ℝ)} (hμν : μ ≤ ν)
    ...
    form.stdComplexLineEnergy J u μ ≤ form.stdComplexLineEnergy J u ν := by
  rw [stdComplexLineEnergy_def, stdComplexLineEnergy_def]
  exact lintegral_mono' hμν le_rfl

/-- Every curve has zero energy with respect to the zero measure. -/
@[simp]
lemma stdComplexLineEnergy_zero_measure (form : SmoothTwoForm I M)
    (J : SmoothAlmostComplexStructure I M) (u : (ℝ × ℝ) → M) :
    form.stdComplexLineEnergy J u 0 = 0 := by
...
lemma stdComplexLineEnergy_const (form : SmoothTwoForm I M)
    ...
    form.stdComplexLineEnergy J (fun _ ↦ c) μ = 0 := by
  rw [stdComplexLineEnergy_def]
  have hz : (fun z ↦ ENNReal.ofReal (form.stdComplexLineEnergyDensity J ((fun _ ↦ c) z)
      (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I
        (fun _ : ℝ × ℝ ↦ c) z).toLinearMap / 2)) = (fun _ ↦ 0) := by
    ext z
    have h1 : mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I (fun _ : ℝ × ℝ ↦ c) z = 0 :=
      (hasMFDerivAt_const (I := modelWithCornersSelf ℝ (ℝ × ℝ)) (I' := I)
        (M' := M) (x := z) c).mfderiv
    rw [h1]
    have hz_dens : form.stdComplexLineEnergyDensity J c (0 : (ℝ × ℝ) →ₗ[ℝ] TangentSpace I c) = 0 :=
      stdComplexLineEnergyDensity_zero form J c
    change ENNReal.ofReal (form.stdComplexLineEnergyDensity J c 0 / 2) = 0
    rw [hz_dens, zero_div, ENNReal.ofReal_zero]
  rw [hz, lintegral_zero]

/-- **Integrated Wirtinger inequality on manifolds.** For a compatible pair `(form, J)`, the
integral of the positive symplectic area density is bounded by the integrated energy. -/
lemma Compatible.lintegral_twoForm_le_stdComplexLineEnergy (hcompat : form.Compatible J)
    (u : ℝ × ℝ → M) (μ : Measure (ℝ × ℝ)) :
    (∫⁻ z, ENNReal.ofReal
      (form (u z)
        (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z stdComplexLineReal)
        (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z stdComplexLineImag)) ∂μ) ≤
      form.stdComplexLineEnergy J u μ := by
  rw [stdComplexLineEnergy_def]
  apply lintegral_mono
  intro z
  apply ENNReal.ofReal_le_ofReal
  have hpoint := hcompat.two_mul_twoForm_mfderiv_le_stdComplexLineEnergyDensity u z
  linarith

/-- **Equality in the integrated Wirtinger inequality on manifolds.** A curve that is
pseudoholomorphic almost everywhere has energy equal to the lower integral of its symplectic area
density. -/
lemma stdComplexLineEnergy_eq_lintegral_twoForm
  ...
  rw [hz.mfderiv_stdComplexLineEnergyDensity_eq_two_mul_twoForm (form := form)]
  congr 1
  ring

private lemma energyIntegrand_eq_zero_iff (htame : form.Tames J) (x : M)
    (F : (ℝ × ℝ) →L[ℝ] TangentSpace I x) :
    ENNReal.ofReal (form.stdComplexLineEnergyDensity J x F.toLinearMap / 2) = 0 ↔ F = 0 := by
  rw [ENNReal.ofReal_eq_zero]
  constructor
  · intro h
    have hzero : form.stdComplexLineEnergyDensity J x F.toLinearMap = 0 := by
      have hpos := htame.stdComplexLineEnergyDensity_nonneg x F.toLinearMap
      linarith
    exact (htame.stdComplexLineEnergyDensity_toLinearMap_eq_zero_iff x F).mp hzero
  · rintro rfl
    have h0 : (0 : (ℝ × ℝ) →L[ℝ] TangentSpace I x).toLinearMap = 0 := rfl
    rw [h0, stdComplexLineEnergyDensity_zero, zero_div]

/-- If the manifold derivative of a curve vanishes almost everywhere, its energy is zero. -/
lemma stdComplexLineEnergy_eq_zero_of_ae_mfderiv_eq_zero (form : SmoothTwoForm I M)
    (J : SmoothAlmostComplexStructure I M) {u : ℝ × ℝ → M} {μ : Measure (ℝ × ℝ)}
    (hderiv : ∀ᵐ z ∂μ, mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z = 0) :
    form.stdComplexLineEnergy J u μ = 0 := by
  rw [stdComplexLineEnergy_def]
  have hzero : (fun z ↦ ENNReal.ofReal
      (form.stdComplexLineEnergyDensity J (u z)
        (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z).toLinearMap / 2)) =ᵐ[μ] (fun _ ↦ 0) := by
    filter_upwards [hderiv] with z hz
    rw [hz]
    have hz_dens : form.stdComplexLineEnergyDensity J (u z)
        (0 : (ℝ × ℝ) →ₗ[ℝ] TangentSpace I (u z)) = 0 :=
      stdComplexLineEnergyDensity_zero form J (u z)
    change ENNReal.ofReal (form.stdComplexLineEnergyDensity J (u z) 0 / 2) = 0
    rw [hz_dens, zero_div, ENNReal.ofReal_zero]
  rw [lintegral_congr_ae hzero, lintegral_zero]

/-- Under tameness, integrated energy vanishes exactly when the manifold derivative vanishes almost
...
lemma Tames.stdComplexLineEnergy_eq_zero_iff (htame : form.Tames J)
    {u : ℝ × ℝ → M} {μ : Measure (ℝ × ℝ)}
    (hmeas : AEMeasurable (fun z ↦ form.stdComplexLineEnergyDensity J (u z)
      (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z).toLinearMap) μ) :
    form.stdComplexLineEnergy J u μ = 0 ↔
      ∀ᵐ z ∂μ, mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z = 0 := by
  rw [stdComplexLineEnergy_def,
  ...
  · intro h
    ...
    have hz' : ENNReal.ofReal
        (form.stdComplexLineEnergyDensity J (u z)
          (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z).toLinearMap / 2) = 0 := hz
    exact (energyIntegrand_eq_zero_iff (form := form) (J := J) htame (u z)
      (mfderiv (modelWithCornersSelf ℝ (ℝ × ℝ)) I u z)).mp hz'
  · intro h
    filter_upwards [h] with z hz
    rw [hz]
    have hz_dens : form.stdComplexLineEnergyDensity J (u z)
        (0 : (ℝ × ℝ) →ₗ[ℝ] TangentSpace I (u z)) = 0 :=
      stdComplexLineEnergyDensity_zero form J (u z)
    change ENNReal.ofReal (form.stdComplexLineEnergyDensity J (u z) 0 / 2) = (0 : ℝ≥0∞)
    rw [hz_dens, zero_div, ENNReal.ofReal_zero]

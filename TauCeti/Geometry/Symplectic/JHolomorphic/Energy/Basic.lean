...
-- The equation above is the whole elimination API of the energy density; nothing unfolds the body
-- again. It is written out by hand rather than being generated by `irreducible_def`, whose
-- equation lemma states the *elaborated* body: that mentions the auxiliary `SMulCommClass ℝ ℝ ℝ`
-- instance proof extracted from the helper definition, which the module system keeps private, so
-- rewriting with it from another module fails to kernel-check.

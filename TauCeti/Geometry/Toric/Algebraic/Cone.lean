...
* `TauCeti.Toric.IsToricCone.prod`: a product of toric cones is a toric cone for the product
  lattice map.
* `PointedCone.nonneg_of_mem_hull` and `PointedCone.eq_zero_of_mem_hull`: a real-linear functional
  that is nonnegative on, respectively vanishes on, a generating set does so on the whole cone
  hull. These are general facts about Mathlib's `PointedCone.hull`, kept in its own namespace.
...
variable {V : Type*} [AddCommGroup V] [Module ℝ V] {t : Set V} {x : V}

/-- A real-linear functional that is nonnegative on a generating set is nonnegative on the whole
cone hull. -/
theorem nonneg_of_mem_hull (f : V →ₗ[ℝ] ℝ) (ht : ∀ x ∈ t, 0 ≤ f x) (hx : x ∈ hull ℝ t) :
    0 ≤ f x := by
...
theorem eq_zero_of_mem_hull (f : V →ₗ[ℝ] ℝ) (ht : ∀ x ∈ t, f x = 0) (hx : x ∈ hull ℝ t) :
    f x = 0 :=
...
theorem IsToricCone.map (hfg : ∀ n, g (i n) = i' (f n)) (hg : Function.Injective g)
  ...
  .of_isLatticeRational (hσ.rational.map hfg) (hσ.salient.map hg)
...
theorem IsToricCone.prod {τ' : PointedCone ℝ V'} (hσ : IsToricCone i σ)
  ...
  .of_isLatticeRational (hσ.rational.prod hτ'.rational) (hσ.salient.prod hτ'.salient)

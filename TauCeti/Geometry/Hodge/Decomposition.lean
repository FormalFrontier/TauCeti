...
* `TauCeti.Hodge.HodgeStructureOn.sum_proj_eq`: the projections in the finitely many degrees
  carrying a nonzero component sum to the identity.
...
/-- The Hodge projections in the degrees of a finite set carrying every nonzero Hodge component
sum to the identity: a vector is the sum of its finitely many Hodge components. -/
theorem sum_proj_eq (hs : HodgeStructureOn W ω n) {S : Finset ℤ}
    (hS : ∀ p ∉ S, hs.piece p = ⊥) (x : W) : ∑ p ∈ S, hs.proj p x = x := by
  refine hs.piece_induction_on (motive := fun y ↦ ∑ p ∈ S, hs.proj p y = y) x
    (fun q y hy ↦ ?_) (by simp) (fun y z hy hz ↦ ?_)
  · by_cases hq : q ∈ S
    · rw [Finset.sum_eq_single_of_mem q hq fun b _ hbq ↦
        hs.proj_apply_eq_zero_of_mem_of_ne hy (Ne.symm hbq), hs.proj_apply_of_mem hy]
    · rw [hS q hq] at hy
      rw [(Submodule.mem_bot ℂ).1 hy]
      simp

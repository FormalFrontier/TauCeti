...
The proof needs the chain rule for a function on the manifold restricted to the curve, which is
recorded first for a general normed-space-valued function.
...
* `TauCeti.Manifold.hasDerivWithinAt_extChartAt_comp_curve`: reading the curve in the chart
  centred at the current point differentiates it to the velocity itself.
...
/-! ### Differentiating a function along a curve -/

...
The identification of `jacobsonRadical X X` with Mathlib's `Ring.jacobson (End X)` is *not* proved
here. `Ring.jacobson` is defined as the infimum of the maximal left ideals, and the quasi-regularity
characterization that would bridge it to the definition below (`Ideal.mem_jacobson_bot`) is
available only over a commutative ring, whereas `End X` is not commutative; the bridge is therefore
the noncommutative characterization theorem of the Jacobson radical, a piece of general ring theory
rather than an API lemma about categories. What the consumers of this file need is the description
by non-units, `mem_jacobsonRadical_self_iff_not_isUnit`, which is proved.
...
omit [IsLocalRing (End X)] [IsLocalRing (End Y)] in
/-- **A composite that is invertible has an invertible first factor**, when the object it passes
through has only the trivial idempotents and the identity of its source is nonzero: the composite
produces an idempotent of `End Y`, which is then `0` or `𝟙 Y`, and `0` is excluded by `𝟙 X ≠ 0`.

Neither hypothesis is stated as `[IsLocalRing (End _)]`, since only these two consequences of
locality are used: a caller with a local `End X` supplies `one_ne_zero`, and one with a local
`End Y` supplies `TauCeti.IsLocalRing.eq_zero_or_eq_one_of_isIdempotentElem`. -/
theorem isIso_of_isIso_comp (hX : 𝟙 X ≠ 0)
    (hY : ∀ e : End Y, IsIdempotentElem e → e = 0 ∨ e = 1) (f : X ⟶ Y) (g : Y ⟶ X)
...
theorem mem_jacobsonRadical_iff_not_isIso {f : X ⟶ Y} :
  ...
  · -- A morphism that splits is invertible here, its retraction turning `𝟙 X` into a composite.
    ...
    have := hsm
    exact isIso_of_isIso_comp (one_ne_zero (α := End X))
      (fun _ he => IsLocalRing.eq_zero_or_eq_one_of_isIdempotentElem he) f (retraction f)

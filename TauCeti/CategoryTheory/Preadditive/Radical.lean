...
/-- **Between objects with local endomorphism rings the irreducible morphisms are exactly the
morphisms in the radical but not in its square** — the statement that the space of irreducible
morphisms `X ⟶ Y` is `rad(X, Y) / rad²(X, Y)`, which is what an arrow of the Auslander-Reiten
quiver records.

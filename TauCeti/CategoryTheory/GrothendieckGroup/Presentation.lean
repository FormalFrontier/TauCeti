...
The presentation's induction, lifting, and extensionality interfaces quantify over objects of `C`,
so callers do not need to manipulate skeleton representatives.
...
/-- The classes of objects generate the presented Grothendieck group as an additive subgroup. -/
...
noncomputable def map (F : C ⥤ D)
  ...
  lift
    { obj := fun X => of (F.obj X)
      ...
      map_rel := fun r hr => by
        simp_rw [← mk_freeClass]
        rw [freeLift_comp mk (fun X : C => freeClass (F.obj X)), AddMonoidHom.comp_apply]
...
lemma coe_mapEquiv (e : C ≌ D)
  ...
  hom_ext fun X => by
    -- `mapEquiv` stores `map e.functor h` as its forward homomorphism; this `change` only
    -- normalizes the coercion from the bundled additive equivalence to that homomorphism.

...
noncomputable def map (F : C ⥤ D)
  ...
  lift
    { obj := fun X => of (F.obj X)
      ...
      map_rel := fun r hr => by
        rw [show (freeLift fun X : C => (of (F.obj X) : PresentedK0 relsD)) =
              (mk : _ →+ PresentedK0 relsD).comp (freeMap F) from
...
@[simp]
lemma map_id (h : ∀ r ∈ relsC, freeMap (𝟭 C) r ∈ AddSubgroup.closure relsC) :
    map (𝟭 C) h = AddMonoidHom.id _ :=
...
noncomputable def mapEquiv (e : C ≌ D)
  ...
  left_inv x :=
    DFunLike.congr_fun
      (show (map e.inverse h').comp (map e.functor h) = AddMonoidHom.id _ from
        hom_ext fun X => by
          rw [AddMonoidHom.comp_apply, map_of, map_of, AddMonoidHom.id_apply]
          exact (of_congr (e.unitIso.app X)).symm) x
  right_inv x :=
    DFunLike.congr_fun
      (show (map e.functor h).comp (map e.inverse h') = AddMonoidHom.id _ from
        hom_ext fun Y => by
          rw [AddMonoidHom.comp_apply, map_of, map_of, AddMonoidHom.id_apply]
          exact of_congr (e.counitIso.app Y)) x
  map_add' := map_add _

...
lemma ofLE_mk {rels rels' : Set (FreeAbelianGroup (ObjectCode C))}
  ...
  rw [ofLE, lift_mk, freeLift_of_eq_mk]
...
noncomputable def map (F : C ⥤ D)
  ...
  lift
    { obj := fun X => of (F.obj X)
      ...
      map_rel := fun r hr => by
        simpa only [of, freeMap, freeLift_comp, AddMonoidHom.comp_apply] using
          (mk_eq_zero_iff (rels := relsD) (freeMap F r)).2 (h r hr) }
...
lemma map_mk (F : C ⥤ D) (h : ∀ r ∈ relsC, freeMap F r ∈ AddSubgroup.closure relsD)
  ...
  simp only [map, lift_mk, of, freeMap, freeLift_comp, AddMonoidHom.comp_apply]

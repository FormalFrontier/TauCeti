...
* `TauCeti.GradedExactStructure.shiftAut` and `TauCeti.GradedExactStructure.shiftZPow`: the
  same automorphism as an element of the automorphism group of exact `K₀`, and the induced
  `ℤ`-action.
...
/-- The grading shift as an element of the automorphism group of exact `K₀`. -/
@[expose]
noncomputable def shiftAut : AddAut (ExactK0 E.toExactStructure) := E.shiftEquiv

@[simp]
lemma shiftAut_apply (x : ExactK0 E.toExactStructure) : E.shiftAut x = E.shiftEquiv x := rfl
...
`n`-fold shift is `n • E.shiftAut`; the Laurent-module repackaging of this action, in which it
...
noncomputable def shiftZPow : ℤ →+ AddAut (ExactK0 E.toExactStructure) :=
  zmultiplesHom _ E.shiftAut

@[simp]
lemma shiftZPow_apply (n : ℤ) : E.shiftZPow n = n • E.shiftAut := rfl

lemma shiftZPow_zero_apply (x : ExactK0 E.toExactStructure) : E.shiftZPow 0 x = x := by
  simp

lemma shiftZPow_one : E.shiftZPow 1 = E.shiftAut := by
  simp
...
lemma shiftZPow_add_one_apply (n : ℤ) (x : ExactK0 E.toExactStructure) :
  ...
  rw [add_comm, shiftZPow_add_apply, shiftZPow_one, shiftAut_apply]

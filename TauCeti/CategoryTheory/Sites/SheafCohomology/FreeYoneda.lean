...
* `TauCeti.CategoryTheory.Sheaf.freeYonedaSheafFunctor J`, the functor
  `C ⥤ Sheaf J AddCommGrpCat`
  sending `U` to the free abelian sheaf `ℤ_U` generated by `yoneda.obj U`;
* `TauCeti.CategoryTheory.Sheaf.freeYonedaSheafSectionsEquiv`, the additive equivalence between
  morphisms `ℤ_U ⟶ F` and sections of `F` over `U`, together with its naturality in each variable;
* `TauCeti.CategoryTheory.Sheaf.mono_freeYonedaSheafFunctor_map`, saying that on a thin site,

...
Let `A` be a torsion-free algebra over an integral domain `k` -- no nonzero scalar annihilates a
nonzero element -- and let `e : ι → A` be a family of pairwise orthogonal idempotents, none of
them zero. Then the `eᵢ` are linearly independent over `k`: multiplying a vanishing combination
...
theorem linearIndependent_of_orthogonalIdempotents {k A ι : Type*} [CommRing k] [IsDomain k]
    [Ring A] [Algebra k A] [Module.IsTorsionFree k A] {e : ι → A} (he : OrthogonalIdempotents e)

...
/-- **The pinned embedding is onto the cyclotomic integers**: every element of the subring
`ℤ[ζ_e]` of `ℂ` generated by the distinguished root has an exact cyclotomic representative. -/
theorem exists_complexEmbedding_eq_of_mem_adjoin [NeZero e] {x : ℂ}
    (hx : x ∈ Algebra.adjoin ℤ ({complexRoot e} : Set ℂ)) :
    ∃ y : Cyclotomic e, complexEmbedding y = x :=
  Algebra.adjoin_le
    (Set.singleton_subset_iff.mpr
      ⟨zeta e, complexEmbedding_zeta⟩ :
      ({complexRoot e} : Set ℂ) ⊆ (complexEmbedding (e := e)).toIntAlgHom.range)
    hx

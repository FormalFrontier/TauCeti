...
/-!
# Uniqueness of the blocks in a Wedderburn presentation
...
This closes the product-level uniqueness target in Layer 2 of the
...
private noncomputable def supported (i : ι) (x : A i) : ∀ i, A i := by
  classical
  exact Pi.single i x

private noncomputable def coordinate (i : ι) : ∀ i, A i :=
  supported (A := A) i 1

omit [Finite ι] [∀ i, IsSimpleRing (A i)] in
@[simp]
private theorem supported_apply_same (i : ι) (x : A i) :
    supported (A := A) i x i = x := by
  classical
  exact Pi.single_eq_same i x

omit [Finite ι] [∀ i, IsSimpleRing (A i)] in
private theorem supported_apply_of_ne {i j : ι} (h : j ≠ i) (x : A i) :
    supported (A := A) i x j = 0 := by
  classical
  exact Pi.single_eq_of_ne h x

omit [Finite ι] [∀ i, IsSimpleRing (A i)] in
@[simp]
private theorem coordinate_apply_same (i : ι) : coordinate (A := A) i i = 1 :=
  supported_apply_same i 1

omit [Finite ι] [∀ i, IsSimpleRing (A i)] in
private theorem coordinate_apply_of_ne {i j : ι} (h : j ≠ i) :
    coordinate (A := A) i j = 0 :=
  supported_apply_of_ne h 1

omit [Finite ι] in
private theorem coordinate_ne_zero (i : ι) : coordinate (A := A) i ≠ 0 := by
  classical
  intro h
  have := congrFun h i
  simp [coordinate, supported] at this

omit [∀ i, IsSimpleRing (A i)] in
private theorem sum_coordinate :
    ∑ i : ι, coordinate (A := A) i = 1 := by
  classical
  ext j
  simp only [Finset.sum_apply, coordinate, supported, Pi.one_apply]
  exact Fintype.sum_pi_single j (fun i : ι ↦ (1 : A i))

omit [Finite ι] [∀ i, IsSimpleRing (A i)] in
private theorem coordinate_mul (i : ι) (x : A i) :
    coordinate (A := A) i * supported (A := A) i x = supported (A := A) i x := by
  classical
  ext j
  by_cases h : j = i
  · subst j
    rw [Pi.mul_apply, coordinate_apply_same, supported_apply_same, one_mul]
  · rw [Pi.mul_apply, coordinate_apply_of_ne h, supported_apply_of_ne h, zero_mul]

omit [Finite ι] [∀ i, IsSimpleRing (A i)] in
private theorem supported_add (i : ι) (x y : A i) :
    supported (A := A) i (x + y) = supported (A := A) i x + supported (A := A) i y := by
  classical
  ext j
  by_cases h : j = i
  · subst j
    simp
  · rw [Pi.add_apply, supported_apply_of_ne h, supported_apply_of_ne h, supported_apply_of_ne h,
      zero_add]

omit [Finite ι] [∀ i, IsSimpleRing (A i)] in
private theorem supported_mul (i : ι) (x y : A i) :
    supported (A := A) i (x * y) = supported (A := A) i x * supported (A := A) i y := by
  classical
  ext j
  by_cases h : j = i
  · subst j
    simp
  · rw [Pi.mul_apply, supported_apply_of_ne h, supported_apply_of_ne h, supported_apply_of_ne h,
      zero_mul]

omit [Finite ι] [∀ i, IsSimpleRing (A i)] in
private theorem coordinate_mem_centralIdempotents (i : ι) :
    coordinate (A := A) i ∈ centralIdempotents (∀ i, A i) := by
  rw [mem_centralIdempotents_pi]
  intro j
  by_cases h : j = i
  · subst j
    rw [coordinate_apply_same]
    exact one_mem_centralIdempotents
  · rw [coordinate_apply_of_ne h]
...
private theorem image_coordinate_eq_zero_or_one (i : ι) (j : κ) :
    f (coordinate (A := A) i) j = 0 ∨ f (coordinate (A := A) i) j = 1 := by
  have hmem : f (coordinate (A := A) i) j ∈ centralIdempotents (B j) :=
    (mem_centralIdempotents_pi B).mp
      (map_mem_centralIdempotents f (coordinate_mem_centralIdempotents i)) j
...
private theorem existsUnique_image_coordinate_eq_one (j : κ) :
    ∃! i : ι, f (coordinate (A := A) i) j = 1 := by
  classical
  have hsum : ∑ i : ι, f (coordinate (A := A) i) j = 1 := by
    rw [← Finset.sum_apply, ← map_sum, sum_coordinate, map_one]
    rfl
  have hex : ∃ i : ι, f (coordinate (A := A) i) j ≠ 0 := by
    by_contra h
  ...
  intro i' hi'
  by_contra hne
  have hmul : coordinate (A := A) i * coordinate (A := A) i' = 0 := by
    ext k
    by_cases hki : k = i
    · subst k
      rw [Pi.mul_apply, coordinate_apply_same, coordinate_apply_of_ne (Ne.symm hne), one_mul,
        Pi.zero_apply]
    · rw [Pi.mul_apply, coordinate_apply_of_ne hki, zero_mul, Pi.zero_apply]
...
private theorem sourceIndex_surjective : Function.Surjective (sourceIndex f) := by
  intro i
  have himage : f (coordinate (A := A) i) ≠ 0 := by
    rw [← map_zero f]
    exact f.injective.ne (coordinate_ne_zero i)
  obtain ⟨j, hj⟩ : ∃ j, f (coordinate (A := A) i) j ≠ 0 := by
...
private theorem image_coordinate_blockEquiv (i : ι) :
    f (coordinate (A := A) i) (blockEquiv f i) = 1 := by
...
private theorem map_single_eq_single (i : ι) (x : A i) :
    f (supported (A := A) i x) =
      supported (A := B) (blockEquiv f i) (f (supported (A := A) i x) (blockEquiv f i)) := by
  classical
  ext j
  by_cases h : j = blockEquiv f i
  · subst j
    rw [supported_apply_same]
  · have hzero := image_coordinate_eq_zero_of_ne f i h
    have hmul := congrFun (congrArg f (coordinate_mul (A := A) i x)) j
    rw [map_mul, Pi.mul_apply, hzero, zero_mul] at hmul
    rw [supported_apply_of_ne h]
    exact hmul.symm

private theorem symm_map_single_eq_single (i : ι) (y : B (blockEquiv f i)) :
    f.symm (supported (A := B) (blockEquiv f i) y) =
      supported (A := A) i (f.symm (supported (A := B) (blockEquiv f i) y) i) := by
  classical
  have hcoord : f (coordinate (A := A) i) = coordinate (A := B) (blockEquiv f i) := by
    rw [coordinate, map_single_eq_single]
    congr 1
    exact image_coordinate_blockEquiv f i
  ext k
  by_cases h : k = i
  · subst k
    rw [supported_apply_same]
  · have hzero : coordinate (A := A) i k = 0 := coordinate_apply_of_ne h
    have hmul : coordinate (A := B) (blockEquiv f i) *
        supported (A := B) (blockEquiv f i) y =
        supported (A := B) (blockEquiv f i) y := coordinate_mul (A := B) (blockEquiv f i) y
...
private noncomputable def factorRingEquiv (i : ι) : A i ≃+* B (blockEquiv f i) where
  toFun x := f (supported (A := A) i x) (blockEquiv f i)
  invFun y := f.symm (supported (A := B) (blockEquiv f i) y) i
  map_add' x y := by rw [supported_add, map_add]; rfl
  map_mul' x y := by rw [supported_mul, map_mul]; rfl
  left_inv x := by
    have h := congrFun (congrArg f.symm (map_single_eq_single f i x)) i
    rw [f.symm_apply_apply, supported_apply_same] at h
    exact h.symm
  right_inv y := by
    have h := congrFun (congrArg f (symm_map_single_eq_single f i y)) (blockEquiv f i)
    rw [f.apply_symm_apply, supported_apply_same] at h
    exact h.symm

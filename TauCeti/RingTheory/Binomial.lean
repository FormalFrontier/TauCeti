...
/-- Every generalized binomial coefficient in `r` belongs to its integral subring. -/
@[simp]
theorem ringChoose_mem_ringChooseSubring (r : R) (n : ℕ) :
    Ring.choose r n ∈ ringChooseSubring r :=
  ringChoose_mem_ringChooseSpan r n
...
theorem ringChooseSubring_le_iff {S : Subring R} {r : R} :
  ...
  constructor
  · intro h n
    exact h (ringChoose_mem_ringChooseSubring r n)
  · intro h

...
* `TauCeti.Ring.choose_add_intCast_mem_ringChooseSpan`: an integer translate
  `(r + z choose n)` lies in the integral span of the coefficients `(r choose k)`.
...
/-- An additive subgroup containing the generalized binomial coefficients of `r` up to degree `n`
contains the degree-`n` coefficient after translating `r` by an integer. -/
theorem choose_add_intCast_mem {A : AddSubgroup R} {r : R} {n : ℕ}
    (hr : ∀ k ≤ n, _root_.Ring.choose r k ∈ A) (z : ℤ) :
...
theorem choose_add_intCast_mem_ringChooseSpan (r : R) (z : ℤ) (n : ℕ) :
  ...
  exact choose_add_intCast_mem
    (A := ringChooseSpan r) (fun k _ => ringChoose_mem_ringChooseSpan r k) z
...
theorem ringChooseSpan_add_intCast (r : R) (z : ℤ) :
  ...
  · rw [ringChooseSpan_le_iff]
    exact TauCeti.Ring.choose_add_intCast_mem_ringChooseSpan r z
  · rw [ringChooseSpan_le_iff]
    intro n
    have h := TauCeti.Ring.choose_add_intCast_mem_ringChooseSpan (r + (z : R)) (-z) n
    simpa using h

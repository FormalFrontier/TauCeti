...
theorem ord_lt_ord_iff_valuation_lt (v : _root_.Valuation F ℤᵐ⁰) {f g : F} (hf : f ≠ 0)
...
private theorem valuation_sum_eq_of_forall_ord_lt (v : _root_.Valuation F ℤᵐ⁰) {ι : Type*}
    {s : Finset ι} {f : ι → F} {j : ι} (hj : j ∈ s) (hf : ∀ i ∈ s, f i ≠ 0)
    (hlt : ∀ i ∈ s, i ≠ j → ord v (f j) < ord v (f i)) :
    v (∑ i ∈ s, f i) = v (f j) := by
  classical
  refine v.map_sum_eq_of_lt hj fun i hi ↦ ?_
  rw [Finset.mem_sdiff, Finset.mem_singleton] at hi
  exact (ord_lt_ord_iff_valuation_lt v (hf j hj) (hf i hi.1)).mp (hlt i hi.1 hi.2)

/-- A finite sum one of whose summands has strictly least order does not vanish. -/
theorem sum_ne_zero_of_forall_ord_lt (v : _root_.Valuation F ℤᵐ⁰) {ι : Type*} {s : Finset ι}
    {f : ι → F} {j : ι} (hj : j ∈ s) (hf : ∀ i ∈ s, f i ≠ 0)
    (hlt : ∀ i ∈ s, i ≠ j → ord v (f j) < ord v (f i)) : ∑ i ∈ s, f i ≠ 0 :=
  v.ne_zero_iff.mp <| by
    rw [valuation_sum_eq_of_forall_ord_lt v hj hf hlt]
    exact v.ne_zero_iff.mpr (hf j hj)
...
`TauCeti.Valuation.ord_add_eq_min_of_ord_ne`; the hypothesis that every summand is nonzero
keeps the junk value `ord_v 0 = 0` out of the comparison. -/
theorem ord_sum_eq_of_forall_lt (v : _root_.Valuation F ℤᵐ⁰) {ι : Type*} {s : Finset ι}
    {f : ι → F} {j : ι} (hj : j ∈ s) (hf : ∀ i ∈ s, f i ≠ 0)
    (hlt : ∀ i ∈ s, i ≠ j → ord v (f j) < ord v (f i)) :
    ord v (∑ i ∈ s, f i) = ord v (f j) := by
  rw [ord_def, ord_def, valuation_sum_eq_of_forall_ord_lt v hj hf hlt]

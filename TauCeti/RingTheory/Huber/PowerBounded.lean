...
-- that `powerBoundedSubring` requires. Continuous multiplication is declared here because the
-- monic-polynomial results below use it through `isBounded_finite`; the closure lemmas omit it.

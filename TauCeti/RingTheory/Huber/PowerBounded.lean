...
* `TauCeti.Huber.isBounded_submonoidClosure` and `TauCeti.Huber.isBounded_subringClosure`
  ...
  power-bounded elements is bounded.
...
variable {A : Type*} [CommMonoidWithZero A] [TopologicalSpace A] [ContinuousMul A]
...
-- These need only a nonarchimedean additive group with continuous multiplication, not the full
-- `NonarchimedeanRing` structure that `powerBoundedSubring` requires.
variable {A : Type*} [CommRing A] [TopologicalSpace A] [NonarchimedeanAddGroup A] [ContinuousMul A]

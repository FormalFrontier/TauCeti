...
This file proves the part of Wedhorn's Corollary 6.4 which identifies the power-bounded
elements of a Huber ring with the union of its rings of definition. The key construction adjoins
a finite family of power-bounded elements to a chosen ring of definition.

If `(A₀, I)` is a pair of definition and `T ⊆ A` is a finite set of power-bounded elements, then
`A₀[T]` is bounded. The image of `I` in `A₀[T]` consequently defines its subspace topology, so
`A₀[T]` is again a ring of definition. Taking `T = {a}` proves that every power-bounded element
belongs to some ring of definition; the converse follows because every ring of definition is
bounded.
...
private theorem mul_mem_addSubgroupClosure_mul_range_pow (B : Subring A) (a : A) {x y : A}
  ...
  | mem u v hu hv =>
    ...
    exact Set.mul_mem_mul (B.mul_mem hb₁ hb₂) ⟨k₁ + k₂, rfl⟩
  | zero_left => simp
  | zero_right => simp
...
/-- Adjoining finitely many power-bounded elements to a ring of definition gives a bounded
subring. This is the boundedness input in Wedhorn's Corollary 6.4. -/
theorem isBounded_adjoinRing (P : PairOfDefinition A) (T : Finset A)
    (hT : ∀ t ∈ T, IsPowerBounded t) :
    IsBounded (Subring.closure ((P.ringOfDefinition : Set A) ∪ (T : Set A)) : Set A) := by
  classical
  induction T using Finset.induction with
  | empty =>
    simpa [Subring.closure_eq] using P.isBounded_ringOfDefinition
  | @insert a S ha ih =>
    have hS : ∀ t ∈ S, IsPowerBounded t :=
      fun t ht ↦ hT t (Finset.mem_insert_of_mem ht)
    specialize ih hS
    let B := Subring.closure ((P.ringOfDefinition : Set A) ∪ (S : Set A))
      ...
      { carrier := AddSubgroup.closure M
        ...
        one_mem' := by
          ...
          exact AddSubgroup.subset_closure <|
            Set.mul_mem_mul B.one_mem ⟨(0 : ℕ), rfl⟩
    ...
    have hgen : (P.ringOfDefinition : Set A) ∪ (insert a S : Finset A) ⊆ (C : Set A) := by
      ...
      · rw [hxa, show a = 1 * a ^ 1 by simp]
        exact AddSubgroup.subset_closure <| Set.mul_mem_mul B.one_mem ⟨(1 : ℕ), rfl⟩
      · rw [show x = x * a ^ 0 by simp]
        exact AddSubgroup.subset_closure <| Set.mul_mem_mul
...
def adjoin (P : PairOfDefinition A) (T : Finset A) (hT : ∀ t ∈ T, IsPowerBounded t) :
  ...
  ringOfDefinition := Subring.closure ((P.ringOfDefinition : Set A) ∪ (T : Set A))
  isOpen_ringOfDefinition := AddSubgroup.isOpen_of_mem_nhds
      (Subring.closure ((P.ringOfDefinition : Set A) ∪ (T : Set A))).toAddSubgroup <|
    mem_of_superset
      (P.isOpen_ringOfDefinition.mem_nhds P.ringOfDefinition.zero_mem) (P.le_adjoinRing T)
  idealOfDefinition := P.idealOfDefinition.map (Subring.inclusion (P.le_adjoinRing T))
  fg_idealOfDefinition := P.fg_idealOfDefinition.map _
  isAdic_idealOfDefinition := by
    let B := Subring.closure ((P.ringOfDefinition : Set A) ∪ (T : Set A))
    let incl := Subring.inclusion (P.le_adjoinRing T)
    ...
    · intro s hs
      ...
      obtain ⟨V, hV, hVB⟩ := isBounded_iff.mp (P.isBounded_adjoinRing T hT) (G : Set A)
      ...
      rw [show ((∑ i ∈ f.support, f i * i : B) : A) =
          ∑ i ∈ f.support, ((f i * i : B) : A) from map_sum B.subtype _ _]
...
theorem adjoin_idealOfDefinition (P : PairOfDefinition A) (T : Finset A)
    ...
    (P.adjoin T hT).idealOfDefinition =
      P.idealOfDefinition.map (Subring.inclusion (P.le_adjoin T hT)) := (rfl)

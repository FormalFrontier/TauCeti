...
This is `TauCeti.eval_diagramSchurPoly_of_apply_last_eq_one`.  Read through the characters of the
general linear group, it is the multiplicity-free branching rule for the restriction
`GLₙ₊₁ ↓ GLₙ`: the irreducible of highest weight `μ` restricts to the direct sum, each with
multiplicity one, of the irreducibles whose highest weights interlace `μ`.
...
Through the characters of the general linear group this is the multiplicity-free branching rule

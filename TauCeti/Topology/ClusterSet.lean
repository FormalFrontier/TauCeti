...
/-!
# Cluster sets, continuous extensions, and inverse maps
...
Cluster-set membership is also reciprocal for maps that are inverse on their domains: the same
nearby pairs witness membership for the forward map and its inverse. This gives an injectivity
criterion for continuous extensions whose inverse cluster sets are subsingletons.
...
* `TauCeti.clusterSetOn_eq_singleton_of_tendsto`,
  `TauCeti.clusterSetOn_eq_singleton_of_continuousWithinAt`,
  `TauCeti.mem_clusterSetOn_of_continuousWithinAt_extension`,
  `TauCeti.clusterSetOn_eq_singleton_of_continuousWithinAt_extension` and
...
If `f` maps `U` into `V` and `g` is a left inverse there, every forward cluster value produces an
inverse cluster value by reading the same nearby pairs in reverse. Mutual inverses therefore give
an equivalence of cluster-set membership. No continuity is needed, so the results remain at the
generality of arbitrary topological spaces. -/
...
Left-inverse cluster transfer puts each `x` in the inverse cluster set at `F x`; hence two points of
`A` with the same `F`-value lie in one subsingleton inverse cluster set. No continuity or separation
hypothesis is needed. -/
theorem injOn_of_clusterSetOn_leftInvOn_subsingleton {A : Set X} {B : Set Y}
    {F : X → Y} (hmem : ∀ x ∈ A, F x ∈ clusterSetOn f U x) (hFB : MapsTo F A B)
    (hgf : LeftInvOn g f U) (hf : MapsTo f U V)
    (hsub : ∀ y ∈ B, (clusterSetOn g V y).Subsingleton) :
    InjOn F A := by
  intro x hx y hy hxy
  have hxinv : x ∈ clusterSetOn g V (F x) :=
    mem_clusterSetOn_of_leftInvOn hgf hf (hmem x hx)
  have hyinv : y ∈ clusterSetOn g V (F x) := by
    simpa only [hxy] using mem_clusterSetOn_of_leftInvOn hgf hf (hmem y hy)
...
`B` at whose points the cluster sets of a left inverse `g : V → U` are subsingletons. Then `F` is
injective on `A`. -/
theorem injOn_of_clusterSetOn_leftInvOn_subsingleton_of_continuousOn_extension

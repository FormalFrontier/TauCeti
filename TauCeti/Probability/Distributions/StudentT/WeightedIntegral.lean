...
itself; the moment evaluations live in `Moments.lean` and the tail/cdf evaluation in `Cdf.lean`.
-/
...
lemma injOn_sq_div_const_Ioi (hν : 0 < ν) :
...
lemma abs_deriv_smul_studentTPDFReal (hν : 0 < ν) (q : ℝ) {z : ℝ} (hz : 0 < z) :
  ...
  have hpow : z ^ (q - 1) * z = z ^ q := by
    ...
    have h : z ^ (q - 1) * z = z ^ (q - 1) * z ^ (1 : ℝ) := by
      rw [Real.rpow_one]
    rw [h, ← Real.rpow_add hzpos]; congr 1; ring

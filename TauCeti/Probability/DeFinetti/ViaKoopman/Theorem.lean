...
* `conditionallyIIDWith_invariantConditional_pathLaw` — the path-space form, with its witness;
...
theorem conditionallyIIDWith_invariantConditional_pathLaw [StandardBorelSpace α] [Nonempty α]
...
theorem conditionallyIID_of_contractable_viaKoopman [StandardBorelSpace α] [Nonempty α]
  ...
  exact ConditionallyIID.of_directing (ConditionallyIIDWith.of_pathLaw hX_meas
    (conditionallyIIDWith_invariantConditional_pathLaw (hX.contractableLaw_pathLaw hXae)))

...
theorem finrank_quotient_eq_one_iff {I : Ideal W.CoordinateRing} :
  ...
  · intro hdeg
    ...
    have hcomp : ∀ p : F[X][Y], ρ (CoordinateRing.mk W p) = p.evalEval x₀ y₀ := fun p ↦ by
      simpa [x₀, y₀] using _root_.WeierstrassCurve.Affine.CoordinateRing.algHom_mk_eq_evalEval ρ p
    have heq : W.Equation x₀ y₀ := by
      simpa [x₀, y₀] using _root_.WeierstrassCurve.Affine.CoordinateRing.equation_of_algHom ρ

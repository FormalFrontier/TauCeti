...
* `WeierstrassCurve.Affine.algebraMap_polynomial_eq_aeval_genericX`: the image of `R[X]` in the
  function field is the polynomials in the generic `x`-coordinate.
* `WeierstrassCurve.Affine.algebraMap_mem_adjoin_generic`: the coordinate ring, read inside the
...
`x`-coordinate.** -/
theorem algebraMap_polynomial_eq_aeval_genericX (p : R[X]) :
...
/-- **The coordinate ring, read inside the function field, is generated by the generic point.**
Every affine function of `W` is a polynomial in `x` and `y` over `R`, so its image lies in the
`R`-subalgebra `R[x, y]` of `R(W)`. The two coordinates are all that is needed: `R[W]` is free of
rank two over `R[X]` with basis `{1, y}`. -/
theorem algebraMap_mem_adjoin_generic (z : W.CoordinateRing) :
  ...
  have hgen (r : R[X]) : algebraMap R[X] W.FunctionField r ∈
    ...
    have hr : aeval W.genericX r ∈ Algebra.adjoin R ({W.genericX} : Set W.FunctionField) := by
      ...
      exact ⟨r, rfl⟩
    rw [algebraMap_polynomial_eq_aeval_genericX]

...
`Finrank.lean`, so that the statement serves `RatFunc F` and `FractionRing F[X]` alike. Being a
fraction field of `F[X]` is not needed: only the algebra structures are.
...
Both halves come from elsewhere. The coordinate-ring containment is
`algebraMap_mem_adjoin_genericX_genericY` of `Affine/FunctionField/GenericPoint.lean`, which proves
it for the `F`-subalgebra generated by both coordinates. The fraction-field step is Mathlib's
...
their combination, and the observation that it needs no hypothesis on `L` beyond the algebra
structures.
-/
...
variable {F : Type*} [CommRing F] [IsDomain F] (W' : _root_.WeierstrassCurve.Affine F)
  (L : Type*) [Field L] [Algebra F[X] L] [Algebra F L] [Algebra L W'.FunctionField]
  [IsScalarTower F[X] L W'.FunctionField] [IsScalarTower F L W'.FunctionField]
...
theorem algebraMap_mem_adjoin_genericY (w : W'.CoordinateRing) :
  ...
  refine Algebra.adjoin_le (S := (IntermediateField.adjoin L
    {W'.genericY}).toSubalgebra.restrictScalars F) ?_ (algebraMap_mem_adjoin_genericX_genericY W' w)
  rintro _ (rfl | rfl)
  · rw [genericX_eq_algebraMap, IsScalarTower.algebraMap_apply F[X] L W'.FunctionField]

...
and `WeierstrassCurve.Affine.algebraMap_mem_adjoin_genericPoint` — the coordinate ring, inside the
function field, lies in the subalgebra they generate — propagates that to the whole coordinate
ring.
...
@[simp]
theorem map_id_eq (φ : CoordinatePullback W₁ W₂) : φ.map (RingHom.id F) = φ := by
  apply ext_coords
...
theorem MapsInfinity.map {φ : CoordinatePullback W₁ W₂} (hφ : φ.MapsInfinity) (f : F →+* K) :
  ...
  exact fun z ↦ hle (algebraMap_mem_adjoin_genericPoint (W₁.map f) z)
...
@[simp]
theorem map_fieldPullback (φ : Isogeny W₁ W₂) (f : F →+* K) (z : W₂.FunctionField) :
...
theorem comp_map (ψ : Isogeny W₂ W₃) (φ : Isogeny W₁ W₂) (f : F →+* K) :
  ...
  · simp only [map_pullback, comp_pullback, AlgHom.comp_apply,
    ...
    exact (map_fieldPullback φ f _).symm
...
theorem map_id_eq (φ : Isogeny W₁ W₂) : φ.map (RingHom.id F) = φ :=
  Isogeny.ext <| by
    rw [map_pullback]
    exact CoordinatePullback.map_id_eq φ.pullback

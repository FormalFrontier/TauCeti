...
theorem ext_coords {φ ψ : CoordinatePullback W₁ W₂}
    ...
    (hY : φ (AdjoinRoot.root W₂.polynomial) = ψ (AdjoinRoot.root W₂.polynomial)) : φ = ψ := by
  apply WeierstrassCurve.Affine.CoordinateRing.algHom_ext_coords hX
  simpa only [AdjoinRoot.mk_X] using hY

...
The universal property is not decoration: it is how the pullbacks given by a point are built and
moved. `Isogeny/MulByInt/Basic.lean` makes `[n]` from the division-polynomial point, and
`Isogeny/BaseChange.lean` carries a pullback along a change of base field by carrying its point.
The identity and the Frobenius pullbacks instead come directly from their underlying ring maps.
...
theorem apply_mk (φ : CoordinatePullback W₁ W₂) (p : F[X][Y]) :
  ...
  simpa only [AdjoinRoot.mk_C, AdjoinRoot.mk_X] using
    WeierstrassCurve.Affine.CoordinateRing.algHom_mk_eq_evalEval φ p
...
theorem ext_coords {φ ψ : CoordinatePullback W₁ W₂}
  ...
  WeierstrassCurve.Affine.CoordinateRing.algHom_ext hX hY
...
theorem equation_apply (φ : CoordinatePullback W₁ W₂) :
    (W₂.map (algebraMap F W₁.FunctionField)).Equation
      (φ (AdjoinRoot.of W₂.polynomial X)) (φ (AdjoinRoot.root W₂.polynomial)) :=
  WeierstrassCurve.Affine.CoordinateRing.equation_of_algHom φ

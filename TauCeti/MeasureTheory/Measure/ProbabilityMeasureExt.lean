...
* `Measure.exists_eq_dirac_probabilityMeasure` — a zero-one probability law on
...
/-- A zero-one probability law on `ProbabilityMeasure α` is a Dirac measure when the measurable
...
theorem Measure.exists_eq_dirac_probabilityMeasure [CountablyGenerated α]
    {π : Measure (ProbabilityMeasure α)} [IsProbabilityMeasure π] [IsZeroOneMeasure π] :
  ...
  have hgen : (inferInstance : MeasurableSpace α) = generateFrom 𝒜 := by
    rw [show 𝒜 = generateSetAlgebra (countableGeneratingSet α) by rfl,
      generateFrom_generateSetAlgebra_eq, generateFrom_countableGeneratingSet]
  have hmeas : ∀ s ∈ 𝒜, MeasurableSet s := by
    intro s hs
    change (inferInstance : MeasurableSpace α).MeasurableSet' s
    rw [hgen]

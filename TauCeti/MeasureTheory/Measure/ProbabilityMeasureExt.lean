...
* `measurable_probabilityMeasure_apply_real` and `measurable_probabilityMeasure_eval_family` —
  measurability of the real-valued evaluations, singly and as a finite family. These are the
  hypotheses any consumer of the extensionality theorem has to discharge to speak about those
  pushforwards at all, so they are public rather than internal.
...
function of the measure. The Giry σ-algebra is generated by the `ℝ≥0∞`-valued evaluations, and
`ENNReal.toReal` is measurable. -/
...
/-- A finite family of evaluations is measurable into the product. -/
theorem measurable_probabilityMeasure_eval_family {n : ℕ}
    (B : Fin n → Set α) (hB : ∀ i, MeasurableSet (B i)) :
...
private theorem measurable_evalReal_apply (s : MeasIdx α) :
  ...
  measurable_probabilityMeasure_apply_real s.2

...
the `n`-punctured disc, and a choice of basis turns that action into a matrix representation over
the ring of Laurent polynomials: the *Burau representation*. This file constructs it over an
arbitrary commutative ring `R` and an arbitrary unit `t : Rˣ`, so the Laurent-polynomial case is
the instance `R = ℤ[T;T⁻¹]`, `t = T`.
...
* `TauCeti.KnotTheory.coe_burau_one`: at `t = 1` the Burau representation is the permutation
  representation of the strands.
...
theorem burauRow_dotProduct_burauCol_of_dist (t : R) {i j : Fin (n - 1)}
...
theorem burauMatrix_mul_comm (t : R) {i j : Fin (n - 1)}
  ...
  rw [burauMatrix_mul_burauMatrix, burauMatrix_mul_burauMatrix,
    burauRow_dotProduct_burauCol_of_dist t h, burauRow_dotProduct_burauCol_of_dist t h.symm]
...
theorem det_burauMatrix (t : R) (i : Fin (n - 1)) : (burauMatrix t i).det = -t := by
  have h : burauMatrix t i = 1 + vecMulVec (-burauCol t i) (burauRow R i) := by
    refine Matrix.ext fun a b => ?_
    rw [burauMatrix_apply, Matrix.add_apply, vecMulVec_apply, Matrix.one_apply, Pi.neg_apply]
    ring
...
/-- The matrix form of `TauCeti.KnotTheory.burau_sigma`. -/
theorem coe_burau_sigma (t : Rˣ) (i : Fin (n - 1)) :
    (burau n t (BraidGroup.sigma i) : Matrix (Fin n) (Fin n) R) = burauMatrix (t : R) i := by
  rw [burau_sigma, coe_burauGL]
...
theorem burau_mulVec_of_forall {t : Rˣ} {v : Fin n → R}
  ...
  refine BraidGroup.sigma_induction_on
    ...
    (fun i => by rw [coe_burau_sigma]; exact h i) ?_ ?_ ?_

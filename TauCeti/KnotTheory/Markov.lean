...
endpoint at the bottom, presents a link. Two braids, possibly on different numbers of strands,
close to isotopic links exactly when they are related by a chain of the two *Markov moves*:
...
This file introduces the braids-of-any-width that the moves act on, the moves themselves, the
equivalence relation `TauCeti.MarkovEquiv` they generate, and the first invariant of that
relation: the number of components of the closure.
...
presentation of a knot -- "Markov moves (braids)" -- and for "braid-to-diagram via Markov" as an
edge of the spanning tree of presentations.
-/
...
/-- **Markov equivalence**: the equivalence relation generated by the Markov moves. By Markov's
theorem (not formalized here) two braids are Markov equivalent exactly when their closures are
isotopic links. -/

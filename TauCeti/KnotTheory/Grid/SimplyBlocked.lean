...
which cover neither an `X`-marking nor the blocked `O`-marking.
...
* `TauCeti.GridChainHat`: the free module on grid states over the remaining variables.
...
/-- The simply blocked grid chain module, freely generated by grid states over the polynomial
ring in the columns other than the blocked column. -/
...
/-- The simply blocked grid differential, linear over the polynomial ring in the unblocked
columns. -/
noncomputable def simplyBlockedDifferential (i : Fin n) :
    GridChainHat R n i →ₗ[MvPolynomial (SimplyBlockedVariable i) R] GridChainHat R n i :=
...
@[simp]
theorem simplyBlockedDifferential_apply_apply (i : Fin n) (c : GridChainHat R n i)
    (y : GridState n) :

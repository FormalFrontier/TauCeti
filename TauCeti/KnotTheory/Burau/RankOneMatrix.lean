...
/-!
# Rank-one matrix families and braid-group representations

This file develops the matrix calculus for families of the form `1 - u i ⊗ v i`. It gives their
products, braid and quadratic relations, inverses, determinants, and the braid-group representation
defined by a family whose pairings have the values occurring in the Burau representation.

The generic API is used in `TauCeti/KnotTheory/Burau/Reduced.lean` to construct the reduced Burau
representation from the pairings between the Burau rows and columns.

## Main definitions

* `TauCeti.KnotTheory.RankOneMatrix.family`: the family of matrices `1 - u i ⊗ v i`.
* `TauCeti.KnotTheory.RankOneMatrix.unit`: a member of the family as an element of the general
  linear group when its self-pairing has the Burau value.
* `TauCeti.KnotTheory.RankOneMatrix.representation`: the braid-group representation defined by a
  family with the Burau pairings.

## Main results

* `TauCeti.KnotTheory.RankOneMatrix.family_mul_comm` and
  `TauCeti.KnotTheory.RankOneMatrix.family_braid_or`: the braid relations.
* `TauCeti.KnotTheory.RankOneMatrix.family_mul_self`: the quadratic relation.
* `TauCeti.KnotTheory.RankOneMatrix.det_representation`: the determinant character of the
  representation.
-/

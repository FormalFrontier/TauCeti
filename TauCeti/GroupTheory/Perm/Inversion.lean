...
sign of `σ` is `(-1)` raised to the number of inversions. Mathlib defines
`Equiv.Perm.signAux` by exactly this parity but keeps it an implementation detail of
`Equiv.Perm.sign`, exposing only the product form
`Equiv.Perm.sign_eq_prod_prod_Ioi`; this file turns that product into the cardinality of the
finite set of inversions, which is the form a combinatorial count of permutations meets it in.
...
theorem sign_eq_neg_one_pow_card_inversion (σ : Equiv.Perm (Fin n)) :
    Equiv.Perm.sign σ =
      (-1 : ℤˣ) ^ (Finset.univ.filter fun p : Fin n × Fin n => p.1 < p.2 ∧ σ p.2 < σ p.1).card := by
  classical

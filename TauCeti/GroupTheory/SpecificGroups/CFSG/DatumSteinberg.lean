...
`TauCeti.ValidLieTypeIndex.datumFrobenius` is the root-datum image of `Frob_q`, and
`TauCeti.GraphTwistedIndex.datumGraphAut` is the root-datum image of `γ` in the orientation fixed
below.
...
@[simp] theorem datumSteinberg_eq_datumFrobenius_iff :
  ...
  rw [datumSteinberg_def, ValidLieTypeIndex.datumFrobenius_def,
    RootPairingIsogeny.comp_ofEquiv_smulId_eq_iff, datumGraphAut_def]

...
this count, while `TauCeti.Sporadic.automorphismRelators_even` checks that every source relator
...
power. Its public body lets the transcription checks reduce it to signed letters. -/
@[expose]
def fi24SourceEquationWord : Relator (Fin 12) :=
  .gen 0 ⬝ .gen 1 ⬝ .gen 2 ⬝ .gen 3 ⬝ .gen 4 ⬝ .gen 5 ⬝ .gen 7

/-- The fourteen-letter word in the source's final relation. Its public body lets the
transcription checks reduce it to signed letters. -/
@[expose]
...
private theorem length_sourceEquationWord : fi24SourceEquationWord.toWord.length = 7 := by
  simp [fi24SourceEquationWord]

private theorem length_sourceLongWord : fi24SourceLongWord.toWord.length = 14 := by
  simp [fi24SourceLongWord]

private theorem length_sourceEquationRelator : fi24SourceEquationRelator.toWord.length = 64 := by
  simp [fi24SourceEquationRelator, Relator.div, length_sourceEquationWord]

private theorem length_sourceLongRelator : fi24SourceLongRelator.toWord.length = 238 := by
  simp [fi24SourceLongRelator, length_sourceLongWord]
...
theorem fi24AutomorphismRelators_even :
  ...
  · simp only [fi24AutomorphismAdditionalRelators, List.mem_cons, List.not_mem_nil,
    ...
    · rw [length_sourceEquationRelator]
      exact even_iff_two_dvd.mpr (by norm_num)
    · rw [length_sourceLongRelator]
      exact even_iff_two_dvd.mpr (by norm_num)
...
private def targetGenerator (i : Fin 12) (_h : i.val ≠ 0) : Fin 11 :=
  ⟨i.val - 1, by omega⟩
...
-/
private def schreierFactors : Bool → PresentationWord (Fin 12) → List (Relator (Fin 11))
  | _, [] => []
  | positive, (i, _) :: w =>
      let rest := schreierFactors (!positive) w
      if h : i.val = 0 then rest
      else
        let g : Relator (Fin 11) := .gen (targetGenerator i h)
...
private def schreierRewrite (positive : Bool) (r : Relator (Fin 12)) : Relator (Fin 11) :=
  relatorOfFactors (schreierFactors positive r.toWord)

/-- The two Reidemeister--Schreier relators contributed by one source relator. -/
private def schreierRelators (r : Relator (Fin 12)) : List (Relator (Fin 11)) :=
  [schreierRewrite false r, schreierRewrite true r]
...
def fi24PrimeRelators : List (Relator (Fin 11)) :=
  (fi24SourcePairRelators ++ fi24AutomorphismAdditionalRelators).flatMap schreierRelators
...
theorem length_fi24PrimeRelators : fi24PrimeRelators.length = 136 := by
  simp [fi24PrimeRelators, schreierRelators, length_fi24SourcePairRelators,
    fi24AutomorphismAdditionalRelators]

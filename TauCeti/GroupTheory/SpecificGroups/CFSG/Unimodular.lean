...
underlying diagram is one of those three, and gives them the group of points of that carrier, its
numbered root subgroups, and its Frobenius. The group is called
`TauCeti.UnimodularLieIndex.GeckGroup`, after the construction it is, and not `AmbientGroup`:
milestone L0 of `TauCetiRoadmap/CFSGStatement/README.md` asks for an ambient group whose root datum
is `TauCeti.DynkinType.simplyConnectedRootDatum` with its pinning, and until the Layer 9
identification supplying that arrives nothing here is offered as the L0 carrier. Six of the
seventeen Lie-type constructors qualify, and they are of two kinds:
...
this file; what they gain here is the group it will be an endomorphism of, and the Frobenius its
square is to be compared with.
...
Nothing here asserts that a constructed group is finite, perfect, or simple, that the Geck carrier
is reductive, that its root datum is the simply connected one, or that its weight torus is maximal;
those are outside this roadmap. The `Aₙ`, `²Aₙ`, `Bₙ`, `Cₙ`, `Dₙ`, `²Dₙ`, `E₆`, `²E₆`, `E₇`, `³D₄`
and `²B₂` families need a carrier built from a full-weight representation of their own, so this
file covers six of the seventeen Lie-type constructors of `TauCeti.LieTypeIndex` and no more.
...
* `TauCeti.LieTypeIndex.HasUnimodularDiagram`: the indices whose underlying Dynkin diagram has
  ...
  and `tits`.
* `TauCeti.UnimodularLieIndex.GeckGroup`: the points of the Geck carrier of the underlying Dynkin
  type over the algebraic closure of the prime field.
* `TauCeti.UnimodularLieIndex.rootSubgroup`: the numbered root subgroups of that group.
* `TauCeti.UnimodularLieIndex.frobenius`: its `q`-power Frobenius, for `q` the field order recorded
  by the index.
...
* `TauCeti.UnimodularLieIndex.frobenius_rootSubgroup`: the Frobenius raises the parameter of every
  ...
  `TauCeti.UnimodularExceptionalIndex.steinberg_rootSubgroup`, the shape of the equation milestone
  L1 asks of them, proved on this carrier.
...
its root subgroups. **This file does not close L0 on any branch, and `GeckGroup` is not offered as
its carrier.** The pinned Chevalley--Demazure group, its root datum and its pinning are a Layer 9
target of `TauCetiRoadmap/ReductiveGroups/README.md` that the CFSG roadmap consumes rather than
builds, and until that layer identifies the Geck carrier with it -- or supplies a different carrier
outright -- what stands here is a concrete matrix group with full character span, numbered root
subgroups and a Frobenius, on the six branches where the lattice condition permits it.

Against that group, `steinberg` is the map `Frob_q` and `Group` the composite
...
and not the Steinberg map, which is what makes it the hypothesis the carrier needs. -/
...
/-- **The points of the Geck carrier of an index with unimodular diagram**, taken over the
algebraic closure of its prime field: a subgroup of `GLₙ`, generally infinite, carrying the
Bourbaki-numbered root subgroups and weight torus that carrier comes with.
...
def rootSubgroup (i : Fin d.dynkinType.rank ⊕ Fin d.dynkinType.rank) :
    Multiplicative d.1.Closure →* GeckGroup d :=
...
def frobenius : GeckGroup d →* GeckGroup d :=
...
power. -/
@[simp]
theorem coe_frobenius_apply (g : GeckGroup d)
...
the group of points of the Geck carrier whose entries lie in `𝔽_q`.
...
theorem mem_fixedSubgroup_frobenius_iff (g : GeckGroup d) :
...
def steinberg : UnimodularLieIndex.GeckGroup d.1 →* UnimodularLieIndex.GeckGroup d.1 :=
  d.1.frobenius
...
power. -/
@[simp]
theorem coe_steinberg_apply (g : UnimodularLieIndex.GeckGroup d.1)
...
the annotation. -/
theorem mem_fixedSubgroup_steinberg_iff (g : UnimodularLieIndex.GeckGroup d.1) :

...
on this diagram and would use the same carrier and the same Frobenius: `TauCeti.TypeBTwoLieIndex`
supplies its validated index subtype, but attaching the L0 carrier and its Steinberg map is separate
work.

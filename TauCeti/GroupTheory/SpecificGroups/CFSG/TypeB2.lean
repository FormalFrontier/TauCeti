...
and never the identification itself. The carrier is the Chevalley construction over `ℤ` run on the
Bourbaki-numbered type-`C` Chevalley generators and the standard admissible lattice: it is the
closed subgroup scheme of `GL_4` generated by the divided-power root subgroups and the full-weight
torus, `TauCeti.UniversalEnvelopingAlgebra.kostantToralGroupScheme`, which
`TauCeti/Algebra/Lie/Symplectic/StandardCarrier/Basic.lean` states it advances the Layer 9
Chevalley--Demazure, pinning and root-subgroup targets with. Its numbered root subgroups and its
full-weight torus are closed subgroup schemes, `TauCeti.SpStd.isClosedImmersion_rootSubgroup` and
...
lattice condition that separates the simply connected form from the adjoint one; and the pinning
equation is proved, conjugation by the torus rescaling the `k`-th root subgroup through that
subgroup's root character, `TauCeti.SpStd.weightTorus_conj_rootSubgroup` on scheme points and
`TauCeti.SpStd.torusPoints_conj_rootSubgroupParam` on the points of a value algebra. Its
datum-named form `TauCeti.SpStd.weightTorus_conj_rootSubgroup_root_simpleIndex` carries the
hypothesis `(C (n + 1)).Valid`, so at the rank-two member it says nothing, `C 2` not being a valid
Dynkin type; what reads the exponent of the equation in the `B₂` datum there is
`TauCeti.SpStd.rootGeneratorWeight_inl_eq_root_simpleIndex_B_two`, from which
`rootGeneratorWeight_carrierNode_eq_root_simpleIndex` below is proved.

Absent is the Layer 9 isomorphism theorem for pinned groups, which is what would turn that data into
an identification with the Chevalley--Demazure group scheme of
`TauCeti.DynkinType.simplyConnectedRootDatum`. Until it arrives every declaration below is stated of
`TauCeti.SpStd.groupScheme 1`, is true of it, and asserts nothing of any other group.

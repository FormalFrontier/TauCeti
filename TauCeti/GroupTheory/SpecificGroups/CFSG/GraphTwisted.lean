...
@[simp] theorem diagramPerm_D (hv : (LieTypeIndex.D n q).Valid)
    (hh : ¬(LieTypeIndex.D n q).UsesHalfFrobenius) : diagramPerm ⟨⟨_, hv⟩, hh⟩ = 1 := by
  simp only [diagramPerm]

@[simp] theorem twistOrder_D (hv : (LieTypeIndex.D n q).Valid)
    (hh : ¬(LieTypeIndex.D n q).UsesHalfFrobenius) : twistOrder ⟨⟨_, hv⟩, hh⟩ = 1 := by
  simp only [twistOrder]
...
@[simp] theorem diagramPerm_E6 (hv : (LieTypeIndex.E6 q).Valid)
    (hh : ¬(LieTypeIndex.E6 q).UsesHalfFrobenius) : diagramPerm ⟨⟨_, hv⟩, hh⟩ = 1 := by
  simp only [diagramPerm]

@[simp] theorem twistOrder_E6 (hv : (LieTypeIndex.E6 q).Valid)
    (hh : ¬(LieTypeIndex.E6 q).UsesHalfFrobenius) : twistOrder ⟨⟨_, hv⟩, hh⟩ = 1 := by
  simp only [twistOrder]
...
@[simp] theorem diagramPerm_E7 (hv : (LieTypeIndex.E7 q).Valid)
    (hh : ¬(LieTypeIndex.E7 q).UsesHalfFrobenius) : diagramPerm ⟨⟨_, hv⟩, hh⟩ = 1 := by
  simp only [diagramPerm]

@[simp] theorem twistOrder_E7 (hv : (LieTypeIndex.E7 q).Valid)
    (hh : ¬(LieTypeIndex.E7 q).UsesHalfFrobenius) : twistOrder ⟨⟨_, hv⟩, hh⟩ = 1 := by
  simp only [twistOrder]

/-- The family `E₈(q)` is not graph-twisted; the `E₈` diagram has no nontrivial symmetry. -/
@[simp] theorem diagramPerm_E8 (hv : (LieTypeIndex.E8 q).Valid)
    (hh : ¬(LieTypeIndex.E8 q).UsesHalfFrobenius) : diagramPerm ⟨⟨_, hv⟩, hh⟩ = 1 := by
  simp only [diagramPerm]

@[simp] theorem twistOrder_E8 (hv : (LieTypeIndex.E8 q).Valid)
    (hh : ¬(LieTypeIndex.E8 q).UsesHalfFrobenius) : twistOrder ⟨⟨_, hv⟩, hh⟩ = 1 := by
  simp only [twistOrder]
...
@[simp] theorem diagramPerm_F4 (hv : (LieTypeIndex.F4 q).Valid)
    (hh : ¬(LieTypeIndex.F4 q).UsesHalfFrobenius) : diagramPerm ⟨⟨_, hv⟩, hh⟩ = 1 := by
  simp only [diagramPerm]

@[simp] theorem twistOrder_F4 (hv : (LieTypeIndex.F4 q).Valid)
    (hh : ¬(LieTypeIndex.F4 q).UsesHalfFrobenius) : twistOrder ⟨⟨_, hv⟩, hh⟩ = 1 := by
  simp only [twistOrder]
...
@[simp] theorem diagramPerm_G2 (hv : (LieTypeIndex.G2 q).Valid)
    (hh : ¬(LieTypeIndex.G2 q).UsesHalfFrobenius) : diagramPerm ⟨⟨_, hv⟩, hh⟩ = 1 := by
  simp only [diagramPerm]

@[simp] theorem twistOrder_G2 (hv : (LieTypeIndex.G2 q).Valid)
    (hh : ¬(LieTypeIndex.G2 q).UsesHalfFrobenius) : twistOrder ⟨⟨_, hv⟩, hh⟩ = 1 := by
  simp only [twistOrder]
...
@[simp] theorem diagramPerm_twistedA (hv : (LieTypeIndex.twistedA n q).Valid)
    (hh : ¬(LieTypeIndex.twistedA n q).UsesHalfFrobenius) :
    diagramPerm ⟨⟨_, hv⟩, hh⟩ = graphPermA n := by
  simp only [diagramPerm]
  rfl

@[simp] theorem twistOrder_twistedA (hv : (LieTypeIndex.twistedA n q).Valid)
    (hh : ¬(LieTypeIndex.twistedA n q).UsesHalfFrobenius) : twistOrder ⟨⟨_, hv⟩, hh⟩ = 2 := by
  simp only [twistOrder]
...
@[simp] theorem twistOrder_twistedD (hv : (LieTypeIndex.twistedD n q).Valid)
    (hh : ¬(LieTypeIndex.twistedD n q).UsesHalfFrobenius) : twistOrder ⟨⟨_, hv⟩, hh⟩ = 2 := by
  simp only [twistOrder]
...
@[simp] theorem diagramPerm_twistedE6 (hv : (LieTypeIndex.twistedE6 q).Valid)
    (hh : ¬(LieTypeIndex.twistedE6 q).UsesHalfFrobenius) :
    diagramPerm ⟨⟨_, hv⟩, hh⟩ = graphPermE6 := by
  simp only [diagramPerm]
  rfl

@[simp] theorem twistOrder_twistedE6 (hv : (LieTypeIndex.twistedE6 q).Valid)
    (hh : ¬(LieTypeIndex.twistedE6 q).UsesHalfFrobenius) : twistOrder ⟨⟨_, hv⟩, hh⟩ = 2 := by
  simp only [twistOrder]
...
@[simp] theorem diagramPerm_trialityD4 (hv : (LieTypeIndex.trialityD4 q).Valid)
    (hh : ¬(LieTypeIndex.trialityD4 q).UsesHalfFrobenius) :
    diagramPerm ⟨⟨_, hv⟩, hh⟩ = trialityPermD4 := by
  simp only [diagramPerm]
  rfl

@[simp] theorem twistOrder_trialityD4 (hv : (LieTypeIndex.trialityD4 q).Valid)
    (hh : ¬(LieTypeIndex.trialityD4 q).UsesHalfFrobenius) : twistOrder ⟨⟨_, hv⟩, hh⟩ = 3 := by
  simp only [twistOrder]

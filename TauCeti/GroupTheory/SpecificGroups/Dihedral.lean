...
means indexing the powers of `c` by `ZMod n`, where `n` is the order of `c`; the index `i` is read
as the integer `ZMod.cast i`, and `TauCeti.dihedralHom` is the resulting homomorphism. Taking that
representative in `ℤ` rather than in `ℕ` costs nothing and covers the modulus `n = 0`, where
`ZMod 0` is `ℤ` and an element of order `0` is one of infinite order: the criterion then recognizes
...
-- `(rfl)`, not `rfl`: the body of `TauCeti.dihedralHom` is not exposed, and the parenthesised form
-- keeps these proofs out of the exported definitional-equality check.
...
theorem dihedralHom_r (hs : s * s = 1) (ht : t * t = 1) (hn : orderOf (s * t) = n) (i : ZMod n) :
    dihedralHom hs ht hn (.r i) = (s * t) ^ (ZMod.cast i : ℤ) := (rfl)
...
theorem dihedralHom_sr (hs : s * s = 1) (ht : t * t = 1) (hn : orderOf (s * t) = n) (i : ZMod n) :
    dihedralHom hs ht hn (.sr i) = s * (s * t) ^ (ZMod.cast i : ℤ) := (rfl)

/-- **A group generated by two involutions is dihedral.** If the involutions `s` and `t` generate
`G` and neither is the identity, then `TauCeti.dihedralHom` identifies `G` with the dihedral group
of order `2 * n`, where `n` is the order of `s * t`.
...
/-- **A group generated by two involutions is the dihedral group of order twice the order of their
product.** The isomorphism is `TauCeti.dihedralHom`: the rotation `r i` is the power `(s * t) ^ i`,
and the reflection `sr i` is `s * (s * t) ^ i`.
...
theorem dihedralGroupMulEquiv_apply (hs : s * s = 1) (ht : t * t = 1) (hs1 : s ≠ 1) (ht1 : t ≠ 1)
    (hn : orderOf (s * t) = n) (hgen : Subgroup.closure ({s, t} : Set G) = ⊤)
    (x : DihedralGroup n) :
    dihedralGroupMulEquiv hs ht hs1 ht1 hn hgen x = dihedralHom hs ht hn x := (rfl)

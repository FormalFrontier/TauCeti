...
## The reflection parity

The proof is the classical one through the reflection cocycle. Since a reflection `t` moves under
conjugation as a word is read off, the count of the occurrences of `t` in an inversion sequence
is not visibly a function of `π ω` alone; but its parity is. The mechanism is a `W`-action on
`W × ZMod 2`, generated by the involutions
...
Two facts about `η` then give the theorem.

* If `η (v, t) = 1` then `t` occurs in the inversion sequence of any reduced word for `v`, so
  `ℓ (v * t) < ℓ v` by `CoxeterSystem.isRightInversion_of_mem_rightInvSeq`.
* `η (t, t) = 1` for every reflection `t`, a formal consequence of the cocycle identity and of
  `η (s_i, s_i) = 1`.

Together with the cocycle identity these force the converse: if `η (v, t)` were `0` then
`η (v * t, t) = η (v, t) + η (t, t) = 1`, so `ℓ v = ℓ (v * t * t) < ℓ (v * t)`, which contradicts
`ℓ (v * t) < ℓ v`. So a right inversion of `v` occurs in the inversion sequence of every reduced
word for `v`, which is the strong exchange condition.
...
/-- A list concatenated with itself contains every element an even number of times. -/
private theorem natCast_count_self_append (t : V) (l : List V) :
...
would make the parity of `t` at `v * t` equal to `1`, so that `v = v * t * t` would be shorter than
`v * t`, which is shorter than `v`. -/
private theorem reflectionParity_of_isRightInversion {v t : W}
  ...
  have hlt := (isRightInversion_of_reflectionParity cs hvt).2
  rw [mul_assoc, h.1.mul_self, mul_one] at hlt
  have := h.2
  omega

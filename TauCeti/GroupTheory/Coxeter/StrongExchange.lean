...
public import TauCeti.Data.List.Count
...
`TauCeti.mem_rightInvSeq_iff_isRightInversion` and `TauCeti.mem_leftInvSeq_iff_isLeftInversion`
below. The root-system-level statement for a Weyl group, proved from the geometry of the positive
roots rather than from the reflection cocycle used here, is
...
private theorem isLiftable_reflectionSwap : M.IsLiftable (reflectionSwap cs) := by
  ...
  rw [← hprod, prod_map_reflectionSwap_apply, hword, hseq, List.count_reverse,
    leftInvSeq_alternatingWord_two_mul cs i i' hm, natCast_count_append_self]
  simp

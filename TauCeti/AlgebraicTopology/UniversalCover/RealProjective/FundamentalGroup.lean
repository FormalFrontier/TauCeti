...
For `2 ≤ n`, real projective `n`-space `RPⁿ` is covered by the simply connected unit sphere `Sⁿ`
via the two-sheeted antipodal quotient `mk n`. The antipodal cover is a regular covering map with
deck group `ℤˣ`.

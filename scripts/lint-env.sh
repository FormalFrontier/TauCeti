...
# `#lint` has no exclusion syntax, so instead of the command the driver calls the same
# Batteries entry points the command itself calls (`getDeclsInPackage`, `getChecks`,
# `lintCore`, `formatLinterResults`), with the explicit default-set-minus-docBlame list
# below and with the declaration list filtered (see PRIVATE DECLARATIONS). What it prints
# is the report `#lint only $LINTERS in TauCeti` would print, so every parsing and
# integrity check below applies unchanged. Running docBlame and dropping its
...
#
# PRIVATE DECLARATIONS are excluded from both passes: `Lean.isPrivateName` filters the
# `#lint` declaration list, and the docstring scan asks `isPrivateOrAutoDecl` rather than
# `isAutoDecl`. Until batteries#1831 ("don't consider private names autogenerated in
# `isAutoDecl`") the framework reported private names as autogenerated and every linter
# skipped them; it now offers them to the linters, and under the module system their
# statements cannot be checked soundly. A private equation lemma of a public definition
# reaches the module-style driver with referents that were never loaded, so `checkType`
# reports "the statement doesn't type check" and `synTaut`/`unusedArguments` fail with
# "Unknown constant" on ~700 declarations that build cleanly. The scan needs the matching
# change for a different reason: it mirrors docBlame's rules, and docBlame answers this
# same question with `isPrivateOrAutoDecl`, so without it the scan flags private
# definitions docBlame itself exempts.
# Both filters run INSIDE the driver, on real `Name`s, never on the report text. A private
# declaration pretty-prints with a trailing `✝`, and so does a declaration a PR can author
# as `«evil✝»`, so filtering the report by name shape would let any PR exempt itself from
# the environment linters.
# This is narrower than what upstream now lints (a private declaration the linters could
# judge soundly is skipped too); giving the driver an environment where private
# declarations are sound to lint is separate work.
#
...
# $LINTERS rendered as Lean name literals, for the driver's `runOnly` list.
LINTERS_NAMES=$(printf '`%s, ' $LINTERS | sed 's/, $//')
...
# The driver runs the `#lint` command's own body (see the header comment) so that the
# declaration list can drop private declarations before the linters see them; the report
# is the one `#lint only $LINTERS in TauCeti` prints. `set_option linter.hashCommand
# false` because the driver is generated, not committed; the style linter would otherwise
# flag the bare `#eval`. That trailing `#eval` prints a per-run nonce proving the process
# survived past the lint (see SECURITY MODEL).
{
  echo "module"
  sed 's/^/public import /' "$MODULE_IMPORT_LIST"
  echo
  sed "s/@LINTERS@/$LINTERS_NAMES/" <<'LEAN'
open Batteries.Tactic.Lint Lean Elab Command in
run_cmd do
  let decls := (← liftCoreM <| getDeclsInPackage `TauCeti).filter (!Lean.isPrivateName ·)
  let linters ← liftCoreM <| getChecks (slow := true) (runOnly := some [@LINTERS@]) none
  let results ← liftCoreM <| lintCore decls linters
  let fmt ← liftCoreM <| formatLinterResults results decls (groupByFilename := true)
    "in TauCeti" (runSlowLinters := true) .medium linters.size
  if results.any (!·.2.isEmpty) then logError fmt
